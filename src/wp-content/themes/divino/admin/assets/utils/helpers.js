...
const handleGetAstraPro = (
	...
	if ( ! divino_admin.pro_installed_status ) {
	...
	const buttonHTML =
		spinnerPosition === 'right'
			? `<span class="button-text">${divino_admin.plugin_activating_text}</span>${spinnerHTML}`
			: `${spinnerHTML}<span class="button-text">${divino_admin.plugin_activating_text}</span>`;
	...
	apiFetch( {
		...
		.then( ( data ) => {
			if ( data.success ) {
				window.open( divino_admin.divino_base_url, '_self' );
				return;
			}
		} )
		.catch( ( error ) => {
...
const saveSetting = debounce(
	(
		{ action = 'divino_update_admin_setting', security = divino_admin.update_nonce, key = '', value },
		dispatch,
