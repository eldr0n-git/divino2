 ...
 * @since 4.1.0
 */
class divino_API_Init extends WP_REST_Controller {
	/**
	 * Instance
	 *
	 * @var null $instance
	 ...
	 * @since 4.0.0
	 */
	private static $option_name = 'divino_admin_settings';

	/**
	 * Admin settings dataset
	 *
	 * @var array $divino_admin_settings Settings array.
	 * @since 4.0.0
	 */
	private static $divino_admin_settings = array();
	...
	public function __construct() {
		self::$divino_admin_settings = get_option( self::$option_name, array() );
	...
	public function get_admin_settings( $request ) {
		$db_option = get_option( 'divino_admin_settings', array() );

		$defaults = apply_filters(
			'divino_dashboard_rest_options',
			array(
				...
				'use_old_header_footer' => divino_get_option( 'is-header-footer-builder', false ),
				'use_upgrade_notices'   => divino_showcase_upgrade_notices(),
				'analytics_enabled'     => get_option( 'divino_analytics_optin', 'no' ) === 'yes',
			)
		);
	...
	public static function get_admin_settings_option( $key, $default = false ) {
		return isset( self::$divino_admin_settings[ $key ] ) ? self::$divino_admin_settings[ $key ] : $default;
	}
	...
	public static function update_admin_settings_option( $key, $value ) {
		$divino_admin_updated_settings         = get_option( self::$option_name, array() );
		$divino_admin_updated_settings[ $key ] = $value;
		update_option( self::$option_name, $divino_admin_updated_settings );
	}
}

divino_API_Init::get_instance();
