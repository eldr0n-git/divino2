...
$divino_banner_control = 'section-' . esc_attr( $special_type ) . '-page-title';

// If description is the only meta available in structure & its blank then no need to render banner markup.
$divino_archive_structure = divino_get_option( $divino_banner_control . '-structure', array( $divino_banner_control . '-title' ) );

if ( empty( $divino_archive_structure ) ) {
	return;
}

$divino_get_archive_description = divino_get_archive_description( $special_type );
...
$divino_data_attrs  = 'data-post-type="' . $special_type . '" data-banner-layout="' . $divino_layout_type . '"';
...
<section <?php echo wp_kses_post( $divino_attr . ' ' . $divino_data_attrs ); ?>>
	<div class="ast-container">
		...
		if ( is_customize_preview() ) {
			...
			divino_banner_elements_order( $divino_archive_structure );
		?>
	</div>
</section>
