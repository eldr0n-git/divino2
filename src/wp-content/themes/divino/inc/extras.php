 ...
 */
if ( ! function_exists( 'divino_body_font_family' ) ) {
	 ...
	 * @return string
	 */
	function divino_body_font_family() {
		...
		return apply_filters( 'divino_body_font_family', $font_family );
	}
}
 ...
 */
if ( ! function_exists( 'divino_header_breakpoint_style' ) ) {
	 ...
	 * @since 1.0.0
	 */
	function divino_header_breakpoint_style( $dynamic_css, $dynamic_css_filtered = '' ) {

		$divino_header_width = divino_get_option( 'header-main-layout-width' );

		if ( 'content' === $divino_header_width ) {
			return $dynamic_css;
		}
		...
		$dynamic_css .= divino_parse_css( $genral_global_responsive );
		$dynamic_css .= divino_parse_css( $padding_below_breakpoint, '', $header_break_point );

		// trim white space for faster page loading.
		$dynamic_css .= divino_Enqueue_Scripts::trim_css( $dynamic_css );

		return $dynamic_css;
	}
}

add_filter( 'divino_dynamic_theme_css', 'divino_header_breakpoint_style' );

/**
 * Function to filter comment form arguments
 */
if ( ! function_exists( 'divino_404_page_layout' ) ) {
	 ...
	 * @return array
	 */
	function divino_404_page_layout( $layout ) {

		if ( is_404() ) {
			$layout = 'no-sidebar';
		}

		return apply_filters( 'divino_404_page_layout', $layout );
	}
}

add_filter( 'divino_page_layout', 'divino_404_page_layout', 10, 1 );

/**
 * Function to check logo component present in which Header section.
 */
if ( ! function_exists( 'divino_is_logo_in_section' ) ) {
	...
	function divino_is_logo_in_section( $section ) {
 ...
 */
if ( ! function_exists( 'divino_get_content_layout' ) ) {
	 ...
	 */
	function divino_get_content_layout() {

		if ( is_singular() ) {

			// Old meta layout migration to new layout.
			$old_meta_content_layout = divino_get_option_meta( 'site-content-layout', '', true );

			if ( $old_meta_content_layout ) {
				$content_layout = divino_toggle_layout( 'ast-site-content-layout', 'meta', false, $old_meta_content_layout );
			} else {
				$content_layout = divino_get_option_meta( 'ast-site-content-layout', '', true );

				// If post meta value is present, apply new layout option.
				if ( $content_layout ) {
					$content_layout = divino_toggle_layout( 'ast-site-content-layout', 'meta', false );
				}
			}
			...
			if ( empty( $content_layout ) ) {
				...
				$content_layout = divino_toggle_layout( 'single-' . $post_type . '-ast-content-layout', 'single', false );
			...
			$content_layout = divino_toggle_layout( 'archive-' . $post_type . '-ast-content-layout', 'archive', false );

			if ( is_search() ) {
				$search_content_layout = divino_get_option( 'ast-search-content-layout', 'default' );
				$content_layout_key    = 'default' !== $search_content_layout ? 'ast-search-content-layout' : 'archive-post-ast-content-layout';
				$content_layout        = divino_toggle_layout( $content_layout_key, 'archive', false );
			}
		...
		return apply_filters( 'divino_get_content_layout', $content_layout );
	}
}
 ...
 * @return mixed content layout.
 */
function divino_toggle_layout( $new_content_option, $level, $post_id = false, $old_meta = false ) {

	// Dynamic layout option for meta case.
	$dynamic_layout_option = 'meta' === $level ? divino_get_option_meta( $new_content_option, '', true ) : divino_get_option( $new_content_option, 'post' === strval( get_post_type() ) ? 'narrow-width-container' : 'default' );
	$current_layout        = '';
	...
	switch ( $dynamic_layout_option ) {
		...
		case 'narrow-width-container':
			...
			if ( divino_with_third_party() && ( ! $migrated_user ) ) {
				$current_layout = 'plain-container';
			}
			break;
 ...
 */
function divino_migrate_meta_layout( $meta_layout ) {
	$new_layout = '';
	switch ( $meta_layout ) {
 ...
 */
if ( ! function_exists( 'divino_check_is_ie' ) ) {
	...
	function divino_check_is_ie() {
		...
		return apply_filters( 'divino_check_is_ie', $is_ie );
	}
}
 ...
 * Replace header logo.
 */
if ( ! function_exists( 'divino_replace_header_logo' ) ) {
	 ...
	 */
	function divino_replace_header_logo( $image, $attachment_id, $size, $icon ) {
		...
		return apply_filters( 'divino_replace_header_logo', $image );
	}
}

if ( ! function_exists( 'divino_strposa' ) ) {
	 ...
	 */
	function divino_strposa( $haystack, $needles, $offset = 0 ) {
...
if ( ! function_exists( 'divino_get_prop' ) ) {
	 ...
	 */
	function divino_get_prop( $array, $prop, $default = null ) {
 ...
 */
function divino_attr( $context, $attributes = array(), $args = array() ) {
	return divino_Attr::get_instance()->divino_attr( $context, $attributes, $args );
}
 ...
 */
function divino_get_theme_author_details() {
 ...
 */
function divino_dropdown_icon_to_menu_link( $title, $item, $args, $depth ) {
	$role = 'application';
	$icon = '';

	/**
	 * These menus are not overriden by the 'divino_Custom_Nav_Walker' class present in Addon - Nav Menu module.
	 *
	 * Hence skipping these menus from getting overriden by blank SVG Icons and adding the icons from theme.
	 *
	 * @since 3.3.0
	 */
	$divino_menu_locations = array(
...
if ( divino_Icons::is_svg_icons() ) {
	add_filter( 'nav_menu_item_title', 'divino_dropdown_icon_to_menu_link', 10, 4 );
}
 ...
 */
function divino_existing_header_footer_configs() {

	return apply_filters( 'divino_existing_header_footer_configs', true );
}
 ...
 * @return mixed
 */
function divino_calculate_spacing( $value, $operation = '', $from = '', $from_unit = '' ) {
 ...
 * @return mixed
 */
function divino_markup_open( $context, $args = array() ) {
	$defaults = array(
		'open'    => '',
		'attrs'   => array(),
		'echo'    => true,
	...
	if ( $context ) {
		$args     = apply_filters( "divino_markup_{$context}_open", $args );
		$open_tag = $args['open'] ? sprintf( $args['open'], divino_attr( $context, $args['attrs'] ) ) : '';
 ...
 * @return mixed
 */
function divino_markup_close( $context, $args = array() ) {
	$defaults = array(
		'close' => '',
		'attrs' => array(),
		'echo'  => true,
	...
	$args = wp_parse_args( $args, $defaults );
	if ( $context ) {
		$args      = apply_filters( "divino_markup_{$context}_close", $args );
 ...
 * @return bool
 */
function divino_target_rules_for_related_posts() {

	$allow_related_posts = false;
	$supported_post_type = apply_filters( 'divino_related_posts_supported_post_types', 'post' );

	if ( divino_get_option( 'enable-related-posts' ) && is_singular( $supported_post_type ) ) {
		$allow_related_posts = true;
	}

	return apply_filters( 'divino_showcase_related_posts', $allow_related_posts );
}
 ...
 * @return bool
 */
function divino_is_elemetor_active() {
 ...
 * @since  3.9.2
 */
function divino_addon_check_version( $version, $compare ) {
	return defined( 'divino_EXT_VER' ) && version_compare( divino_EXT_VER, $version, $compare );
}
 ...
 */
function divino_get_webfont_url( $url, $format = 'woff2' ) {

	// Check if already Google font URL present or not. Basically avoiding 'divino_WebFont_Loader' class rendering.
	/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	$divino_font_url = divino_get_option( 'divino_font_url', false );
	if ( $divino_font_url ) {
		return json_decode( $divino_font_url );
	}

	// Now create font URL if its not present.
	$font = divino_webfont_loader_instance( $url );
 ...
 * @param string $format The font-format. If you need to support IE, change this to "woff".
 */
function divino_load_preload_local_fonts( $url, $format = 'woff2' ) {

	// Check if cached font files data preset present or not. Basically avoiding 'divino_WebFont_Loader' class rendering.
	$divino_local_font_files = get_site_option( 'divino_local_font_files', false );

	if ( is_array( $divino_local_font_files ) && ! empty( $divino_local_font_files ) ) {
		$font_format = apply_filters( 'divino_local_google_fonts_format', $format );
		foreach ( $divino_local_font_files as $local_font ) {
			if ( $local_font ) {
 ...
 * @since 3.6.0
 */
function divino_get_transparent_header_default_value() {
	$divino_settings = divino_get_options();
	return apply_filters( 'divino_transparent_header_default_border', isset( $divino_settings['transparent-header-default-border'] ) ? $divino_settings['transparent-header-default-border'] : true );
}
 ...
 * @since 3.7.0
 */
function divino_has_gcp_typo_preset_compatibility() {
	if ( defined( 'divino_EXT_VER' ) && version_compare( divino_EXT_VER, '3.6.0', '<' ) ) {
		return false;
	}
	return true;
}
 ...
 * @return string
 */
function divino_button_default_padding_updated() {
	$divino_settings = divino_get_options();
	return apply_filters( 'divino_update_button_padding_defaults', isset( $divino_settings['btn-default-padding-updated'] ) ? $divino_settings['btn-default-padding-updated'] : true );
}
 ...
 * @return string
 */
function divino_scndry_btn_default_padding() {
	$divino_settings = divino_get_options();
	return apply_filters( 'divino_update_secondary_button_padding_defaults', isset( $divino_settings['scndry-btn-default-padding'] ) ? $divino_settings['scndry-btn-default-padding'] : true );
}
 ...
 * @return bool
 */
function divino_has_widgets_block_editor() {
 ...
 * @return bool
 */
function divino_can_remove_elementor_toc_margin_space() {
	$divino_settings = divino_get_options();
	return apply_filters( 'divino_remove_elementor_toc_margin', isset( $divino_settings['remove-elementor-toc-margin-css'] ) ? false : true );
}
 ...
 * @return bool
 */
function divino_can_add_styling_for_hr() {
	$divino_settings = divino_get_options();
	return apply_filters( 'divino_highlight_elementor_hr_tag', isset( $divino_settings['add-styling-for-hr'] ) ? false : true );
}
/**
 ...
 */
function divino_has_global_color_format_support() {
	$divino_settings = divino_get_options();
	return apply_filters( 'divino_apply_global_color_format_support', isset( $divino_settings['support-global-color-format'] ) ? false : true );
}
 ...
 * @since 4.10.0
 */
function divino_get_palette_names() {
	 ...
	 */
	return apply_filters( 'divino_get_palette_names', $palette_names );
}
 ...
 * @return void
 */
function divino_clear_theme_addon_asset_cache() {
	divino_clear_all_assets_cache();
}

add_action( 'divino_theme_update_after', 'divino_clear_theme_addon_asset_cache', 10 );
 ...
 * @return bool
 */
function divino_maybe_disable_global_color_in_elementor() {
	return apply_filters( 'divino_disable_global_colors_in_elementor', false );
}
 ...
 * @return bool
 */
function divino_check_elementor_pro_3_5_version() {
 ...
 * @return bool
 */
function divino_apply_content_background_fullwidth_layouts() {
	$divino_site_layout              = divino_get_option( 'site-layout' );
	$divino_apply_content_background = divino_get_option( 'apply-content-background-fullwidth-layouts', true );
	return $divino_apply_content_background && 'ast-box-layout' !== $divino_site_layout && 'ast-padded-layout' !== $divino_site_layout;
}
 ...
 * @since 3.5.0
 */
function divino_search_static_css() {
	$search_css = '
	...
	return divino_Enqueue_Scripts::trim_css( $search_css );
}
 ...
 * @return bool
 */
function divino_showcase_upgrade_notices() {
	return ! defined( 'divino_EXT_VER' ) && divino_get_option( 'ast-disable-upgrade-notices', true ) ? true : false;
}
 ...
 * @since 4.0.0
 */
function divino_get_font_extras( $config, $setting, $unit = false ) {
	$css = isset( $config[ $setting ] ) ? $config[ $setting ] : '';

	if ( $unit && $css ) {
		$unit_val = isset( $config[ $unit ] ) ? $config[ $unit ] : '';
		$unit_val = 'line-height-unit' === $unit ? apply_filters( 'divino_font_line_height_unit', $unit_val ) : $unit_val;
		$css     .= $unit_val;
	}
 ...
 * @since 4.0.0
 */
function divino_get_font_array_css( $font_family, $font_weight, $font_size, $font_extras, $color = '' ) {
	$font_extras_ast_option = divino_get_option(
		$font_extras,
		array(
			'line-height'         => '',
			'line-height-unit'    => 'em',
	...
	return array(
		...
		'font-family'     => divino_get_css_value( $font_family, 'font' ),
		'font-weight'     => divino_get_css_value( $font_weight, 'font' ),
		'font-size'       => ! empty( $font_size ) ? divino_responsive_font( $font_size, 'desktop' ) : '',
		'line-height'     => divino_get_font_extras( $font_extras_ast_option, 'line-height', 'line-height-unit' ),
		'text-transform'  => divino_get_font_extras( $font_extras_ast_option, 'text-transform' ),
		'letter-spacing'  => divino_get_font_extras( $font_extras_ast_option, 'letter-spacing', 'letter-spacing-unit' ),
		'text-decoration' => divino_get_font_extras( $font_extras_ast_option, 'text-decoration' ),
	);
}
 ...
 * @return string
 */
function divino_get_dynamic_image_aspect_ratio( $aspect_ratio_type, $predefined_scale, $custom_scale_width, $custom_scale_height ) {
 ...
 * @return string
 */
function divino_get_current_language_slug() {
	$lang = '';
	if ( function_exists( 'pll_current_language' ) ) {
		$lang = pll_current_language();
	}
	return apply_filters( 'divino_addon_site_current_language', $lang );
}
 ...
 * @return array
 */
function divino_get_queried_post_types() {
	$queried_post_types = array_keys(
		get_post_types(
			apply_filters(
				'divino_dynamic_get_post_types_query_args',
				array(
					'public'   => true,
					'_builtin' => false,
				)
 ...
 * @since 4.5.0
 */
function divino_get_palette_presets() {
	$color_palette_reorganize = divino_Dynamic_CSS::divino_4_8_0_compatibility();
	return array(
		'Oak'    => array(
			'#0067FF',
			'#005EE9',
 ...
 * @since 4.6.0
 */
function divino_get_blog_layout() {
	return is_search() && '' !== divino_get_option( 'ast-search-results-style' ) ? divino_get_option( 'ast-search-results-style' ) : divino_get_option( 'blog-layout' );
}
 ...
 * @since 4.6.0
 */
function divino_get_blog_posts_per_page() {
	$posts_per_page = is_search() && divino_get_option( 'ast-search-results-per-page' )
		? divino_get_option( 'ast-search-results-per-page' )
		: divino_get_option( 'blog-post-per-page' );
 ...
 * @since 4.4.0
 */
function divino_customizer_live_search_posttypes() {
	$supported_post_types = array();
	if ( is_customize_preview() ) {
		$supported_post_types = divino_get_queried_post_types();
	}
	return apply_filters( 'divino_live_search_posttypes', $supported_post_types );
}
 ...
 * @return array
 */
function divino_customizer_search_post_types_choices() {
	$all_post_types    = divino_customizer_live_search_posttypes();
 ...
 * @return bool
 */
function divino_enable_edd_featured_image_defaults() {
	$divino_settings = divino_get_options();
	return apply_filters( 'divino_enable_edd_featured_image_defaults', isset( $divino_settings['can-update-edd-featured-image-default'] ) ? false : true );
}
