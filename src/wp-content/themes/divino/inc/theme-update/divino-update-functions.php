 ...
 * @return void.
 */
function divino_clear_all_assets_cache() {
...
function divino_theme_background_updater_4_0_0() {
	...
	if ( ! isset( $admin_dashboard_settings['theme-setup-admin-migrated'] ) ) {
		...
		update_option( 'divino_admin_settings', $admin_dashboard_settings );
	}
	...
	if ( defined( 'divino_EXT_VER' ) && divino_Ext_Extension::is_active( 'scroll-to-top' ) ) {
		/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
...
function divino_theme_background_updater_4_2_0() {
	...
	if ( ! isset( $theme_options['v4-2-0-update-migration'] ) ) {
		...
		if ( isset( $theme_options['site-content-layout'] ) ) {
			$theme_options = divino_apply_layout_migration( 'site-content-layout', 'ast-site-content-layout', 'site-content-style', 'site-sidebar-style', $theme_options );
		}
		...
		foreach ( $blog_types as $blog_type ) {
			foreach ( $post_types as $post_type ) {
				...
				if ( isset( $theme_options[ $old_layout ] ) ) {
					$theme_options = divino_apply_layout_migration( $old_layout, $new_layout, $content_style, $sidebar_style, $theme_options );
				}
			}
		}
		...
		foreach ( $third_party_layouts as $layout ) {
			...
			if ( isset( $theme_options[ $old_layout ] ) ) {
				if ( 'lifterlms' === $layout ) {
					...
					$theme_options = divino_apply_layout_migration( $old_layout, $new_layout, $content_style, 'lifterlms-course-lesson-sidebar-style', $theme_options );
				}
				$theme_options = divino_apply_layout_migration( $old_layout, $new_layout, $content_style, $sidebar_style, $theme_options );
			}
		}
 ...
 */
function divino_apply_layout_migration( $old_layout, $new_layout, $content_style, $sidebar_style, $theme_options ) {
	switch ( divino_get_option( $old_layout ) ) {
...
function divino_theme_background_updater_4_6_0() {
	...
	if ( ! empty( $docs_legacy_data ) ) {
		delete_option( 'divino_docs_data' );
	}
}
 ...
 * @return void
 */
function divino_theme_background_updater_4_8_7() {
	...
	update_option( 'divino_nps_show', true );
}
 ...
 * @return void
 */
function divino_theme_background_updater_4_8_9() {
	// Bail early if the starter template is being imported.
	if ( get_option( 'divino_sites_import_started' ) === 'yes' || divino_get_option( 'new-color-labels' ) ) {
 ...
 * @return void
 */
function divino_theme_background_updater_4_10_0() {
	...
	$analytics_options = array(
		'bsf_analytics_installed_time' => 'divino_analytics_installed_time',
		'bsf_analytics_optin'          => 'divino_analytics_optin',
	);
