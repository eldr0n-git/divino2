...
if ( ! class_exists( 'divino_ExtTransparenty_Header_Markup' ) ) {
	 ...
	 * @since 1.0.0
	 */
	class divino_Ext_Transparent_Header_Markup {
		/**
		 * Member Variable
		 *
		 * @var object instance
		...
		public function __construct() {
			...
			/* Fixed header markup */
			add_action( 'divino_header', array( $this, 'transparent_header_logo' ), 1 );

			/**
			 * Metabox setup
			 */
			add_filter( 'divino_meta_box_options', array( $this, 'add_options' ) );
			add_action( 'divino_meta_box_markup_after', array( $this, 'add_options_markup' ) );

			add_action( 'divino_customizer_save', array( $this, 'customizer_save' ) );
		}
		...
		public function add_body_class( $classes ) {
			...
			$inherit_desk_logo              = divino_get_option( 'different-transparent-logo', false );
			$transparent_header_logo        = divino_get_option( 'transparent-header-logo', true );
			$transparent_header_logo_retina = divino_get_option( 'transparent-header-retina-logo', true );
		...
		public static function is_transparent_header() {
			...
			// Transparent Header.
			$enable_trans_header = divino_get_option( 'transparent-header-enable' );
			$trans_meta_option   = divino_get_option_meta( 'theme-transparent-header-meta', 'default' );
			...
			if ( $enable_trans_header ) {
				...
				if ( is_404() && '1' == divino_get_option( 'transparent-header-disable-404-page', divino_get_option( 'transparent-header-disable-archive' ) ) ) {
				...
				if ( is_search() && '1' == divino_get_option( 'transparent-header-disable-search-page', divino_get_option( 'transparent-header-disable-archive' ) ) ) {
					$enable_trans_header = false;
				}
				...
				if ( is_archive() && '1' == divino_get_option( 'transparent-header-disable-archive-pages', divino_get_option( 'transparent-header-disable-archive' ) ) ) {
					$enable_trans_header = false;
				}

				if ( is_home() && '1' == divino_get_option( 'transparent-header-disable-index' ) && ( 'posts' !== get_option( 'show_on_front' ) ) ) {
					$enable_trans_header = false;
				}

				if ( is_front_page() && 'posts' === get_option( 'show_on_front' ) && '1' == divino_get_option( 'transparent-header-disable-latest-posts-index' ) ) {
					$enable_trans_header = false;
				}

				if ( is_page() && '1' == divino_get_option( 'transparent-header-disable-page' ) ) {
					$enable_trans_header = false;
				}

				if ( is_single() && '1' == divino_get_option( 'transparent-header-disable-posts' ) ) {
					$enable_trans_header = false;
				}
			}

			if ( class_exists( 'divino_Woocommerce' ) ) {
				if ( is_product() && '1' == divino_get_option( 'transparent-header-disable-woo-products' ) ) {
					$enable_trans_header = false;
				}
			}
			...
			return apply_filters( 'divino_is_transparent_header', $enable_trans_header );
		}
		...
		public function transparent_header_logo() {
			...
			if ( self::is_transparent_header() && '1' == $inherit_desk_logo && '' !== $transparent_header_logo ) {
				...
				add_filter( 'divino_has_custom_logo', '__return_true' );
		...
		public function replace_trans_header_attr( $attr, $attachment, $size ) {
			...
			if ( $custom_logo_id == $attachment->ID ) {
				...
				if ( '1' == $diff_retina_logo ) {
					...
					$retina_logo = divino_get_option( 'transparent-header-retina-logo' );

					$attr['srcset'] = '';

					if ( apply_filters( 'divino_transparent_header_retina', true ) && '' !== $retina_logo ) {
		...
		public function add_options_markup( $meta ) {
			...
			$show_meta_field   = ! divino_check_is_bb_themer_layout();
			?>
		...
		public function customizer_save() {
			...
			if ( '' !== $trans_logo ) {
				...
				divino_Customizer::generate_logo_by_width( $trans_logo_id );
		...
		public function transparent_logo_image_sizes( $sizes, $metadata ) {
			...
			$logo_width = divino_get_option( 'transparent-header-logo-width' );
 ...
 */
divino_Ext_Transparent_Header_Markup::get_instance();
