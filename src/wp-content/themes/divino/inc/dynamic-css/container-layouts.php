 ...
 * @since 3.8.3
 */
function divino_check_any_page_builder_is_active( $post_id ) {
 ...
 * @since 3.3.0
 */
function divino_container_layout_css() {
	$container_layout = divino_get_content_layout();
	...
	$tab_one_max_breakpoint = divino_get_tablet_breakpoint( '', 1 );
	...
	$image_position    = divino_get_option( 'ast-dynamic-single-' . $current_post_type . '-article-featured-image-position-layout-1', 'behind' );
	...
	$customizer_default_update = divino_check_is_structural_setup();
	$page_title_header_padding = true === $customizer_default_update ? '2em' : '4em';
	// Transparent Header.
	$display_title = get_post_meta( absint( divino_get_post_id() ), 'site-post-title', true );
	...
	$page_container_css .= '
		...
		@media (min-width: ' . strval( divino_get_tablet_breakpoint( '', 1 ) ) . 'px) {
	...
	if ( 'page-builder' === $container_layout ) {
		...
		$divino_blog_improvements  = divino_Dynamic_CSS::divino_4_6_0_compatibility();
		$post_navigation_selector = $divino_blog_improvements ? ', .ast-page-builder-template .post-navigation' : '';
		if ( true === $customizer_default_update ) {
			...
			if ( 'disabled' !== $display_title && true === apply_filters( 'divino_stretched_layout_with_spacing', true ) && false === divino_check_any_page_builder_is_active( divino_get_post_id() ) ) {
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$page_container_css .= '
		...
		if ( false === divino_get_option( 'improve-gb-editor-ui', true ) ) {
			/** @psalm-suppress InvalidScalarArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		...
		return divino_Enqueue_Scripts::trim_css( $page_container_css );
	}
