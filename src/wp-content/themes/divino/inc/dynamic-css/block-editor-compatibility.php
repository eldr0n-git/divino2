...
if ( divino_block_based_legacy_setup() ) {
	add_filter( 'divino_dynamic_theme_css', 'divino_block_editor_compatibility_css' );
} else {
	add_filter( 'divino_dynamic_theme_css', 'divino_load_modern_block_editor_ui' );
}
 ...
 * @since 3.6.5
 */
function divino_get_block_editor_required_css() {
	return '
		.wp-block-file {
			display: flex;
			align-items: center;
 ...
 * @since 3.6.5
 */
function divino_block_editor_compatibility_css( $dynamic_css ) {

	if ( divino_Dynamic_CSS::is_block_editor_support_enabled() ) {
		...
		$dynamic_css .= divino_Enqueue_Scripts::trim_css( $compatibility_css );
	}

	/** @psalm-suppress InvalidScalarArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	if ( divino_get_option( 'improve-gb-editor-ui', true ) ) {
		/** @psalm-suppress InvalidScalarArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$is_site_rtl = is_rtl();

		$editor_improvement_css = divino_get_block_editor_required_css();
	...
	$dynamic_css .= divino_Enqueue_Scripts::trim_css( $editor_improvement_css );
 ...
 * @since 3.8.0
 */
function divino_load_modern_block_editor_ui( $dynamic_css ) {
	$dynamic_css             .= divino_get_block_editor_required_css();
	$ltr_left                 = is_rtl() ? 'right' : 'left';
	$ltr_right                = is_rtl() ? 'left' : 'right';
	$divino_block_editor_v2_ui = divino_get_option( 'wp-blocks-v2-ui', true );
	$ast_container_width      = divino_get_option( 'site-content-width', 1200 ) . 'px';
	$blocks_spacings          = divino_WP_Editor_CSS::divino_get_block_spacings();

	/** @psalm-suppress InvalidCast */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	$tablet_breakpoint = (string) divino_get_tablet_breakpoint();
	/** @psalm-suppress InvalidCast */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort

	/** @psalm-suppress InvalidCast */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	$mobile_breakpoint = (string) divino_get_mobile_breakpoint();
	/** @psalm-suppress InvalidCast */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	...
	$ast_content_width = apply_filters( 'divino_block_content_width', $divino_block_editor_v2_ui ? $ast_container_width : '910px' );
	$ast_wide_width    = apply_filters( 'divino_block_wide_width', $divino_block_editor_v2_ui ? 'calc(' . esc_attr( $ast_container_width ) . ' + var(--wp--custom--ast-default-block-left-padding) + var(--wp--custom--ast-default-block-right-padding))' : $ast_container_width );
	...
	$spectra_gutenberg_compat_css = divino_Dynamic_CSS::spectra_gutenberg_compat_css();
	$v4_block_editor_compat       = divino_Dynamic_CSS::v4_block_editor_compat();
	...
	$divino_wide_particular_selector = $divino_block_editor_v2_ui ? '.entry-content[data-ast-blocks-layout] > .alignwide' : '.entry-content[data-ast-blocks-layout] > .alignwide, .entry-content[data-ast-blocks-layout] .wp-block-cover__inner-container, .entry-content[data-ast-blocks-layout] > p';
	$divino_full_stretched_selector  = $divino_block_editor_v2_ui ? '.ast-plain-container.ast-no-sidebar .entry-content > .alignfull, .ast-page-builder-template .ast-no-sidebar .entry-content > .alignfull' : '.ast-plain-container.ast-no-sidebar .entry-content .alignfull, .ast-page-builder-template .ast-no-sidebar .entry-content .alignfull';
	...
	if ( $divino_block_editor_v2_ui ) {
		$single_post_continer_spacing = divino_get_option( 'single-post-inside-spacing' );

		$container_lg_spacing = true === divino_check_is_structural_setup() ? '3' : '6.67';

		$divino_continer_left_spacing  = defined( 'divino_EXT_VER' ) && divino_responsive_spacing( $single_post_continer_spacing, 'left', 'desktop' ) ? divino_responsive_spacing( $single_post_continer_spacing, 'left', 'desktop', $container_lg_spacing ) : 'var(--ast-container-default-xlg-padding)';
		$divino_continer_right_spacing = defined( 'divino_EXT_VER' ) && divino_responsive_spacing( $single_post_continer_spacing, 'right', 'desktop' ) ? divino_responsive_spacing( $single_post_continer_spacing, 'right', 'desktop', $container_lg_spacing ) : 'var(--ast-container-default-xlg-padding)';

		$alignwide_1200_left_negative_margin  = $divino_continer_left_spacing ? 'calc(-1 * min(' . $divino_continer_left_spacing . ', 20px))' : '-20px';
		$alignwide_1200_right_negative_margin = $divino_continer_right_spacing ? 'calc(-1 * min(' . $divino_continer_right_spacing . ', 20px))' : '-20px';

		$heading_width_comp          = divino_Dynamic_CSS::divino_4_8_0_compatibility() ? 'none' : 'var(--wp--custom--ast-content-width-size)';
		$container_margin_left_comp  = divino_Dynamic_CSS::divino_4_8_4_compatibility() ? 'auto' : 'calc(-1 * var(--wp--custom--ast-default-block-left-padding) )';
		$container_margin_right_comp = divino_Dynamic_CSS::divino_4_8_4_compatibility() ? 'auto' : 'calc(-1 * var(--wp--custom--ast-default-block-right-padding) )';
		...
		$dynamic_css .= '
			...
			@media(min-width: 1201px) {
				.ast-separate-container .entry-content > .alignfull {
					margin-left: calc(-1 * ' . $divino_continer_left_spacing . ' );
					margin-right: calc(-1 * ' . $divino_continer_right_spacing . ' );
				}
	...
	$dynamic_css .= divino_parse_css( $mobile_css, '', $mobile_breakpoint );
	/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	...
	$lists_padding = divino_Dynamic_CSS::divino_4_6_0_compatibility() ? 'padding-' . $direction . ': 20px;' : '';
