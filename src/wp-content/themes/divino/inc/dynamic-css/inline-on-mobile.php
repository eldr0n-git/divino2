...
add_filter( 'divino_dynamic_theme_css', 'divino_inline_on_mobile_css' );
 ...
 * @return string
 */
function divino_inline_on_mobile_css( $dynamic_css ) {

	$inline_on_mobile_enable = false;
	for ( $index = 1; $index <= divino_Builder_Helper::$component_limit; $index++ ) {
		if ( false === divino_get_option( 'header-menu' . $index . '-menu-stack-on-mobile' ) ) {
	...
	return $dynamic_css . divino_Enqueue_Scripts::trim_css( $inline_on_mobile_css );
}
