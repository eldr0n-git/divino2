	...
	if ( divino_check_current_post_comment_enabled() || 0 < get_comments_number() ) {
		...
		$desktop_comment_global = array(
			'.comment-reply-title'                         => array(
				'font-size' => divino_get_font_css_value( (int) $body_font_size_desktop * 1.66666 ),
			),
			// Single Post Meta.
		...
		$update_customizer_defaults = true === divino_check_is_structural_setup();
      ...
      .ast-separate-container .ast-comment-list .bypostauthor .bypostauthor {
		...
		$global_button_comment_mobile = array(
			...
			'.comment-reply-title'                         => array(
				'font-size' => ! empty( $body_font_size['mobile'] ) ? divino_get_font_css_value( (int) $body_font_size['mobile'] * 1.66666, 'px', 'mobile' ) : '',
			),
			'.ast-comment-list #cancel-comment-reply-link' => array(
		...
		$global_button_comment_tablet = array(
			...
			'.comment-reply-title'                         => array(
				'font-size' => ! empty( $body_font_size['tablet'] ) ? divino_get_font_css_value( (int) $body_font_size['tablet'] * 1.66666, 'px', 'tablet' ) : '',
			),
			'.ast-comment-list #cancel-comment-reply-link' => array(
		...
		$dynamic_css .= divino_parse_css( $global_button_comment_tablet, '', divino_get_tablet_breakpoint() );
		...
		return $dynamic_css .= divino_parse_css( $global_button_tablet_lang_direction_css, '', divino_get_tablet_breakpoint() );
	}
	return $dynamic_css;
}
