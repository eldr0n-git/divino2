...
add_filter( 'divino_dynamic_theme_css', 'divino_pagination_css', 11 );
 ...
 * @since 3.3.0
 */
function divino_pagination_css( $dynamic_css ) {

	if ( divino_check_pagination_enabled() || false !== apply_filters( 'divino_enable_pagination_css', false ) ) {
		...
		$pagination_color_output = array(
			...
			'.ast-pagination a, .nav-links a'    => array(
				'color' => esc_attr( divino_Dynamic_CSS::divino_4_6_0_compatibility() ? $text_color : $link_color ),
			),
			'.ast-pagination a:hover, .ast-pagination a:focus, .ast-pagination > span:hover:not(.dots), .ast-pagination > span.current' => array(
				'color' => esc_attr( $link_hover_color ),
			),
		);
		$dynamic_css            .= divino_parse_css( $pagination_color_output );
		...
		return $dynamic_css .= divino_Enqueue_Scripts::trim_css( $pagination_static_css );
	}
	return $dynamic_css;
}
