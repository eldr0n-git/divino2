...
add_filter( 'divino_dynamic_theme_css', 'divino_comments_css', 11 );
 ...
 * @since 3.2.0
 */
function divino_comments_css( $dynamic_css ) {

	if ( divino_check_current_post_comment_enabled() || 0 < get_comments_number() ) {
		...
		$is_site_rtl                 = is_rtl();
		$reply_title_space_threshold = divino_Dynamic_CSS::divino_4_4_0_compatibility() ? 1.3 : 1.66666;
		...
		$desktop_comment_global = array(
			'.comment-reply-title'                         => array(
				'font-size' => divino_get_font_css_value( (int) $body_font_size_desktop * $reply_title_space_threshold ),
			),
			// Single Post Meta.
			'.ast-comment-meta'                            => array(
				'line-height' => '1.666666667',
				'color'       => esc_attr( $link_color ),
				'font-size'   => divino_get_font_css_value( (int) $body_font_size_desktop * 0.8571428571 ),
			),
			'.ast-comment-list #cancel-comment-reply-link' => array(
				'font-size' => divino_responsive_font( $body_font_size, 'desktop' ),
			),
		);
		$dynamic_css .= divino_parse_css( $desktop_comment_global );

		$update_customizer_strctural_defaults = true === divino_check_is_structural_setup();
		...
		$blog_improvements                    = divino_Dynamic_CSS::divino_4_6_0_compatibility();
		...
		$single_post_comment_css = '.comments-title {
          ...
          .comment-reply-title {
            font-weight: ' . esc_attr( divino_Dynamic_CSS::divino_4_4_0_compatibility() ? '600' : 'normal' ) . ';
            line-height: 1.65;
          }
		...
		if ( divino_Dynamic_CSS::divino_4_6_0_compatibility() ) {
			...
			if ( $post_with_boxed_layout && 'inside' !== divino_get_option( 'comments-box-placement', '' ) ) {
				$single_post_comment_css .= '
		...
		$dynamic_css .= divino_Enqueue_Scripts::trim_css( $single_post_comment_css );
		...
		$dynamic_css                 .= divino_parse_css( $static_layout_css_min_comment, divino_get_tablet_breakpoint( '', '1' ) );
		$global_button_comment_mobile = array(
			'.ast-separate-container .ast-comment-list li.depth-1' => array(
				'padding'       => divino_Dynamic_CSS::divino_4_6_0_compatibility() ? '' : '1.5em 1em',
				'margin-bottom' => divino_Dynamic_CSS::divino_4_4_0_compatibility() ? '0' : '1.5em',
			),
			...
			// Single Post Meta.
			'.ast-comment-meta'                            => array(
				'font-size' => ! empty( $body_font_size['mobile'] ) ? divino_get_font_css_value( (int) $body_font_size['mobile'] * 0.8571428571, 'px', 'mobile' ) : '',
			),
			'.comment-reply-title'                         => array(
				'font-size' => ! empty( $body_font_size['mobile'] ) ? divino_get_font_css_value( (int) $body_font_size['mobile'] * $reply_title_space_threshold, 'px', 'mobile' ) : '',
			),
			'.ast-comment-list #cancel-comment-reply-link' => array(
		...
		$dynamic_css .= divino_parse_css( $global_button_comment_mobile, '', divino_get_mobile_breakpoint() );
		...
		$global_button_comment_tablet = array(
			...
			'.ast-comment-meta'                            => array(
				...
				'font-size' => ! empty( $body_font_size['tablet'] ) ? divino_get_font_css_value( (int) $body_font_size['tablet'] * 0.8571428571, 'px', 'tablet' ) : '',
			),
			'.ast-separate-container .ast-comment-list li.depth-1' => array(
				'padding' => divino_Dynamic_CSS::divino_4_6_0_compatibility() ? '' : '1.5em 2.14em',
			),
			'.ast-separate-container .comment-respond'     => array(
				'padding' => divino_Dynamic_CSS::divino_4_6_0_compatibility() ? '' : '2em 2.14em',
			),
			'.comment-reply-title'                         => array(
				'font-size' => ! empty( $body_font_size['tablet'] ) ? divino_get_font_css_value( (int) $body_font_size['tablet'] * $reply_title_space_threshold, 'px', 'tablet' ) : '',
			),
			'.ast-comment-list #cancel-comment-reply-link' => array(
		...
		if ( divino_Dynamic_CSS::divino_4_6_0_compatibility() ) {
			$dynamic_css .= '
				...
				@media(min-width: ' . strval( divino_get_mobile_breakpoint( '', 1 ) ) . 'px) {
					header.ast-comment-meta {
						display: flex;
						width: 100%;
						margin-bottom: 0;
					...
					.ast-comment .comment-reply-link:hover {
						background: ' . divino_get_option( 'theme-color' ) . ';
						color: #fff;
					}
				...
				@media(min-width: ' . strval( divino_get_tablet_breakpoint() ) . 'px) {
			...
			';
			if ( 'above' === divino_get_option( 'comment-form-position' ) ) {
				$dynamic_css .= '
					.comment-respond {
			...
			$comments_section_placement = divino_get_option( 'comments-box-placement', '' );
		...
		return $dynamic_css .= divino_parse_css( $global_button_comment_tablet, '', divino_get_tablet_breakpoint() );
	}
	return $dynamic_css;
}
