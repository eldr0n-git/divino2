...
/**
 * divino_After_Setup_Theme initial setup
 *
 * @since 1.0.0
 */
if ( ! class_exists( 'divino_After_Setup_Theme' ) ) {

	/**
	 * divino_After_Setup_Theme initial setup
	 */
	class divino_After_Setup_Theme {
		/**
		 * Instance
		 *
		 * @var $instance
		...
		public function setup_theme() {
			...
			if ( class_exists( 'divino_Starter_Content', false ) ) {
				$divino_starter_content = new divino_Starter_Content();
				add_theme_support( 'starter-content', $divino_starter_content->get() );
			}
			...
			$dir_name    = 'minified';
			$file_prefix = '.min';
			if ( apply_filters( 'divino_theme_editor_style', true ) ) {
				add_editor_style( 'assets/css/' . $dir_name . '/editor-style' . $file_prefix . '.css' );
			}

			if ( apply_filters( 'divino_fullwidth_oembed', true ) ) {
			...
			if ( true === apply_filters( 'divino_rank_math_theme_support', true ) ) {
				add_theme_support( 'rank-math-breadcrumbs' );
			}

			// Native AMP Support.
			if ( true === apply_filters( 'divino_amp_support', true ) ) {
				add_theme_support(
					'amp',
					apply_filters(
						'divino_amp_theme_features',
						array(
							'paired' => true,
						)
					)
		...
		public function modify_theme_palette_names( $theme_json ) {
			...
			if ( ! empty( $json_data['settings']['color']['palette']['theme'] ) ) {
				...
				$color_labels = divino_Global_Palette::get_palette_labels(); // Use the reusable function for labels.
				$name_pair    = array();
			...
			if ( ! isset( $content_width ) ) {
				...
				if ( is_home() || is_post_type_archive( 'post' ) ) {
					...
					if ( 'custom' === $blog_width ) {
						$content_width = apply_filters( 'divino_content_width', divino_get_option( 'blog-max-width', 1200 ) );
					} else {
						$content_width = apply_filters( 'divino_content_width', divino_get_option( 'site-content-width', 1200 ) );
					}
				} elseif ( is_single() ) {
					...
					if ( 'post' === get_post_type() ) {
						...
						if ( 'custom' === $single_post_max ) {
							$content_width = apply_filters( 'divino_content_width', divino_get_option( 'blog-single-max-width', 1200 ) );
						} else {
							$content_width = apply_filters( 'divino_content_width', divino_get_option( 'site-content-width', 1200 ) );
						}
					}
		...
		public function responsive_oembed_wrapper( $html, $url, $attr, $core_yt_block = false ) {
			$add_divino_oembed_wrapper = apply_filters( 'divino_responsive_oembed_wrapper_enable', true );
			$ast_embed_wrapper_class  = apply_filters( 'divino_embed_wrapper_class', '' );

			$allowed_providers = apply_filters(
				'divino_allowed_fullwidth_oembed_providers',
				array(
					'vimeo.com',
					'youtube.com',
					'youtu.be',
			...
			if ( divino_strposa( $url, $allowed_providers ) && $add_divino_oembed_wrapper ) {
				if ( $core_yt_block ) {
		 ...
		 * @return array
		 */
		public function divino_enforce_woo_shortcode_pages( $pages_data ) {
 ...
 */
divino_After_Setup_Theme::get_instance();
