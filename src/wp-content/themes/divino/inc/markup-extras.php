...
add_action( 'wp_head', 'divino_pingback_header' );

/**
 * Add a pingback url auto-discovery header for singularly identifiable articles.
 */
function divino_pingback_header() {
 ...
 * Schema for <body> tag.
 */
if ( ! function_exists( 'divino_schema_body' ) ) {
	 ...
	 * @since 1.0.0
	 */
	function divino_schema_body() {

		if ( true !== apply_filters( 'divino_schema_enabled', true ) ) {
			return;
		}
		...
		// Get the result.
		$result = apply_filters( 'divino_schema_body_itemtype', $itemtype );

		// Return our HTML.
		echo apply_filters( 'divino_schema_body', "itemtype='https://schema.org/" . esc_attr( $result ) . "' itemscope='itemscope'" ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}
 ...
 */
if ( ! function_exists( 'divino_body_classes' ) ) {
	 ...
	 * @return array
	 */
	function divino_body_classes( $classes ) {
		if ( wp_is_mobile() ) {
		...
		$content_layout   = divino_apply_boxed_layouts( $content_layout, $is_boxed, $is_sidebar_boxed );
		...
		if ( ! divino_Builder_Helper::$is_header_footer_builder_active ) {
			...
			$outside_menu = divino_get_option( 'header-display-outside-menu' );
			 ...
			 */
			$header_content_layout = divino_get_option( 'header-main-layout-width' );
		...
		if ( 'no-sidebar' !== $sidebar_layout ) {
			if ( divino_get_option( 'site-sticky-sidebar' ) ) {
				$classes[] = 'ast-sticky-sidebar';
			}
		}

		// Add class for logo svg icon.
		if ( divino_logo_svg_icon() ) {
...
add_filter( 'body_class', 'divino_body_classes' );
 ...
 * @return bool
 */
function divino_is_content_style_boxed( $post_id = false ) {

	$post_type            = strval( get_post_type() );
	$blog_type            = is_singular() ? 'single' : 'archive';
	$global_content_style = divino_get_option( 'site-content-style' );
	$meta_content_style   = divino_get_option_meta( 'site-content-style', '', true );
	$is_boxed             = false;
	...
	$content_style = divino_get_option( $blog_type . '-' . $post_type . '-content-style', '' );

	// Third party compatibility.
	$third_party = divino_with_third_party();
	if ( ! empty( $third_party ) ) {
		$third_party_content_style = divino_get_option( $third_party . '-content-style', '' );

		if ( in_array( $third_party, array( 'lifterlms', 'learndash' ) ) && ! in_array( $post_type, divino_Posts_Structure_Loader::get_supported_post_types() ) && empty( $meta_content_style ) ) {
			$blog_type = '';
		}
		...
		$third_party_meta_page = divino_third_party_archive_meta( 'site-content-style' );
	...
	// Search.
	if ( is_search() ) {
		$content_style = divino_get_option( 'ast-search-content-style', '' );
	...
	return apply_filters( 'divino_is_content_layout_boxed', $is_boxed );
}
 ...
 */
function divino_with_third_party( $is_sidebar_option = false ) {
	...
	if ( class_exists( 'Easy_Digital_Downloads' ) && divino_is_edd_page() ) {
		return 'edd';
	}
 ...
 */
function divino_is_sidebar_style_boxed( $post_id = false ) {

	$post_type            = divino_get_post_type();
	$blog_type            = is_singular() ? 'single' : 'archive';
	$global_sidebar_style = divino_get_option( 'site-sidebar-style' );
	$meta_sidebar_style   = divino_get_option_meta( 'site-sidebar-style', '', true );
	...
	$sidebar_style = divino_get_option( $blog_type . '-' . $post_type . '-sidebar-style', '' );

	// Third party compatibility.
	$third_party = divino_with_third_party( true );
	if ( ! empty( $third_party ) ) {
		$third_party_sidebar_style = divino_get_option( $third_party . '-sidebar-style', '' );

		if ( in_array( $third_party, array( 'lifterlms', 'learndash' ) ) && ! in_array( $post_type, divino_Posts_Structure_Loader::get_supported_post_types() ) && empty( $meta_sidebar_style ) ) {
			$blog_type = '';
		}
		...
		// Third party shop/archive page meta case.
		$third_party_meta_page = divino_third_party_archive_meta( 'site-sidebar-style' );
	...
	// Search.
	if ( is_search() ) {
		$sidebar_style = divino_get_option( 'ast-search-sidebar-style', '' );
		if ( ! empty( $sidebar_style ) && 'default' !== $sidebar_style ) {
			$is_sidebar_boxed = ( 'boxed' === $sidebar_style );
		}
	}

	return apply_filters( 'divino_is_sidebar_layout_boxed', $is_sidebar_boxed );
}
 ...
 */
function divino_apply_boxed_layouts( $content_layout, $is_boxed, $is_sidebar_boxed, $post_id = false ) {
 ...
 */
function divino_third_party_archive_meta( $option ) {
	...
	if ( 'woocommerce' === $third_party && ( is_shop() || is_product_taxonomy() ) ) {
		...
		// @codingStandardsIgnoreEnd
	} elseif ( 'edd' === $third_party && divino_is_edd_single_page() ) {
		$page_id = get_the_ID();
...
if ( ! function_exists( 'divino_number_pagination' ) ) {
	 ...
	 */
	function divino_number_pagination() {
		global $wp_query;
		$enabled = apply_filters( 'divino_pagination_enabled', true );
		...
		the_posts_pagination(
			array(
				'prev_text'          => divino_default_strings( 'string-blog-navigation-previous', false ),
		...
		echo '</div>';
		$output = ob_get_clean();
		echo apply_filters( 'divino_pagination_markup', $output ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
	}
}

add_action( 'divino_pagination', 'divino_number_pagination' );

/**
 * Return or echo site logo markup.
 */
if ( ! function_exists( 'divino_logo' ) ) {
	 ...
	 */
	function divino_logo( $device = 'desktop', $echo = true ) {

		$logo_svg_icon        = divino_logo_svg_icon();
		$site_tagline         = divino_get_option( 'display-site-tagline-responsive' );
		$display_site_tagline = $site_tagline['desktop'] || $site_tagline['tablet'] || $site_tagline['mobile'] ? true : false;
		$site_title           = divino_get_option( 'display-site-title-responsive' );
		...
		$html            = '';
		$has_custom_logo = apply_filters( 'divino_has_custom_logo', has_custom_logo() );
		$trans_logo      = divino_get_option( 'transparent-header-logo' );
		$diff_trans_logo = divino_get_option( 'different-transparent-logo' );

		// Site logo.
		if ( ( $has_custom_logo && ! empty( $ast_custom_logo_id ) ) || ( true === $diff_trans_logo && ! empty( $trans_logo ) ) ) {

			if ( apply_filters( 'divino_replace_logo_width', true ) ) {
				add_filter( 'wp_get_attachment_image_src', 'divino_replace_header_logo', 10, 4 );
			}
			...
			if ( apply_filters( 'divino_replace_logo_width', true ) ) {
				remove_filter( 'wp_get_attachment_image_src', 'divino_replace_header_logo', 10 );
			}
		}
		...
		$html .= divino_get_site_title_tagline( $display_site_title, $display_site_tagline, $device );

		$html = apply_filters( 'divino_logo', $html, $display_site_title, $display_site_tagline );
 ...
 */
function divino_add_custom_logo_attributes( $html ) {
	$attributes = '';
	return str_replace( 'rel="home"', 'rel="home"' . apply_filters( 'divino_custom_logo_attributes', $attributes ) . '', $html );
}

add_filter( 'get_custom_logo', 'divino_add_custom_logo_attributes' );
 ...
 * @since 4.7.0
 */
function divino_logo_svg_icon() {

	if ( ! apply_filters( 'divino_enable_logo_svg_icon', divino_get_option( 'use-logo-svg-icon' ) ) ) {
		return '';
	}
	...
	$logo_svg_code = '';
	$logo_svg_icon = divino_get_option( 'logo-svg-icon' );
	...
	switch ( $logo_svg_icon['type'] ) {
		case 'icon-library':
			$svg_icons = function_exists( 'divino_get_logo_svg_icons_array' ) ? divino_get_logo_svg_icons_array() : array();
 ...
 */
function divino_get_site_title_tagline( $display_site_title, $display_site_tagline, $device = 'desktop' ) {
	$html = '';

	if ( ! apply_filters( 'divino_disable_site_identity', false ) ) {

		// Site Title.
		$tag = apply_filters( 'divino_show_site_title_h1_tag', false ) ? 'h1' : 'span';
		...
		$site_title_markup = apply_filters(
			...
			sprintf(
				 ...
				 */
				apply_filters( 'divino_site_title_tag', $tag ),
				/**
				 ...
				 */
				esc_url( apply_filters( 'divino_site_title_href', home_url( '/' ) ) ),
				/**
				 ...
				 */
				apply_filters( 'divino_site_title', get_bloginfo( 'name' ) ),
				divino_attr(
					'site-title',
					array(
						'class' => 'site-title',
		...
		$site_tagline_markup = apply_filters(
			'divino_site_description_markup',
			sprintf(
				 ...
				 * @since 1.8.5
				 */
				apply_filters( 'divino_site_tagline_tag', 'p' ),
				/**
				 ...
				 */
				apply_filters( 'divino_site_description', get_bloginfo( 'description' ) )
			)
		);
 ...
 */
if ( ! function_exists( 'divino_get_dynamic_header_content' ) ) {
	 ...
	 */
	function divino_get_dynamic_header_content( $option ) {

		$output  = array();
		$section = divino_get_option( $option );

		switch ( $section ) {

			case 'search':
					$output[] = divino_get_search( $option );
				break;

			case 'text-html':
					$output[] = divino_get_custom_html( $option . '-html' );
				break;

			case 'widget':
					$output[] = divino_get_custom_widget( $option );
				break;

			case 'button':
					$output[] = divino_get_custom_button( $option . '-button-text', $option . '-button-link-option', $option . '-button-style' );
				break;

			default:
					$output[] = apply_filters( 'divino_get_dynamic_header_content', '', $option, $section );
				break;
		}

		return apply_filters( 'divino_get_dynamic_header_content_final', $output );
	}
}
 ...
 */
if ( ! function_exists( 'divino_get_search_icon' ) ) {
	/**
	 ...
	 * @since 4.8.12
	 */
	function divino_get_search_icon( $option = '', $device = '' ) {
		/**
		 ...
		 */
		$icon_markup = apply_filters( 'divino_get_search_icon', '', $option, $device );
		...
		return divino_Icons::get_icons( 'search' );
	}
}
 ...
 */
if ( ! function_exists( 'divino_get_search' ) ) {
	 ...
	 */
	function divino_get_search( $option = '', $device = '' ) {
		ob_start();
		?>
		...
		return apply_filters( 'divino_get_search', $search_html, $option, $device );
	}
}
 ...
 */
if ( ! function_exists( 'divino_get_custom_html' ) ) {
 ...
 * Get custom Button.
 */
if ( ! function_exists( 'divino_get_custom_button' ) ) {
	 ...
	 */
	function divino_get_custom_button( $button_text = '', $button_options = '', $button_style = '' ) {

		$custom_html    = '';
		$button_classes = '';
		$button_text    = divino_get_i18n_option( $button_text, divino_Builder_Helper::get_translatable_string( $button_text ) );
		$button_style   = divino_get_option( $button_style );
		$outside_menu   = divino_get_option( 'header-display-outside-menu' );

		$header_button = divino_get_option( $button_options );
		...
		if ( defined( 'SUBDOMAIN_INSTALL' ) && ! SUBDOMAIN_INSTALL ) {
			$enable_subdirectory_path = apply_filters( 'divino_enable_subdirectory_path', false );
			if (
		...
		$outside_menu_item = apply_filters( 'divino_convert_link_to_button', $outside_menu );
 ...
 */
if ( ! function_exists( 'divino_get_custom_widget' ) ) {
	 ...
	 */
	function divino_get_custom_widget( $option_name = '' ) {
		...
		echo '<div class="ast-' . esc_attr( $widget_id ) . '-area"' . apply_filters( 'divino_sidebar_data_attrs', '', $widget_id ) . '>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				divino_get_sidebar( $widget_id );
		echo '</div>';
 ...
 */
if ( ! function_exists( 'divino_get_small_footer' ) ) {
	 ...
	 */
	function divino_get_small_footer( $section = '' ) {

		$small_footer_type = divino_get_option( $section );
		$output            = null;

		switch ( $small_footer_type ) {
			case 'menu':
					$output = divino_get_small_footer_menu();
				break;

			case 'custom':
					$output = divino_get_small_footer_custom_text( $section . '-credit' );
				break;

			case 'widget':
					$output = divino_get_custom_widget( $section );
				break;
		}
 ...
 */
if ( ! function_exists( 'divino_get_small_footer_custom_text' ) ) {
	 ...
	 */
	function divino_get_small_footer_custom_text( $option = '' ) {
		$output = $option;
 ...
 */
if ( ! function_exists( 'divino_get_small_footer_menu' ) ) {
	 ...
	 * @return html
	 */
	function divino_get_small_footer_menu() {
 ...
 */
if ( ! function_exists( 'divino_header_markup' ) ) {
	 ...
	 * @since 1.0.0
	 */
	function divino_header_markup() {

		do_action( 'divino_header_markup_before' );
		?>
		<header
		<?php
				echo wp_kses_post(
					divino_attr(
						...
						array(
							'id'    => 'masthead',
							'class' => join( ' ', divino_get_header_classes() ),
						)
					)
				);
		?>
		>
			...
			divino_masthead_bottom();

			do_action( 'divino_sticky_header_markup' );
			do_action( 'divino_bottom_header_after_markup' );
			?>
		</header><!-- #masthead -->
		<?php

		do_action( 'divino_header_markup_after' );
	}
}

add_action( 'divino_header', 'divino_header_markup' );

/**
 * Function to get site title/logo
 */
if ( ! function_exists( 'divino_site_branding_markup' ) ) {
	 ...
	 * @since 1.0.0
	 */
	function divino_site_branding_markup() {
		?>
...
add_action( 'divino_masthead_content', 'divino_site_branding_markup', 8 );

/**
 * Function to get Toggle Button Markup
 */
if ( ! function_exists( 'divino_toggle_buttons_markup' ) ) {
	 ...
	 * @since 1.0.0
	 */
	function divino_toggle_buttons_markup() {
		$disable_primary_navigation = divino_get_option( 'disable-primary-nav' );
		$custom_header_section      = divino_get_option( 'header-main-rt-section' );
		/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$hide_custom_menu_mobile = divino_get_option( 'hide-custom-menu-mobile', false );
		$above_header_merge      = divino_get_option( 'above-header-merge-menu' );
		$above_header_on_mobile  = divino_get_option( 'above-header-on-mobile' );
		$below_header_merge      = divino_get_option( 'below-header-merge-menu' );
		$below_header_on_mobile  = divino_get_option( 'below-header-on-mobile' );
		$menu_bottons            = true;
		...
		if ( apply_filters( 'divino_enable_mobile_menu_buttons', $menu_bottons ) ) {
			?>
		<div class="ast-mobile-menu-buttons">

			<?php divino_masthead_toggle_buttons_before(); ?>

			<?php divino_masthead_toggle_buttons(); ?>

			<?php divino_masthead_toggle_buttons_after(); ?>
...
add_action( 'divino_masthead_content', 'divino_toggle_buttons_markup', 9 );

/**
 * Function to get Primary navigation menu
 */
if ( ! function_exists( 'divino_primary_navigation_markup' ) ) {
	 ...
	 * @since 1.0.0
	 */
	function divino_primary_navigation_markup() {
		...
		if ( $disable_primary_navigation ) {
			...
			$display_outside = divino_get_option( 'header-display-outside-menu' );
			...
			if ( 'none' !== $custom_header_section && ! $display_outside ) {
				 ...
				 * @since 1.4.0
				 */
				do_action( 'divino_main_header_custom_menu_item_before' );

				echo divino_masthead_get_menu_items(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
				 ...
				 * @since 1.4.0
				 */
				do_action( 'divino_main_header_custom_menu_item_after' );
			...
			$submenu_class = apply_filters( 'divino_primary_submenu_border_class', ' submenu-with-border' );
			 ...
			 * @var Array
			 */
			$primary_menu_classes = apply_filters( 'divino_primary_menu_classes', array( 'main-header-menu', 'ast-menu-shadow', 'ast-nav-menu', 'ast-flex', 'ast-justify-content-flex-end', $submenu_class ) );
			...
			if ( has_nav_menu( 'primary' ) ) {
				...
				echo '<div ' . divino_attr( 'ast-main-header-bar-alignment' ) . '>'; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
...
add_action( 'divino_masthead_content', 'divino_primary_navigation_markup', 10 );
 ...
 */
function divino_menu_anchor_class_for_nav_menus( $atts ) {
...
add_filter( 'nav_menu_link_attributes', 'divino_menu_anchor_class_for_nav_menus', 11 );
 ...
 * @param array $atts   An array of all parameters assigned to menu anchors.
 */
function divino_menu_anchor_class_for_page_menus( $atts ) {
...
add_filter( 'page_menu_link_attributes', 'divino_menu_anchor_class_for_page_menus' );
 ...
 */
function divino_page_css_class( $css_class, $page, $depth, $args, $current_page ) {
	$css_class[] = 'menu-item';
...
add_filter( 'page_css_class', 'divino_page_css_class', 20, 5 );

/**
 * Function to get site Footer
 */
if ( ! function_exists( 'divino_footer_markup' ) ) {
	...
	function divino_footer_markup() {
		...
		<?php
				echo wp_kses_post(
					divino_attr(
						...
						array(
							'id'    => 'colophon',
							'class' => join( ' ', divino_get_footer_classes() ),
						)
					)
				);
		?>
		>
			...
			<?php divino_footer_content_top(); ?>

			<?php divino_footer_content(); ?>

			<?php divino_footer_content_bottom(); ?>
...
add_action( 'divino_footer', 'divino_footer_markup' );

/**
 * Function to get Header Breakpoint
 */
if ( ! function_exists( 'divino_header_break_point' ) ) {
	 ...
	 * @return number
	 */
	function divino_header_break_point() {
		$mobile_header_brakpoint = true === divino_Builder_Helper::$is_header_footer_builder_active ? divino_get_tablet_breakpoint() : divino_get_option( 'mobile-header-breakpoint', 921 );
		return absint( apply_filters( 'divino_header_break_point', $mobile_header_brakpoint ) );
	}
}
 ...
 */
if ( ! function_exists( 'divino_edit_post_link' ) ) {
	 ...
	 * @return void
	 */
	function divino_edit_post_link( $text, $before = '', $after = '', $id = 0, $class = 'post-edit-link' ) {

		if ( apply_filters( 'divino_edit_post_link', false ) ) {
 ...
 */
if ( ! function_exists( 'divino_header_classes' ) ) {
	 ...
	 * @since 1.0.0
	 */
	function divino_header_classes() {
		echo 'class="' . esc_attr( join( ' ', divino_get_header_classes() ) ) . '"';
	}
}
 ...
 */
function divino_get_header_classes() {
		$classes                       = array( 'site-header' );
		$menu_logo_location            = divino_get_option( 'header-layouts' );
		$mobile_header_alignment       = divino_get_option( 'header-main-menu-align' );
		$primary_menu_disable          = divino_get_option( 'disable-primary-nav' );
		$primary_menu_custom_item      = divino_get_option( 'header-main-rt-section' );
		$logo_title_inline             = divino_get_option( 'logo-title-inline' );
		$mobile_header_logo            = divino_get_option( 'mobile-header-logo' );
		$different_mobile_header_order = divino_get_option( 'different-mobile-logo' );
		/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$hide_custom_menu_mobile     = divino_get_option( 'hide-custom-menu-mobile', false );
		$menu_mobile_target          = divino_get_option( 'mobile-header-toggle-target', 'icon' );
		$submenu_container_animation = divino_get_option( 'header-main-submenu-container-animation' );
		$builder_menu_mobile_target  = divino_get_option( 'header-builder-menu-toggle-target', 'icon' );
	...
	$classes = array_unique( apply_filters( 'divino_header_class', $classes ) );

	$classes = array_map( 'sanitize_html_class', $classes );

	return apply_filters( 'divino_get_header_classes', $classes );
}

/**
 * Function to get Footer Classes
 */
if ( ! function_exists( 'divino_footer_classes' ) ) {
	 ...
	 * @since 1.0.0
	 */
	function divino_footer_classes() {
		echo 'class="' . esc_attr( join( ' ', divino_get_footer_classes() ) ) . '"';
	}
}
 ...
 */
function divino_get_footer_classes() {
	$classes = array_unique( apply_filters( 'divino_footer_class', array( 'site-footer' ) ) );
	$classes = array_map( 'sanitize_html_class', $classes );

	return apply_filters( 'divino_get_footer_classes', $classes );
}

/**
 * Function to filter comment form's default fields
 */
if ( ! function_exists( 'divino_comment_form_default_fields_markup' ) ) {
	 ...
	 */
	function divino_comment_form_default_fields_markup( $fields ) {
		...
		$name_label    = divino_default_strings( 'string-comment-label-name', false );
		$email_label   = divino_default_strings( 'string-comment-label-email', false );
		$website_label = divino_default_strings( 'string-comment-label-website', false );
		...
		return apply_filters( 'divino_comment_form_default_fields_markup', $fields );
	}
}

add_filter( 'comment_form_default_fields', 'divino_comment_form_default_fields_markup' );

/**
 * Function to filter comment form arguments
 */
if ( ! function_exists( 'divino_comment_form_default_markup' ) ) {
	...
	function divino_comment_form_default_markup( $args ) {
		 ...
		 * @return bool
		 */
		$all_post_type_support = apply_filters( 'divino_comment_form_all_post_type_support', false );
		if ( 'post' === get_post_type() || $all_post_type_support ) {
			$args['id_form']           = 'ast-commentform';
			$args['title_reply']       = divino_default_strings( 'string-comment-title-reply', false );
			$args['cancel_reply_link'] = divino_default_strings( 'string-comment-cancel-reply-link', false );
			$args['label_submit']      = divino_default_strings( 'string-comment-label-submit', false );
			$args['comment_field']     = '<div class="ast-row comment-textarea"><fieldset class="comment-form-comment"><legend class ="comment-form-legend"></legend><div class="comment-form-textarea ' . divino_attr( 'ast-grid-lg-12' ) . '"><label for="comment" class="screen-reader-text">' . esc_html( divino_default_strings( 'string-comment-label-message', false ) ) . '</label><textarea id="comment" name="comment" placeholder="' . esc_attr( divino_default_strings( 'string-comment-label-message', false ) ) . '" cols="45" rows="8" aria-required="true"></textarea></div></fieldset></div>';
		}
		return apply_filters( 'divino_comment_form_default_markup', $args );
	}
}

add_filter( 'comment_form_defaults', 'divino_comment_form_default_markup' );

/**
 * Display Blog Post Excerpt
 */
if ( ! function_exists( 'divino_the_excerpt' ) ) {
	 ...
	 * @since 1.0.0
	 */
	function divino_the_excerpt() {

		$excerpt_type = apply_filters( 'divino_excerpt_type', divino_get_option( 'blog-post-content' ) );

		do_action( 'divino_the_excerpt_before', $excerpt_type );
		?>
		...
		do_action( 'divino_the_excerpt_after', $excerpt_type );
	}
}
 ...
 * Display Sidebars
 */
if ( ! function_exists( 'divino_get_sidebar' ) ) {
	/**
	 * Get Sidebar
	 *
	 * @since 1.0.1.1
	 ...
	 * @return void
	 */
	function divino_get_sidebar( $sidebar_id ) {
 ...
 * Get Footer widgets
 */
if ( ! function_exists( 'divino_get_footer_widget' ) ) {
	 ...
	 * @return void
	 */
	function divino_get_footer_widget( $sidebar_id ) {
			...
			<div class="widget ast-no-widget-row">
				...
				<?php if ( is_customize_preview() ) { ?>
					<div class="customizer-navigate-on-focus" data-section="widgets" data-type="panel">
						<?php divino_Builder_UI_Controller::render_customizer_edit_button(); ?>
				<?php } ?>
						<p class='no-widget-text'>
							<a href='<?php echo esc_url( admin_url( 'widgets.php' ) ); ?>'>
...
if ( ! function_exists( 'divino_entry_header_class' ) ) {
	 ...
	 * @since 1.0.15
	 */
	function divino_entry_header_class( $echo = true ) {

		$post_id          = divino_get_post_id();
		$post_type        = strval( get_post_type() );
		$classes          = array();
		$title_markup     = divino_the_title( '', '', $post_id, false );
		$thumb_markup     = divino_get_post_thumbnail( '', '', false );
		$post_meta_markup = divino_get_post_meta( divino_get_option( 'ast-dynamic-single-' . $post_type . '-metadata', array( 'comments', 'author', 'date' ) ) );
		$single_structure = 'page' === $post_type ? divino_get_option( 'ast-dynamic-single-page-structure', array( 'ast-dynamic-single-page-image', 'ast-dynamic-single-page-title' ) ) : divino_get_option( 'ast-dynamic-single-' . esc_attr( $post_type ) . '-structure', array( 'ast-dynamic-single-' . $post_type . '-title', 'ast-dynamic-single-' . $post_type . '-meta' ) );
		...
		if ( empty( $single_structure ) ) {
			...
			foreach ( $single_structure as $key ) {
				$structure_key = 'single-' . divino_get_last_meta_word( $key );
		...
		$classes = array_unique( apply_filters( 'divino_entry_header_class', $classes ) );
		$classes = array_map( 'sanitize_html_class', $classes );
...
if ( ! function_exists( 'divino_get_post_thumbnail' ) ) {
	 ...
	 * @return string|void
	 */
	function divino_get_post_thumbnail( $before = '', $after = '', $echo = true ) {
		...
		if ( $check_is_singular ) {
			$is_featured_image = divino_get_option_meta( 'ast-featured-img' );
		} else {
			$is_featured_image = divino_get_option( 'ast-featured-img' );
		}

		$featured_image_size = 'large';
		if ( $check_is_archive ) {
			$featured_image_size = divino_get_option( 'blog-image-size', 'large' );
		}
		...
		$featured_image = apply_filters( 'divino_featured_image_enabled', $featured_image );

		$blog_post_thumb   = divino_get_option( 'blog-post-structure' );
		$single_post_thumb = divino_get_option( 'ast-dynamic-single-' . $post_type . '-structure', array( 'ast-dynamic-' . $post_type . '-post-title', 'ast-dynamic-' . $post_type . '-post-meta' ) );
		...
		if ( ( ( ! $check_is_singular && in_array( 'image', $blog_post_thumb ) ) || ( $check_is_singular && in_array( 'ast-dynamic-single-' . $post_type . '-image', $single_post_thumb ) ) || is_page() ) && has_post_thumbnail() ) {
			...
			if ( $featured_image && ( ! $check_is_singular || ( ! post_password_required() && ! is_attachment() && has_post_thumbnail() ) ) ) {
				...
				$image_size = $check_is_singular ? divino_get_option( 'ast-dynamic-single-' . $post_type . '-article-featured-image-size', 'large' ) : $featured_image_size;
				$post_thumb = apply_filters(
					'divino_featured_image_markup',
					get_the_post_thumbnail(
						get_the_ID(),
						apply_filters( 'divino_post_thumbnail_default_size', $image_size ),
						apply_filters( 'divino_post_thumbnail_itemprop', '' )
					)
				);
				...
				if ( '' != $post_thumb ) {
					...
					if ( ! $check_is_singular ) {
						$output .= apply_filters(
							'divino_blog_post_featured_image_link_before',
							'<a ' . divino_attr(
								'article-image-url',
								array(
									'class'      => '',
					...
					$output .= $post_thumb;
					if ( ! $check_is_singular ) {
						$output .= apply_filters( 'divino_blog_post_featured_image_link_after', '</a>' );
					}
					$output .= '</div>';
				}
			}
		...
		if ( ! $check_is_singular ) {
			$output = apply_filters( 'divino_blog_post_featured_image_after', $output );
		}

		$output = apply_filters( 'divino_get_post_thumbnail', $output, $before, $after );
 ...
 */
if ( ! function_exists( 'divino_replace_header_attr' ) ) {
	 ...
	 */
	function divino_replace_header_attr( $attr, $attachment, $size ) {
		...
		if ( apply_filters( 'divino_is_logo_attachment', $is_logo_attachment, $attachment ) ) {
		...
		if ( apply_filters( 'divino_is_retina_logo_attachment', $is_logo_attachment, $attachment ) ) {

			$diff_retina_logo = divino_get_option( 'different-retina-logo' );

			if ( '1' == $diff_retina_logo ) {

				$retina_logo = divino_get_option( 'ast-header-retina-logo' );

				$attr['srcset'] = '';

				if ( apply_filters( 'divino_main_header_retina', true ) && '' !== $retina_logo ) {
					$cutom_logo     = wp_get_attachment_image_src( $custom_logo_id, 'full' );
					$cutom_logo_url = $cutom_logo[0];

					if ( divino_check_is_ie() ) {
		...
		return apply_filters( 'divino_replace_header_attr', $attr );
	}
}
...
if ( ! function_exists( 'divino_color_palette' ) ) {
	...
	function divino_color_palette() {
		...
		return apply_filters( 'divino_color_palettes', $color_palette );
	}
}

if ( ! function_exists( 'divino_get_theme_name' ) ) {
...
if ( ! function_exists( 'divino_get_addon_name' ) ) {
	...
	function divino_get_addon_name() {
		...
		return apply_filters( 'divino_addon_name', $pro_name );
	}
}
 ...
 */
function divino_post_navigation_template() {
...
add_filter( 'navigation_markup_template', 'divino_post_navigation_template' );
 ...
 * @since 3.9.0
 */
function divino_skip_elementor_onboarding( $network_wide ) {
	// Deleted transient & setting up onboaded flag true to skip steps.
	delete_transient( 'elementor_activation_redirect' );
	update_option( 'elementor_onboarded', true );
}

add_action( 'activate_elementor/elementor.php', 'divino_skip_elementor_onboarding' );
 ...
 * @since 4.1.0
 */
function divino_bbpress_issue( $value ) {
	/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
...
add_filter( 'divino_single_layout_one_banner_visibility', 'divino_bbpress_issue', 50 );
 ...
 * @return void
 */
function divino_render_header_svg_mask() {

	$transparent_header_logo_color = divino_get_option( 'transparent-header-logo-color' );
	$header_logo_color             = divino_get_option( 'header-logo-color' );

	if ( $header_logo_color && 'unset' !== $header_logo_color ) {
		/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		divino_render_svg_mask( 'ast-img-color-filter', 'header_logo_svg_color', $header_logo_color );
	}

	if ( $transparent_header_logo_color && 'unset' !== $transparent_header_logo_color ) {
		/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		divino_render_svg_mask( 'ast-img-color-filter-2', 'header_logo_svg_color', $transparent_header_logo_color );
	}
}

add_action( 'wp_footer', 'divino_render_header_svg_mask' );

/**
 * Render Featured Image for single post at 'divino_entry_before' hook before post <article>
 *
 * @since 4.4.0
 */
function divino_single_post_entry_featured_image() {
	$post_type           = strval( get_post_type() );
	$featured_image_size = divino_get_option( 'ast-dynamic-single-' . $post_type . '-article-featured-image-size', 'large' );

	if ( apply_filters( 'divino_post_featured_image_condition', has_post_thumbnail() ) ) {
		do_action( 'divino_article_featured_image_before' );

		$output     = '';
		$post_thumb = apply_filters(
			'divino_article_featured_image_markup',
			get_the_post_thumbnail(
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				absint( divino_get_post_id() ),
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				apply_filters( 'divino_post_featured_image_default_size', $featured_image_size ),
				apply_filters( 'divino_post_featured_image_itemprop', '' )
			)
		);

		if ( '' != $post_thumb ) {
			$featured_image_width = 'layout-1' === divino_get_option( 'ast-dynamic-single-' . $post_type . '-layout', 'layout-1' ) ? divino_get_option( 'ast-dynamic-single-' . $post_type . '-article-featured-image-width-type', 'wide' ) : '';
			$output              .= '<div class="ast-single-post-featured-section post-thumb ast-article-image-container--' . esc_attr( $featured_image_width ) . '">';
			$output              .= $post_thumb;
			$output              .= '</div>';
		}

		$output = apply_filters( 'divino_featured_post_thumbnail', $output );
		echo wp_kses_post( $output );

		do_action( 'divino_article_featured_image_after' );
	}
}

/**
 * Prepare rendering Featured Image for single post at 'divino_header_after' hook after header.
 * Required on single post only.
 *
 * @since 4.4.0
 */
function divino_setup_article_featured_image() {
	if ( ! is_singular() ) {
		return;
	}
	...
	if ( ( 'disabled' === divino_get_option_meta( 'ast-featured-img' ) && divino_Dynamic_CSS::divino_4_6_2_compatibility() ) || false === divino_get_option( 'ast-single-' . $post_type . '-title' ) ) {
	...
	$banner_title_layout = divino_get_option( 'ast-dynamic-single-' . $post_type . '-layout', 'layout-1' );
	$single_structure    = divino_get_option( 'ast-dynamic-single-' . $post_type . '-structure', divino_get_option( 'ast-dynamic-single-' . $post_type . '-structure', 'page' === $post_type ? array( 'ast-dynamic-single-' . $post_type . '-image', 'ast-dynamic-single-' . $post_type . '-title' ) : array( 'ast-dynamic-single-' . $post_type . '-title', 'ast-dynamic-single-' . $post_type . '-meta' ) ) );
	...
	if ( 'layout-1' === $banner_title_layout ) {
		$article_featured_image_position = divino_get_option( 'ast-dynamic-single-' . $post_type . '-article-featured-image-position-layout-1', 'behind' );
	} else {
		$article_featured_image_position = divino_get_option( 'ast-dynamic-single-' . $post_type . '-article-featured-image-position-layout-2', 'none' );
	}

	if ( 'none' !== $article_featured_image_position ) {
		add_action( 'divino_entry_before', 'divino_single_post_entry_featured_image' );
	}
}

add_action( 'divino_header_after', 'divino_setup_article_featured_image' );
 ...
 */
function divino_add_aria_expanded_submenu_items_attr( $output, $item, $depth, $args ) {
...
add_filter( 'walker_nav_menu_start_el', 'divino_add_aria_expanded_submenu_items_attr', 10, 4 );
