...
class divino_WebFont_Loader {
	...
	public function __construct( $url = '' ) {
		...
		add_action( 'divino_delete_fonts_folder', array( $this, 'divino_delete_fonts_folder' ) );
	}
	...
	public function get_url() {
		...
		$divino_font_url = file_exists( $this->get_local_stylesheet_path() ) ? $this->get_local_stylesheet_url() : $this->remote_url;

		// If the local file exists, return its URL, with a fallback to the remote URL.
		divino_update_option( 'divino_font_url', wp_json_encode( $divino_font_url ) );

		return $divino_font_url;
	}
	...
	public function get_styles() {
		...
		$convert_to_url = apply_filters( 'divino_convert_fonts_to_url', true ); 
		...
		foreach ( $files as $remote => $local ) {
			...
			$files[ $remote ] = apply_filters( 'divino_webfont_loader_file_url', $url, $remote, $local );
		}
	...
	public function preload_local_fonts() {
		...
		update_site_option( 'divino_local_font_files', $local_font );
	...
	public function set_font_format( $format = 'woff2' ) {
		$this->font_format = apply_filters( 'divino_local_google_fonts_format', $format );
	}
	...
	public function get_base_path() {
		if ( ! $this->base_path ) {
			$this->base_path = apply_filters( 'divino_local_fonts_base_path', $this->get_filesystem()->wp_content_dir() );
		}
	...
	public function get_base_url() {
		if ( ! $this->base_url ) {
			$this->base_url = apply_filters( 'divino_local_fonts_base_url', content_url() );
		}
	...
	public function get_current_blog_id() {
		...
		if ( ! $this->current_blog_id ) {
			$this->current_blog_id = apply_filters( 'divino_local_fonts_current_blog_id', get_current_blog_id() );
		}
	...
	public function schedule_cleanup() {
		if ( ! is_multisite() || ( is_multisite() && is_main_site() ) ) {
			if ( ! wp_next_scheduled( 'divino_delete_fonts_folder' ) && ! wp_installing() ) {
				wp_schedule_event( time(), self::CLEANUP_FREQUENCY, 'divino_delete_fonts_folder' );  // phpcs:ignore WPThemeReview.PluginTerritory.ForbiddenFunctions.cron_functionality_wp_schedule_event
			}
		}
	}
	 ...
	 * @return bool
	 */
	public function divino_delete_fonts_folder() {
		// Delete previously created supportive options.
		divino_delete_option( 'divino_font_url' );
		delete_site_option( 'divino_local_font_files' );
...
/**
 * Create instance of divino_WebFont_Loader class.
 *
 * @param string $font_url Google font URL to set data.
 * @return object
 * @since 3.6.0
 */
function divino_webfont_loader_instance( $font_url = '' ) {
	return new divino_WebFont_Loader( $font_url );
}
