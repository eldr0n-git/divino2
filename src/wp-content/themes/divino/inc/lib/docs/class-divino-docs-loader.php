...
class divino_Docs_Loader {
	...
	public function __construct( $url = '', $subfolder_name = 'bsf-docs' ) {
		...
		add_action( 'divino_delete_docs_folder', array( $this, 'divino_delete_docs_folder' ) );
	}
	...
	public function get_url() {
		...
		$divino_docs_url = file_exists( $this->get_local_docs_file_path() ) ? $this->get_local_docs_json_url() : $this->remote_url;

		return $divino_docs_url;
	}
	...
	public function get_local_docs_filename() {
		return apply_filters( 'divino_local_docs_file_name', 'docs' );
	}
	...
	public function get_base_path() {
		if ( ! $this->base_path ) {
			$this->base_path = apply_filters( 'divino_local_docs_base_path', $this->get_filesystem()->wp_content_dir() . 'uploads' );
		}
	...
	public function get_base_url() {
		if ( ! $this->base_url ) {
			$this->base_url = apply_filters( 'divino_local_docs_base_url', content_url() . '/uploads' );
		}
	...
	public function schedule_cleanup() {
		if ( ! wp_next_scheduled( 'divino_delete_docs_folder' ) && ! wp_installing() ) {
			wp_schedule_event( time(), self::CLEANUP_FREQUENCY, 'divino_delete_docs_folder' );  // phpcs:ignore WPThemeReview.PluginTerritory.ForbiddenFunctions.cron_functionality_wp_schedule_event
		}
	}
	 ...
	 * @return bool
	 */
	public function divino_delete_docs_folder() {
...
/**
 * Create instance of divino_Docs_Loader class.
 *
 ...
 * @since 4.6.0
 */
function divino_docs_loader_instance( $docs_rest_url = '', $subfolder_name = 'bsf-docs' ) {
	return new divino_Docs_Loader( $docs_rest_url, $subfolder_name );
}
