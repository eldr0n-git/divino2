...
if ( ! class_exists( 'divino_Notices' ) ) :
	...
	class divino_Notices {
		...
		public static function add_notice( $args = array() ) {
			...
			$notices   = get_option( 'allowed_divino_notices', array() );
			if ( ! in_array( $notice_id, $notices, true ) ) {
				$notices[] = $notice_id; // Add notice id to the array.
				update_option( 'allowed_divino_notices', $notices ); // Update the option.
			}
		}
		...
		public function dismiss_notice() {
			...
			if ( ! apply_filters( 'divino_notices_user_cap_check', current_user_can( $capability ) ) ) {
				return;
			}

			$allowed_notices = get_option( 'allowed_divino_notices', array() ); // Get allowed notices.
		...
		public function show_notices() {
			$defaults = array(
				...
				'display-with-other-notices' => true,    // Should the notice be displayed if other notices  are being displayed from divino_Notices.
				'is_dismissible'             => true,
		...
		public static function markup( $notice = array() ) {
			...
			do_action( "divino_notice_after_markup_{$notice['id']}" );

			do_action( 'divino_notice_after_markup' );
		}
