...
if ( ! class_exists( 'divino_Edd' ) ) {
	...
	class divino_Edd {
		...
		public function __construct() {
			...
			require_once divino_THEME_DIR . 'inc/compatibility/edd/edd-common-functions.php';// phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
			...
			add_filter( 'divino_dynamic_theme_css', array( $this, 'add_inline_styles' ) );
			...
			add_filter( 'divino_single_post_navigation', array( $this, 'edd_single_post_navigation' ) );

			// Header Cart Icon.
			add_action( 'divino_edd_header_cart_icons_before', array( $this, 'header_cart_icon_markup' ) );
			add_filter( 'divino_edd_cart_in_menu_class', array( $this, 'header_cart_icon_class' ), 99 );
		}
		...
		public function header_cart_icon_markup() {
			...
			$icon               = divino_get_option( 'edd-header-cart-icon' );
			...
			if ( 'default' === $icon || ! defined( 'divino_EXT_VER' ) || ( defined( 'divino_EXT_VER' ) && ! divino_Ext_Extension::is_active( 'edd' ) ) ) {
				...
				add_filter( 'divino_edd_default_header_cart_icon', '__return_false' );
		...
		public function header_cart_icon_class( $classes ) {
			...
			$header_cart_icon_style = divino_get_option( 'edd-header-cart-icon-style' );

			$classes[]                  = 'ast-edd-menu-cart-' . $header_cart_icon_style;
			$header_cart_icon_has_color = divino_get_option( 'edd-header-cart-icon-color' );
		...
		public function edd_single_post_navigation( $args ) {
			...
			$disable_single_product_navigation = divino_get_option( 'disable-edd-single-product-nav' );
		...
		public function edd_initialization() {
			$is_edd_archive_page        = divino_is_edd_archive_page();
			$is_edd_single_product_page = divino_is_edd_single_product_page();

			if ( $is_edd_archive_page ) {
				add_action( 'divino_template_parts_content', array( $this, 'edd_content_loop' ) );
				remove_action( 'divino_template_parts_content', array( divino_Loop::get_instance(), 'template_parts_default' ) );

				// Add edd wrapper.
				add_action( 'divino_template_parts_content_top', array( $this, 'divino_edd_templat_part_wrap_open' ), 25 );
				add_action( 'divino_template_parts_content_bottom', array( $this, 'divino_edd_templat_part_wrap_close' ), 5 );

				// Remove closing and ending div 'ast-row'.
				remove_action( 'divino_template_parts_content_top', array( divino_Loop::get_instance(), 'divino_templat_part_wrap_open' ), 25 );
				remove_action( 'divino_template_parts_content_bottom', array( divino_Loop::get_instance(), 'divino_templat_part_wrap_close' ), 5 );
			}
			if ( $is_edd_single_product_page ) {
				remove_action( 'divino_template_parts_content', array( divino_Loop::get_instance(), 'template_parts_post' ) );

				add_action( 'divino_template_parts_content', array( $this, 'edd_single_template' ) );
		 ...
		 * @return void
		 */
		public function divino_edd_templat_part_wrap_open() {
			?>
		 ...
		 */
		public function divino_edd_templat_part_wrap_close() {
			?>
		...
		public function edd_single_template() {
			...
			<div <?php post_class(); ?>>
				...
				<?php divino_entry_content_single(); ?>
		...
		public function edd_content_loop() {
			...
			<div <?php post_class(); ?>>
				 ...
				 */
				do_action( 'divino_edd_archive_product_content' );
				?>
			</div>
			<?php
		}
		...
		public function divino_header_cart( $output, $section, $section_type ) {
			...
			if ( 'edd' === $section_type && apply_filters( 'divino_edd_header_cart_icon', true ) ) {
		...
		public function edd_mini_cart_markup() {
			...
			$cart_menu_classes = apply_filters( 'divino_edd_cart_in_menu_class', array( 'ast-menu-cart-with-border' ) );
		...
		public function divino_get_edd_cart() {
			...
			<a class="ast-edd-cart-container" href="<?php echo esc_url( $edd_cart_link ); ?>" title="<?php echo esc_attr( $view_shopping_cart ); ?>">
						...
						<?php
						do_action( 'divino_edd_header_cart_icons_before' );

						if ( apply_filters( 'divino_edd_default_header_cart_icon', true ) ) {
							?>
							<div class="ast-edd-cart-menu-wrap">
								<span class="count">
									<?php
									if ( apply_filters( 'divino_edd_header_cart_total', true ) ) {
										$cart_items = count( edd_get_cart_contents() );
						...
						do_action( 'divino_edd_header_cart_icons_after' );
		...
		public function add_inline_scripts() {
			...
			$edd_css_output = divino_parse_css( $max_tablet_edd_css, divino_get_mobile_breakpoint( '', 1 ), divino_get_tablet_breakpoint() );
			...
			if ( ! divino_Builder_Helper::apply_flex_based_css() ) {
				...
				/* Parse CSS from array() -> max-width: (tablet-breakpoint) px & min-width: (mobile-breakpoint + 1) px */
				$edd_css_output .= divino_parse_css( $max_tablet_edd_lang_direction_css, divino_get_mobile_breakpoint( '', 1 ), divino_get_tablet_breakpoint() );
			}
			...
			$edd_css_output .= divino_parse_css( $mobile_edd_css, '', divino_get_mobile_breakpoint() );
		...
		public function add_inline_styles( $dynamic_css, $dynamic_css_filtered = '' ) {
			...
			$edd_archive_width     = divino_get_option( 'edd-archive-width' );
			$edd_archive_max_width = divino_get_option( 'edd-archive-max-width' );
			$css_output            = '';
			...
			if ( 'custom' === $edd_archive_width ) {
				...
				$site_width  = array(
					'.ast-edd-archive-page .site-content > .ast-container' => array(
						'max-width' => divino_get_css_value( $edd_archive_max_width, 'px' ),
					),
				);
			...
			$dynamic_css .= apply_filters( 'divino_theme_edd_dynamic_css', $css_output );
		...
		public function edd_products_item_class( $classes = '' ) {
			...
			if ( $is_edd_archive_page ) {
				$shop_grid = divino_get_option( 'edd-archive-grids' );
		...
		public function replace_store_sidebar( $sidebar ) {
			...
			$is_edd_page                = divino_is_edd_page();
			$is_edd_single_product_page = divino_is_edd_single_product_page();
		...
		public function store_sidebar_layout( $sidebar_layout ) {
			...
			if ( $is_edd_page ) {
				...
				// Global.
				$edd_sidebar = divino_get_option( 'site-sidebar-layout' );
				...
				// Customizer General.
				$edd_customizer_sidebar = divino_get_option( 'edd-general-sidebar-layout' );
				...
				if ( $is_edd_single_product_page ) {
					$edd_single_product_sidebar = divino_get_option( 'single-download-sidebar-layout' );
					...
					$edd_sidebar_layout = divino_get_option( 'archive-download-sidebar-layout' );
				} else {
					$edd_sidebar_layout = divino_get_option_meta( 'site-sidebar-layout', '', true );
				}
		...
		public function store_content_layout( $layout ) {
			...
			$is_edd_page         = divino_is_edd_page();
			$is_edd_single_page  = divino_is_edd_single_page();
			$is_edd_archive_page = divino_is_edd_archive_page();

			if ( $is_edd_page ) {

				// Global.
				$edd_layout = divino_toggle_layout( 'ast-site-content-layout', 'global', false );
				...
				// Customizer General.
				$edd_customizer_layout = divino_toggle_layout( 'edd-ast-content-layout', 'global', false );
				...
				if ( $is_edd_single_page ) {
					$edd_single_product_layout = divino_toggle_layout( 'single-download-ast-content-layout', 'single', false );
					...
					$edd_page_layout = divino_toggle_layout( 'archive-download-ast-content-layout', 'archive', false );
				} else {
					$edd_page_layout = divino_get_option_meta( 'site-content-layout', '', true );
					if ( isset( $edd_page_layout ) ) {
						$edd_page_layout = divino_toggle_layout( 'ast-site-content-layout', 'meta', false, $edd_page_layout );
					} else {
						$edd_page_layout = divino_toggle_layout( 'ast-site-content-layout', 'meta', false );
					}
				}
...
if ( apply_filters( 'divino_enable_edd_integration', true ) ) {
	divino_Edd::get_instance();
}
