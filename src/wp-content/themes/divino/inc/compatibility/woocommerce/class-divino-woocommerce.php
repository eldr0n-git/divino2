...
if ( ! class_exists( 'divino_Woocommerce' ) ) {
	...
	class divino_Woocommerce {
		...
		public function __construct() {
			...
			require_once divino_THEME_DIR . 'inc/compatibility/woocommerce/woocommerce-common-functions.php';// phpcs:ignore: WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
			...
			add_filter( 'divino_get_sidebar', array( $this, 'replace_store_sidebar' ) );
			// Store Sidebar Layout.
			add_filter( 'divino_page_layout', array( $this, 'store_sidebar_layout' ) );
			// Store Content Layout.
			add_filter( 'divino_get_content_layout', array( $this, 'store_content_layout' ) );
			...
			add_filter( 'divino_get_dynamic_header_content', array( $this, 'divino_header_cart' ), 10, 3 );

			// Add Cart option in dropdown.
			add_filter( 'divino_header_section_elements', array( $this, 'header_section_elements' ) );
			...
			add_filter( 'woocommerce_get_stock_html', 'divino_woo_product_in_stock', 10, 2 );

			add_filter( 'divino_schema_body', array( $this, 'remove_body_schema' ) );

			// Header Cart Icon.
			add_action( 'divino_woo_header_cart_icons_before', array( $this, 'header_cart_icon_markup' ) );

			add_action( 'divino_cart_in_menu_class', array( $this, 'header_cart_icon_class' ), 99 );

			// WooCommerce Store Notice.
			add_filter( 'woocommerce_demo_store', array( $this, 'divino_woocommerce_update_store_notice_atts' ) );

			add_filter( 'divino_dynamic_theme_css', array( $this, 'divino_woocommerce_store_dynamic_css' ) );

			// Single Product Free shipping.
			add_action( 'divino_woo_single_price_after', array( $this, 'woocommerce_shipping_text' ) );
			...
			add_action( 'admin_bar_menu', array( $this, 'divino_update_customize_admin_bar_link' ), 45 );
			...
			if ( ! defined( 'divino_EXT_VER' ) || ( defined( 'divino_EXT_VER' ) && ! divino_Ext_Extension::is_active( 'woocommerce' ) ) ) {
		...
		public static function load_theme_side_woocommerce_strcture() {
			return ! defined( 'divino_EXT_VER' ) || divino_addon_check_version( '3.9.2', '>=' );
		}
		...
		public function modern_add_to_cart() {
			...
			// Product link markup.
			$header_woo_cart = divino_get_option( 'woo-header-cart-icon', 'default' );
			$cart_icon       = true === divino_Icons::is_svg_icons() ? divino_Icons::get_icons( 'default' === $header_woo_cart ? 'bag' : $header_woo_cart ) : divino_Builder_UI_Controller::fetch_svg_icon( 'shopping-' . $header_woo_cart, false );
			$classes         = implode(
				' ',
				array_filter(
					array(
		...
		public function add_modern_triggers_on_image() {
			...
			if ( divino_is_shop_page_modern_style() ) {
				...
				$html = apply_filters( 'divino_addon_shop_cards_buttons_html', $markup, $product );
				/** @psalm-suppress TooManyArguments */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		...
		public function get_sale_flash_markup( $sale_notification, $product ) {
			...
			$classes = array();
			/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$classes[] = divino_is_shop_page_modern_style() ? 'ast-on-card-button ast-onsale-card' : 'onsale';
			/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$classes = implode( ' ', $classes );

			// Generate markup.
			return '<span  ' . divino_attr(
		...
		public function sale_flash( $markup, $post, $product ) {
			...
			/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			if ( ( ! is_singular( 'product' ) && divino_is_shop_page_modern_style() ) ) {
				/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				return '';
			}
		...
		public function change_cart_close_icon( $string ) {
			return str_replace( '&times;', divino_Builder_UI_Controller::fetch_svg_icon( 'close', false ), $string );
		}
		...
		public function store_widgets_dynamic() {
			...
			if ( divino_has_pro_woocommerce_addon() && divino_get_option( 'shop-filter-accordion' ) ) {
				$shop_filter_array['before_title']   = '<h2 class="widget-title">';
				$shop_filter_array['after_title']    = divino_Builder_UI_Controller::fetch_svg_icon( 'angle-down', false ) . '</h2>';
		 ...
		 */
		public function divino_woocommerce_update_store_notice_atts( $notice ) {

			$store_notice_position = divino_get_option( 'store-notice-position' );
		 ...
		 */
		public function divino_woocommerce_store_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {
			...
			$store_notice_color    = divino_get_option( 'store-notice-text-color' );
			$store_notice_bg_color = divino_get_option( 'store-notice-background-color' );
			/**
			...
			if ( ! $notice_hidden && 'hang-over-top' === divino_get_option( 'store-notice-position' ) ) {
			...
			if ( is_user_logged_in() ) {
				...
				$dynamic_css .= divino_parse_css( $admin_bar_desktop_css, '783' );
				...
				$dynamic_css .= divino_parse_css( $admin_bar_responsive_css, '', '782' );
			}
			return $dynamic_css;
		}
		...
		public function svg_cart_icon( $cart_total_label_position, $cart_label_markup, $cart_info_markup, $cart_icon ) {
			...
			add_filter( 'divino_woo_default_header_cart_icon', '__return_false' );
		...
		public function header_cart_icon_markup() {
			...
			if ( apply_filters( 'divino_woocommerce_cart_icon', $woo_cart_icon_new_user ) ) {
				...
				if ( false === divino_Builder_Helper::$is_header_footer_builder_active && ! defined( 'divino_EXT_VER' ) ) {
					return;
				}
			}
			...
			$cart_check_total = divino_get_option( 'woo-header-cart-total-label' ) && null !== WC()->cart ? intval( WC()->cart->get_cart_contents_total() ) > 0 : true;
			/** @psalm-suppress RedundantConditionGivenDocblockType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			...
			$cart_icon = sprintf(
				...
				$icon ? ( false !== divino_Icons::is_svg_icons() ? divino_Icons::get_icons( $icon ) : '' ) : ''
			);
		...
		public function header_cart_icon_class( $classes ) {
			...
			$header_cart_icon_style = divino_get_option( 'woo-header-cart-icon-style' );

			$classes[]                  = 'ast-menu-cart-' . $header_cart_icon_style;
			$header_cart_icon_has_color = divino_get_option( 'woo-header-cart-icon-color' );
		...
		public function cart_page_upselles() {
			...
			$upselles_enabled = divino_get_option( 'enable-cart-upsells' );
			if ( ! $upselles_enabled ) {
		...
		public function woo_filter_style( $styles ) {
			...
			$css_uri = divino_THEME_URI . 'assets/css/' . $dir_name . '/compatibility/woocommerce/';
			...
			if ( ! divino_Builder_Helper::apply_flex_based_css() ) {
				$styles = array(
					...
					'woocommerce-smallscreen' => array(
						...
						'version' => divino_THEME_VERSION,
						'media'   => 'only screen and (max-width: ' . apply_filters( 'woocommerce_style_smallscreen_breakpoint', divino_get_tablet_breakpoint() . 'px' ) . ')', // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
						'has_rtl' => true,
					),
		...
		public function theme_defaults( $defaults ) {
			...
			$defaults['single-product-plus-minus-button'] = divino_has_pro_woocommerce_addon() ? true : false;
		...
		public function shop_columns( $col ) {
			...
			$divino_shop_col = divino_get_option(
				'shop-grids',
				array(
					'desktop' => 4,
					'tablet'  => 3,
			...
			return $divino_shop_col['desktop'];
		}
		 ...
		 * @return bool
		 */
		public function divino_woo_is_subcategory( $category_id = null ) {
		...
		public function shop_no_of_products() {
			...
			if ( is_product_taxonomy() && 'subcategories' === $taxonomy_page_display ) {
				$products = divino_get_option( 'shop-no-of-products' );
				if ( $this->divino_woo_is_subcategory() ) {
					return $products;
				}
		...
		public function shop_page_products_item_class( $classes = '' ) {
			...
			$rt_section = divino_get_option( 'header-main-rt-section' );

			if ( 'woocommerce' === $rt_section ) {
				$classes[] = 'ast-woocommerce-cart-menu';
			}

			if ( is_store_notice_showing() && 'hang-over-top' === divino_get_option( 'store-notice-position' ) ) {
		...
		public function get_grid_column_count( $type = 'archive', $device = 'desktop', $default = 2 ) {
			...
			if ( 'archive' === $type ) {
				$products_grid = divino_get_option(
					'shop-grids',
					array(
						'desktop' => 4,
						'tablet'  => 3,
				...
				$products_grid = divino_get_option( 'single-product-related-upsell-grid' );
			}
		...
		public function store_sidebar_layout( $sidebar_layout ) {
			...
			if ( is_shop() || is_product_taxonomy() || is_checkout() || is_cart() || is_account_page() || is_product() ) {
				...
				$woo_sidebar                 = divino_get_option( 'woocommerce-sidebar-layout' );
				$divino_with_modern_ecommerce = divino_get_option( 'modern-ecommerce-setup', true );
				...
				if ( is_shop() || is_product_taxonomy() ) {
					$global_page_specific_layout = divino_get_option( 'archive-product-sidebar-layout', 'default' );
				}

				if ( is_product() ) {
					$single_product_fallback_sidebar = false === $divino_with_modern_ecommerce ? divino_get_option( 'site-sidebar-layout' ) : divino_get_option( 'woocommerce-sidebar-layout' );
					$single_product_sidebar          = divino_get_option( 'single-product-sidebar-layout', 'default' );
				...
				if ( is_shop() ) {
					...
					$shop_sidebar = divino_get_option_meta( 'site-sidebar-layout', '', true );
				}
			...
			return apply_filters( 'divino_get_store_sidebar_layout', $sidebar_layout );
		}
		/**
		 * WooCommerce Container
		 *
		...
		public function store_content_layout( $layout ) {
			...
			if ( is_woocommerce() || is_checkout() || is_cart() || is_account_page() ) {
				...
				$woo_layout = divino_toggle_layout( 'woocommerce-ast-content-layout', 'global', false );
				...
				if ( is_shop() || is_product_taxonomy() ) {
					$global_page_specific_layout = divino_toggle_layout( 'archive-product-ast-content-layout', 'archive', false );
				}

				if ( is_product() ) {
					$global_page_specific_layout = divino_toggle_layout( 'single-product-ast-content-layout', 'single', false );
				}
				...
				if ( is_shop() ) {
					$shop_page_id = get_option( 'woocommerce_shop_page_id' );
					$shop_layout  = divino_toggle_layout( 'ast-site-content-layout', 'meta', $shop_page_id );
				} elseif ( is_product_taxonomy() ) {
					$shop_layout = 'default';
				} else {
					$old_meta_layout = divino_get_option_meta( 'site-content-layout', '', true );
					if ( isset( $old_meta_layout ) ) {
						$shop_layout = divino_toggle_layout( 'ast-site-content-layout', 'meta', false, $old_meta_layout );
					} else {
						$shop_layout = divino_toggle_layout( 'ast-site-content-layout', 'meta', false );
					}
				}
			...
			return apply_filters( 'divino_get_store_content_layout', $layout );
		}
		...
		public function shop_meta_option() {
			...
			if ( is_shop() ) {
				...
				if ( 'disabled' === $footer_layout ) {
					remove_action( 'divino_footer_content', 'divino_footer_small_footer_template', 5 );
				}
			}
		}
		...
		public function shop_customization() {
			...
			if ( ! apply_filters( 'divino_woo_shop_product_structure_override', false ) ) {

				add_action( 'woocommerce_before_shop_loop_item', 'divino_woo_shop_thumbnail_wrap_start', 6 );
				/**
				 * Add sale flash before shop loop.
				 */
				add_action( 'woocommerce_before_shop_loop_item', 'woocommerce_show_product_loop_sale_flash', 9 );

				add_action( 'woocommerce_after_shop_loop_item', 'divino_woo_shop_thumbnail_wrap_end', 8 );
				/**
				 * Add Out of Stock to the Shop page
				 */
				add_action( 'woocommerce_shop_loop_item_title', 'divino_woo_shop_out_of_stock', 8 );
				 ...
				 */
				add_action( 'woocommerce_after_shop_loop_item', 'divino_woo_woocommerce_shop_product_content' );
			}
		}
		...
		public function single_product_customization() {
			...
			if ( divino_get_option( 'single-product-breadcrumb-disable' ) ) {
		...
		public function woocommerce_init() {
			add_action( 'woocommerce_after_mini_cart', array( $this, 'divino_update_flyout_cart_layout' ) );
		...
		public function before_main_content_start() {
			$site_sidebar = divino_page_layout();
		 ...
		 * @return bool
		 */
		public static function divino_update_default_font_styling() {
			$divino_settings = divino_get_options();
			return apply_filters( 'divino_default_font_style_update', isset( $divino_settings['ast-font-style-update'] ) ? false : true );
		}
		...
		public function add_scripts_styles() {
			...
			$if_free_shipping                        = divino_get_option( 'single-product-enable-shipping' );
			$single_product_heading_tab_active_color = divino_get_option( 'single-product-heading-tab-active-color' );
			...
			$font_style_updates                      = self::divino_update_default_font_styling();
			...
			$can_update_cart_color   = divino_cart_color_default_icon_old_header();
			$cart_new_color_setting  = divino_get_option( $icon_cart_color_slug, $theme_color );
			...
			$cart_h_color = divino_get_foreground_color( $link_h_color );

			$site_content_width         = divino_get_option( 'site-content-width', 1200 );
			$woo_shop_archive_width     = divino_get_option( 'shop-archive-width' );
			$woo_shop_archive_max_width = divino_get_option( 'shop-archive-max-width' );
			...
			$divino_support_woo_btns_global = divino_Dynamic_CSS::divino_woo_support_global_settings();

			if ( $divino_support_woo_btns_global ) {
				$woo_btn_compatibility_desktop = divino_get_font_array_css( divino_get_option( 'font-family-button' ), divino_get_option( 'font-weight-button' ), $theme_btn_font_size, 'font-extras-button' );
				$woo_btn_compatibility_tablet  = array(
			...
			if ( ! divino_wc_is_star_rating_compatibility() ) {
			...
			if ( divino_Dynamic_CSS::v4_block_editor_compat() ) {
			...
			if ( is_cart() && false === divino_Builder_Helper::apply_flex_based_css() && true === divino_get_option( 'cart-modern-layout' ) && true === divino_get_option( 'enable-cart-upsells' ) ) {
				$css_desktop_output['.woocommerce[class*="rel-up-columns-"] .site-main div.product .related.products ul.products li.product, .woocommerce-page .site-main ul.products li.product'] = array(
					'width' => '100%',
				);
			}
			...
			if ( apply_filters( 'divino_theme_woocommerce_global_h2_font', divino_get_option( 'woo_support_global_settings', false ) ) ) {
			...
			if ( is_account_page() && false === divino_get_option( 'modern-woo-account-view', false ) ) {
				$css_output .= '
			...
			if ( 'flyout' === divino_get_option( 'woo-header-cart-click-action' ) ) {
				$css_output .= '.woocommerce a.added_to_cart { display: none; }';
			}

			/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			if ( ! ( defined( 'divino_EXT_VER' ) && class_exists( 'divino_Ext_Extension' ) && divino_Ext_Extension::is_active( 'woocommerce' ) ) ) {
				/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$css_output .= '
			...
			if ( true === divino_check_is_structural_setup() ) {
			...
			$css_output .= divino_parse_css( $css_desktop_output );
			...
			$css_output .= divino_parse_css( $tablet_css_shop_page_grid, divino_get_mobile_breakpoint( '', 1 ), divino_get_tablet_breakpoint() );
			...
			if ( ! divino_Builder_Helper::apply_flex_based_css() ) {
				...
				$css_output .= divino_parse_css( $tablet_shop_page_grid_lang_direction_css, divino_get_mobile_breakpoint( '', 1 ), divino_get_tablet_breakpoint() );
			}
			...
			$css_output .= divino_parse_css( $min_tablet_css, divino_get_tablet_breakpoint( '', 1 ) );
			...
			$css_output .= divino_parse_css( $css_global_button_tablet, '', divino_get_tablet_breakpoint() );
			...
			$css_output .= divino_parse_css( $css_global_button_mobile, '', divino_get_mobile_breakpoint() );
			...
			$css_output .= divino_parse_css( $global_button_mobile_lang_direction_css, '', divino_get_mobile_breakpoint() );

			if ( 'page-builder' !== divino_get_content_layout() ) {
				...
				if ( 'custom' === $woo_shop_archive_width ) {
					...
					$site_width  = array(
						'.ast-woo-shop-archive .site-content > .ast-container' => array(
							'max-width' => divino_get_css_value( $woo_shop_archive_max_width, 'px' ),
						),
					);
					$css_output .= divino_parse_css( $site_width, divino_get_tablet_breakpoint( '', 1 ) );
			...
			/* Parse CSS from array()*/
			$css_output .= divino_parse_css( $woo_product_css, divino_get_tablet_breakpoint( '', 1 ) );
			...
			*/
			if ( self::divino_global_btn_woo_comp() ) {
				if ( ! $divino_support_woo_btns_global ) {
					...
					$css_output           .= divino_parse_css( $woo_global_button_css );
				}
				...
				if ( $if_free_shipping ) {
					...
					$css_output            .= divino_parse_css( $woo_free_shipping_text );
				}
			}

			if ( ! is_shop() && ! is_product() ) {

				$css_output .= divino_parse_css(
					array(
			...
			$css_output .= divino_parse_css( $woo_product_lang_direction_css, divino_get_tablet_breakpoint( '', 1 ) );
			/** @psalm-suppress InvalidScalarArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			/**
			 * Single page cart button size.
			 */
			$single_product_cart_button_width = divino_get_option( 'single-product-cart-button-width' );
			...
			$css_output_cart_button_width_desktop = array(
				...
				$single_cart_button => array(
					'width' => divino_get_css_value( $single_product_cart_button_width_desktop, '%' ),
				),
			);

			$css_output .= divino_parse_css( $css_output_cart_button_width_desktop );

			$css_output_cart_button_width_mobile = array(

				$single_cart_button => array(
					'width' => divino_get_css_value( $single_product_cart_button_width_mobile, '%' ),
				),
			);
			...
			$css_output_cart_button_width_tablet = array(
				...
				$single_cart_button => array(
					'width' => divino_get_css_value( $single_product_cart_button_width_tablet, '%' ),
				),
			);
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$css_output .= divino_parse_css( $css_output_cart_button_width_tablet, '', divino_get_tablet_breakpoint() );
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$css_output .= divino_parse_css( $css_output_cart_button_width_mobile, '', divino_get_mobile_breakpoint() );
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			...
			$css_output .= divino_parse_css( $css_output_woo_select_default );

			$is_sticky_add_to_cart_position_active = divino_get_option( 'single-product-sticky-add-to-cart' );
			...
			if ( is_product() && $is_sticky_add_to_cart_position_active ) {
				 ...
				 */
				$sticky_add_to_cart_position         = divino_get_option( 'single-product-sticky-add-to-cart-position' );
				$sticky_add_to_cart_text_color       = divino_get_option( 'single-product-sticky-add-to-cart-text-color' );
				$sticky_add_to_cart_bg_color         = divino_get_option( 'single-product-sticky-add-to-cart-bg-color' );
				$sticky_add_to_cart_btn_text_color   = divino_get_option( 'single-product-sticky-add-to-cart-btn-n-color' );
				$sticky_add_to_cart_btn_text_color_h = divino_get_option( 'single-product-sticky-add-to-cart-btn-h-color' );
				$sticky_add_to_cart_btn_bg_color     = divino_get_option( 'single-product-sticky-add-to-cart-btn-bg-n-color' );
				$sticky_add_to_cart_btn_bg_color_h   = divino_get_option( 'single-product-sticky-add-to-cart-btn-bg-h-color' );
				...
				if ( is_admin_bar_showing() ) {
					...
					$css_output .= divino_parse_css( $sticky_add_to_cart_admin_bar, '601' );
				}

				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$css_output .= divino_parse_css( $sticky_add_to_cart_responsive_tablet, '', divino_get_tablet_breakpoint() );
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort

				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$css_output .= divino_parse_css( $sticky_add_to_cart_responsive_mobile, '', divino_get_mobile_breakpoint() );
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort

				$css_output .= divino_parse_css( $sticky_add_to_cart_p );
				$css_output .= divino_parse_css( $sticky_add_to_cart );
			}

			$divino_add_to_cart_quantity_btn_enabled = divino_add_to_cart_quantity_btn_enabled();
			...
			if ( 'shop-page-modern-style' === divino_get_option( 'shop-style' ) ) {
				$modern_shop_page_css = '';

				if ( 'none' !== divino_get_option( 'product-sale-notification', 'default' ) ) {
					$modern_shop_page_css .= '
						.ast-onsale-card {
							position: absolute;
							top: 1.5em;
			...
			if ( self::load_theme_side_woocommerce_strcture() ) {
				$css_output .= $this->divino_shop_summary_box_alignment();
			}
			...
			$woo_variation_layout = divino_get_option( 'single-product-variation-tabs-layout' );
			...
			if ( 'horizontal' === $woo_variation_layout ) {
				...
				$css_output .= divino_parse_css( $css_output_woo_variation_layout );
			}
			...
			$css_output .= divino_parse_css( $woo_active_filter_css );

			// Single product payment.
			$single_product_payment_array = divino_get_option( 'single-product-structure' );
			...
			if (
				divino_load_woocommerce_login_form_password_icon()
				&& (
					is_account_page() ||
					(
						is_checkout() &&
				...
				$css_output                   .= divino_parse_css( $css_output_show_password_icon );
			}

			wp_add_inline_style( 'woocommerce-general', apply_filters( 'divino_theme_woocommerce_dynamic_css', $css_output ) );
			...
			/* Parse CSS from array() */
			$yith_wcwl_main_style = divino_parse_css( $yith_wcwl_main_style );
			...
			/* Parse CSS from array()*/
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$yith_wcwl_main_style .= divino_parse_css( $yith_wcwl_main_style_small, '', divino_get_tablet_breakpoint() );
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		 ...
		 * @return string
		 */
		public function divino_shop_summary_box_alignment() {
			$shop_product_alignment = divino_get_option( 'shop-product-align-responsive' );
			$desktop_alignment      = isset( $shop_product_alignment['desktop'] ) ? $shop_product_alignment['desktop'] : '';
			$tablet_alignment       = isset( $shop_product_alignment['tablet'] ) ? $shop_product_alignment['tablet'] : '';
			$mobile_alignment       = isset( $shop_product_alignment['mobile'] ) ? $shop_product_alignment['mobile'] : '';
			...
			$tablet_breakpoint = divino_get_tablet_breakpoint();
			$mobile_breakpoint = divino_get_mobile_breakpoint();
		 ...
		 * @since 1.0.0
		 */
		public function divino_header_cart( $output, $section, $section_type ) {

			if ( 'woocommerce' === $section_type && apply_filters( 'divino_woo_header_cart_icon', true ) ) {
		...
		public function woo_mini_cart_markup( $device = 'desktop' ) {
			...
			$cart_click_action   = divino_get_option( 'woo-header-cart-click-action', 'default' );
			$desktop_cart_flyout = 'flyout' === $cart_click_action ? 'ast-desktop-cart-flyout' : '';
			$cart_menu_classes   = apply_filters( 'divino_cart_in_menu_class', array( 'ast-menu-cart-with-border', $desktop_cart_flyout ) );
			ob_start();
			if ( is_customize_preview() && true === divino_Builder_Helper::$is_header_footer_builder_active ) {
		 ...
		 * @since  1.0.0
		 */
		public function divino_get_cart_link() {
			// Suppress TooFewArguments warning for this line
			/** @psalm-suppress TooFewArguments */
			$woo_cart_link = divino_woocommerce_get_cart_url();
			/** @psalm-suppress RedundantConditionGivenDocblockType */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			...
			<a href="<?php echo esc_url( $woo_cart_link ); ?>" class="cart-container ast-cart-desktop-position-<?php echo esc_attr( $cart_total_label_position['desktop'] ); ?> ast-cart-mobile-position-<?php echo esc_attr( $cart_total_label_position['mobile'] ); ?> ast-cart-tablet-position-<?php echo esc_attr( $cart_total_label_position['tablet'] ); ?>" aria-label="<?php echo esc_attr( $aria_label ); ?>">
						...
						<?php
						do_action( 'divino_woo_header_cart_icons_before' );

						if ( apply_filters( 'divino_woo_default_header_cart_icon', true ) ) {
							?>
							<div class="ast-cart-menu-wrap">
								<span class="count">
								<span class="ast-count-text">
									<?php
									if ( apply_filters( 'divino_woo_header_cart_total', true ) && null != WC()->cart ) {
						...
						do_action( 'divino_woo_header_cart_icons_after' );
		...
		public function cart_link_fragment( $fragments ) {
			...
			add_filter( 'woocommerce_get_cart_url', 'divino_woocommerce_get_cart_url' );

			ob_start();
			$this->divino_get_cart_link();
			...
			remove_filter( 'woocommerce_get_cart_url', 'divino_woocommerce_get_cart_url' );
		 ...
		 * @since 3.9.0
		 */
		public function divino_update_flyout_cart_layout() {
			if ( WC()->cart->is_empty() ) {
				do_action( 'divino_empty_cart_before' );
				?>
				...
				<?php
				do_action( 'divino_empty_cart_after' );
			}
		}
		...
		public function woocommerce_proceed_to_checkout_button_html() {
			$cart_button_text = divino_get_option( 'woo-cart-button-text' );
		 ...
		 */
		public function divino_update_customize_admin_bar_link( $wp_admin_bar ) {
		 ...
		 */
		public static function divino_global_btn_woo_comp() {
			$divino_settings = divino_get_options();
			return apply_filters( 'divino_global_btn_woo_comp', isset( $divino_settings['global-btn-woo-css'] ) ? false : true );
		}
		 ...
		 */
		public function divino_woo_woocommerce_template_product_title( $product_type ) {
		...
		public function woocommerce_product_single_payments() {
			...
			$payment_list     = divino_get_option( 'single-product-payment-list' );
						...
						if ( isset( $single['source'] ) && $single['source'] ) {
							...
							<li class="ast-custom-payment">
								...
								if ( isset( $single['icon'] ) && $single['icon'] ) {
									...
									?>
								<li class="ast-custom-payment">
										<?php echo do_shortcode( divino_Builder_UI_Controller::fetch_svg_icon( $selected_icon, false ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
							</li>
									<?php
								}
							}
		...
		public function single_product_content_structure( $product_type = '' ) {
			...
			/** @psalm-suppress TooManyArguments */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$single_structure = apply_filters( 'divino_woo_single_product_structure', divino_get_option( 'single-product-structure' ), $product_type );
			...
			if ( is_array( $single_structure ) && ! empty( $single_structure ) ) {
				 ...
				 */
				$divino_addons_condition = divino_has_pro_woocommerce_addon();
				...
				foreach ( $single_structure as $value ) {
					...
					switch ( $value ) {
						case 'title':
							 ...
							 */
							do_action( 'divino_woo_single_title_before' );
							$this->divino_woo_woocommerce_template_product_title( $product_type );
							do_action( 'divino_woo_single_title_after' );
							break;
						case 'price':
							/**
							 * Add Product Price on single product page for all products.
							 */
							do_action( 'divino_woo_single_price_before' );
							woocommerce_template_single_price();
							do_action( 'divino_woo_single_price_after' );
							break;
						case 'ratings':
							/**
							 * Add rating on single product page for all products.
							 */
							do_action( 'divino_woo_single_rating_before' );
							woocommerce_template_single_rating();
							do_action( 'divino_woo_single_rating_after' );
							break;
						case 'short_desc':
							do_action( 'divino_woo_single_short_description_before' );
							woocommerce_template_single_excerpt();
							do_action( 'divino_woo_single_short_description_after' );
							break;
						case 'add_cart':
							do_action( 'divino_woo_single_add_to_cart_before' );
							woocommerce_template_single_add_to_cart();
							do_action( 'divino_woo_single_add_to_cart_after' );
							break;
						case 'summary-extras':
							/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
							if ( $divino_addons_condition && is_callable( array( divino_Ext_WooCommerce_Markup::get_instance(), 'single_product_extras' ) ) ) {
								/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
								do_action( 'divino_woo_single_extras_before' );
								divino_Ext_WooCommerce_Markup::get_instance()->single_product_extras();
								do_action( 'divino_woo_single_extras_after' );
							}
							break;
						case 'single-product-payments':
								do_action( 'divino_woo_single_product_payments_before' );
								$this->woocommerce_product_single_payments();
								do_action( 'divino_woo_single_product_payments_after' );

							break;
						case 'meta':
							do_action( 'divino_woo_single_category_before' );
							woocommerce_template_single_meta();
							do_action( 'divino_woo_single_category_after' );
							break;
						case 'category':
							do_action( 'divino_woo_single_product_category_before' );
							$this->single_product_category();
							do_action( 'divino_woo_single_product_category_after' );
							break;
						default:
							break;
					}
				...
				if ( $divino_addons_condition && divino_get_option( 'accordion-inside-woo-summary' ) && 'accordion' === divino_get_option( 'single-product-tabs-layout' ) && divino_get_option( 'single-product-tabs-display' ) ) {
					/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
					divino_Ext_WooCommerce_Markup::get_instance()->woo_product_tabs_layout_output();
				}
			}
		}
		...
		public function single_product_sticky_add_to_cart() {
			...
			if ( is_product() && divino_get_option( 'single-product-sticky-add-to-cart' ) ) {
				...
				$sticky_position  = divino_get_option( 'single-product-sticky-add-to-cart-position' );
				$add_to_cart_ajax = divino_get_option( 'single-product-ajax-add-to-cart' );
				// @codingStandardsIgnoreStart
				/**
...
if ( apply_filters( 'divino_enable_woocommerce_integration', true ) ) {
	divino_Woocommerce::get_instance();
}
