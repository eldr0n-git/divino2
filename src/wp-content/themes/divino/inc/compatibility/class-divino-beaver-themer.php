...
if ( ! class_exists( 'divino_Beaver_Themer' ) ) {
	...
	class divino_Beaver_Themer {
		...
		public function __construct() {
			...
			add_filter( 'divino_dynamic_theme_css', array( $this, 'beaver_themer_compatibility_styles' ) );
		}
		...
		public function beaver_themer_compatibility_styles( $dynamic_css ) {
			...
			if ( class_exists( 'FLBuilderModel' ) ) {
				...
				$parse_css = divino_parse_css( $beaver_themer_styles );
		...
		public function render_post_class( $classes ) {
			...
			if ( count( $result ) > 0 ) {
				...
				add_filter( 'divino_post_link_enabled', '__return_false' );
			}
		...
		public function theme_header_footer_render() {
			...
			if ( ! empty( $header_ids ) ) {
				...
				add_action( 'divino_header', 'FLThemeBuilderLayoutRenderer::render_header' );
			}
			...
			if ( ! empty( $footer_ids ) ) {
				...
				add_action( 'divino_footer', 'FLThemeBuilderLayoutRenderer::render_footer' );
			}
			...
			if ( ! empty( $template_ids ) ) {
				...
				if ( 'archive' === $template_type || 'singular' === $template_type || '404' === $template_type ) {
					...
					if ( 'disabled' === $footer_widgets ) {
						add_filter( 'divino_advanced_footer_disable', '__return_true' );
					}
				}
			}
		}
		...
		public function builder_before_render_content( $post_id ) {
			...
			?>
			<?php if ( 'left-sidebar' === divino_page_layout() ) { ?>
		...
		public function builder_after_render_content( $post_id ) {
			...
			<?php if ( 'right-sidebar' === divino_page_layout() ) { ?>
 ...
 */
divino_Beaver_Themer::get_instance();
