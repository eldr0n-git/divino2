...
class divino_SureCart {
	...
	public function __construct() {
		...
		add_action( 'divino_header_after', array( $this, 'divino_surecart_archive_page_banner_support' ) );
		add_action( 'divino_entry_top', array( $this, 'revert_surecart_support' ) );
		add_filter( 'divino_page_layout', array( $this, 'sc_shop_sidebar_layout' ) );
		add_filter( 'divino_get_content_layout', array( $this, 'sc_shop_content_layout' ) );
		add_action( 'wp', array( $this, 'remove_navigation_for_sc_product' ) );

		// Boxed layout support.
		add_filter( 'divino_is_content_layout_boxed', array( $this, 'sc_shop_content_boxed_layout' ) );
		add_filter( 'divino_is_sidebar_layout_boxed', array( $this, 'sc_shop_sidebar_boxed_layout' ) );
		add_action( 'customize_register', array( $this, 'customize_register' ), 2 );
		add_filter( 'divino_theme_defaults', array( $this, 'divino_surecart_default_options' ) );
		add_filter( 'divino_archive_post_title', array( $this, 'customize_surecart_archive_title_area' ), 10, 2 );
		add_filter( 'divino_single_post_title', array( $this, 'customize_surecart_single_title_area' ), 10, 2 );
	 ...
	 * @since 4.4.0
	 */
	public function divino_is_surecart_shop_page() {
		...
		$page_id = absint( divino_get_post_id() );
	...
	public function sc_shop_sidebar_layout( $sidebar_layout ) {
		if ( $this->divino_is_surecart_shop_page() ) {
			$sc_shop_sidebar = divino_get_option( 'archive-' . $this->post_type . '-sidebar-layout', 'default' );
		...
		return apply_filters( 'divino_get_surecart_shop_sidebar_layout', $sidebar_layout );
	}
	...
	public function sc_shop_content_layout( $content_layout ) {
		if ( $this->divino_is_surecart_shop_page() ) {
			$sc_shop_layout = divino_toggle_layout( 'archive-' . $this->post_type . '-ast-content-layout', 'single', false );
		...
		return apply_filters( 'divino_get_store_content_layout', $content_layout );
	}
	...
	public function sc_shop_content_boxed_layout( $is_style_boxed ) {
		if ( $this->divino_is_surecart_shop_page() ) {
			$sc_shop_layout_style = divino_get_option( 'archive-' . $this->post_type . '-content-style', 'default' );
		...
		return apply_filters( 'divino_get_store_layout_style', $is_style_boxed );
	}
	...
	public function sc_shop_sidebar_boxed_layout( $is_style_boxed ) {
		if ( $this->divino_is_surecart_shop_page() ) {
			$sc_shop_layout_style = divino_get_option( 'archive-' . $this->post_type . '-sidebar-style', 'default' );
		...
		return apply_filters( 'divino_get_store_sidebar_style', $is_style_boxed );
	}
	 ...
	 * @since 4.4.0
	 */
	public function divino_surecart_archive_page_banner_support() {
		if ( false === $this->divino_is_surecart_shop_page() ) {
			return;
		}
		...
		$banner_layout = divino_get_option( 'ast-dynamic-archive-sc_product-layout', 'layout-1' );
		add_filter( 'divino_banner_elements_structure', array( $this, 'update_divino_banner_elements_structure' ) );
		add_filter( 'divino_banner_elements_post_type', array( $this, 'update_divino_banner_elements_post_type' ) );
		add_filter( 'divino_banner_elements_prefix', array( $this, 'update_divino_banner_elements_prefix' ) );
		add_filter( 'the_title', array( $this, 'update_the_title' ), 10, 2 );

		$title_area_enabled = divino_get_option( 'ast-archive-sc_product-title' );
		if ( $title_area_enabled ) {
			if ( 'layout-2' === $banner_layout ) {
				$divino_banner_hook = apply_filters( 'divino_banner_hook', 'divino_content_before' );
				add_action( $divino_banner_hook, array( $this, 'divino_surecart_hero_section' ), 20 );
			} else {
				add_filter( 'divino_single_layout_one_banner_visibility', '__return_false' );
				add_filter( 'divino_apply_hero_header_banner', '__return_false' );
	...
	public function divino_force_render_banner_layout_1() {
		add_filter( 'divino_remove_entry_header_content', '__return_false' );
		?>
	 ...
	 * @since 4.4.0
	 */
	public function divino_surecart_hero_section() {
		if ( false === apply_filters( 'divino_apply_hero_header_banner', true ) ) {
			return;
		}

		$args = array( 'post_type' => $this->post_type );
		do_action( 'divino_before_archive_' . $this->post_type . '_banner_content' );
		get_template_part( 'template-parts/archive', 'banner', $args );
		do_action( 'divino_after_archive_' . $this->post_type . '_banner_content' );
	 ...
	 * @since 4.4.0
	 */
	public function update_divino_banner_elements_structure( $structure ) {
		return divino_get_option( 'ast-dynamic-archive-' . $this->post_type . '-structure', array( 'ast-dynamic-archive-' . $this->post_type . '-title', 'ast-dynamic-archive-' . $this->post_type . '-description' ) );
	}
	 ...
	 * @since 4.4.0
	 */
	public function update_divino_banner_elements_post_type( $post_type ) {
	 ...
	 * @since 4.4.0
	 */
	public function update_divino_banner_elements_prefix( $prefix ) {
		return 'archive';
	}
	...
	public function update_the_title( $title, $post_id ) {
		...
		$custom_title = divino_get_option( 'ast-dynamic-archive-' . $this->post_type . '-custom-title', '' );
	...
	public function revert_surecart_support() {
		...
		remove_filter( 'divino_is_content_layout_boxed', array( $this, 'sc_shop_content_boxed_layout' ) );
		remove_filter( 'divino_is_sidebar_layout_boxed', array( $this, 'sc_shop_sidebar_boxed_layout' ) );

		remove_filter( 'divino_banner_elements_structure', array( $this, 'update_divino_banner_elements_structure' ) );
		remove_filter( 'divino_banner_elements_post_type', array( $this, 'update_divino_banner_elements_post_type' ) );
		remove_filter( 'divino_banner_elements_prefix', array( $this, 'update_divino_banner_elements_prefix' ) );
	 ...
	 * @since 4.7.3
	 */
	public function divino_surecart_default_options( $defaults ) {
		...
		$surecart_banner_padding = divino_Posts_Structure_Loader::get_customizer_default( 'responsive-padding' );
