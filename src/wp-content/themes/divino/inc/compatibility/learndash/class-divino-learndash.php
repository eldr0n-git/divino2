...
if ( ! class_exists( 'divino_LearnDash' ) ) {
	...
	class divino_LearnDash {
		...
		public function __construct() {
			...
			add_filter( 'divino_dynamic_theme_css', array( $this, 'add_dynamic_styles' ) );
		...
		public function add_dynamic_styles( $dynamic_css, $dynamic_css_filtered = '' ) {
			...
			$link_forground_color  = divino_get_foreground_color( $link_color );
			$theme_forground_color = divino_get_foreground_color( $theme_color );
			...
			$css_output = array(
				...
				'.learndash .notcompleted:before, #learndash_profile .notcompleted:before, .learndash_topic_dots ul .topic-notcompleted span:before, .learndash_navigation_lesson_topics_list .topic-notcompleted span:before, .learndash_navigation_lesson_topics_list ul .topic-notcompleted span:before, .learndash .topic-notcompleted span:before' => array(
					'color' => divino_hex_to_rgba( $text_color, .5 ),
				),
				...
				'body .thumbnail.course .ld_course_grid_price:before' => array(
					'border-top-color'   => divino_hex_to_rgba( $theme_color, .75 ),
					'border-right-color' => divino_hex_to_rgba( $theme_color, .75 ),
				),
				'body .wpProQuiz_loadQuiz, body .wpProQuiz_lock' => array(
					'border-color'     => divino_hex_to_rgba( $link_color, .5 ),
					'background-color' => divino_hex_to_rgba( $link_color, .1 ),
				),
			...
			if ( ! divino_get_option( 'learndash-lesson-serial-number' ) ) {
			...
			if ( ! divino_get_option( 'learndash-differentiate-rows' ) ) {
			...
			$tablet_typography = array(
				...
				'#ld_course_list .entry-title' => array(
					'font-size' => divino_responsive_font( $archive_post_title_font_size, 'tablet', 30 ),
				),
			);
			/* Parse CSS from array()*/
			$css_output .= divino_parse_css( $tablet_typography, '', divino_get_tablet_breakpoint() );
			...
			$css_output .= divino_parse_css( $mobile_min_width_css, divino_get_mobile_breakpoint( '', 1 ) );
			...
			$mobile_typography = array(
				...
				'#ld_course_list .entry-title'          => array(
					'font-size' => divino_responsive_font( $archive_post_title_font_size, 'mobile', 30 ),
				),
			...
			$css_output .= divino_parse_css( $mobile_typography, '', divino_get_mobile_breakpoint() );
			...
			/* Parse CSS from array() -> max-width: (mobile-breakpoint) px */
			$css_output .= divino_parse_css( $mobile_typography_lang_direction_css, '', divino_get_mobile_breakpoint() );

			$dynamic_css .= apply_filters( 'divino_theme_learndash_dynamic_css', $css_output );
		...
		public function sidebar_layout( $layout ) {
			...
			if ( is_singular( 'sfwd-courses' ) || is_singular( 'sfwd-lessons' ) || is_singular( 'sfwd-topic' ) || is_singular( 'sfwd-quiz' ) || is_singular( 'sfwd-certificates' ) || is_singular( 'sfwd-assignment' ) ) {
				...
				$learndash_sidebar = divino_get_option( 'learndash-sidebar-layout' );
				...
				$sidebar = divino_get_option_meta( 'site-sidebar-layout', '', true );
			...
			if ( $learndash_shortcode_used && ! ( is_singular( 'sfwd-courses' ) || is_singular( 'sfwd-lessons' ) || is_singular( 'sfwd-topic' ) || is_singular( 'sfwd-quiz' ) || is_singular( 'sfwd-certificates' ) || is_singular( 'sfwd-assignment' ) ) ) {
				...
				if ( ! did_action( 'divino_header' ) ) {
					// Page Meta Sidebar.
					$layout = divino_get_option_meta( 'site-sidebar-layout', '', true );
					if ( empty( $layout ) ) {
						// Page Sidebar.
						$layout = divino_get_option( 'single-page-sidebar-layout' );
						// Default Site Sidebar.
						if ( 'default' === $layout || empty( $layout ) ) {
							// Get the global sidebar value.
							// NOTE: Here not used `true` in the below function call.
							$layout = divino_get_option( 'site-sidebar-layout' );
						}
					}
				}
			}
		...
		public function content_layout( $layout ) {
			...
			if ( is_singular( 'sfwd-courses' ) || is_singular( 'sfwd-lessons' ) || is_singular( 'sfwd-topic' ) || is_singular( 'sfwd-quiz' ) || is_singular( 'sfwd-certificates' ) || is_singular( 'sfwd-assignment' ) ) {
				...
				$learndash_layout = divino_toggle_layout( 'learndash-ast-content-layout', 'global', false );
				...
				$learndash_layout = divino_get_option_meta( 'site-content-layout', '', true );
				if ( isset( $learndash_layout ) ) {
					$learndash_layout = divino_toggle_layout( 'ast-site-content-layout', 'meta', false, $learndash_layout );
				} else {
					$learndash_layout = divino_toggle_layout( 'ast-site-content-layout', 'meta', false );
				}
		...
		public static function ld_static_css() {
			...
			return divino_Enqueue_Scripts::trim_css( $ld_static_css );
		}
	}

}

if ( apply_filters( 'divino_enable_learndash_integration', true ) ) {

	/**
	 * Kicking this off by calling 'get_instance()' method
	 */
	divino_LearnDash::get_instance();
}
