...
if ( ! class_exists( 'divino_AMP' ) ) {
	...
	class divino_AMP {
		...
		public function __construct() {
			add_action( 'wp', array( $this, 'divino_amp_init' ) );
		}
		 ...
		 * @return void
		 */
		public function divino_amp_init() {
			...
			add_filter( 'divino_nav_toggle_data_attrs', array( $this, 'add_nav_toggle_attrs' ) );
			add_filter( 'divino_search_slide_toggle_data_attrs', array( $this, 'add_search_slide_toggle_attrs' ) );
			add_filter( 'divino_search_field_toggle_data_attrs', array( $this, 'add_search_field_toggle_attrs' ) );
			add_action( 'wp_footer', array( $this, 'render_amp_states' ) );
			add_filter( 'divino_attr_ast-main-header-bar-alignment', array( $this, 'nav_menu_wrapper' ) );
			add_filter( 'divino_attr_ast-menu-toggle', array( $this, 'menu_toggle_button' ), 20, 3 );
			add_filter( 'divino_theme_dynamic_css', array( $this, 'dynamic_css' ) );
			add_filter( 'divino_toggle_button_markup', array( $this, 'toggle_button_markup' ), 20, 2 );
			add_filter( 'divino_schema_body', array( $this, 'body_id' ) );
			 ...
			 * @since 4.0.0
			 */
			if ( true === divino_get_option( 'scroll-to-top-enable' ) ) {
				remove_action( 'wp_footer', array( divino_Scroll_To_Top_Loader::get_instance(), 'html_markup_loader' ) );
				remove_filter( 'divino_dynamic_theme_css', 'divino_scroll_to_top_dynamic_css' );
			}
		}
		...
		public function dynamic_css( $compiled_css ) {
			...
			$parse_css = $compiled_css . divino_parse_css( $css, '', divino_header_break_point() );
			...
			$parse_css .= divino_parse_css( $css, '', divino_header_break_point() );

			if ( true === divino_Builder_Helper::$is_header_footer_builder_active ) {

				$divino_break_point_navigation = array(
				...
				// Tablet CSS.
				$divino_medium_break_point_navigation = array(
					'.ast-amp .footer-sml-layout-2 .ast-small-footer-section-2' => array(
						'margin-top' => '1em',
					),
				);

				$parse_css .= divino_parse_css( $divino_medium_break_point_navigation, divino_get_tablet_breakpoint() );

				// Mobile CSS.
				$divino_small_break_point_navigation = array(
				...
				$parse_css .= divino_parse_css( $divino_small_break_point_navigation, '', divino_get_mobile_breakpoint() );
			}

			$parse_css .= divino_parse_css( $divino_break_point_navigation, '', divino_header_break_point() );
			...
			if ( false === divino_Icons::is_svg_icons() ) {
				...
				$divino_break_point_navigation['[data-section="section-header-mobile-trigger"] .ast-button-wrap .ast-mobile-menu-buttons-minimal'] = array(
					'background' => 'transparent',
					'border'     => 'none',
				);
			}

			return $parse_css . divino_parse_css( $divino_break_point_navigation, '', divino_header_break_point() );
		}
