...
if ( ! class_exists( 'divino_Elementor_Pro' ) ) {
	...
	class divino_Elementor_Pro {
		...
		public function __construct() {
			...
			add_action( 'divino_header', array( $this, 'do_header' ), 0 );
			add_action( 'divino_footer', array( $this, 'do_footer' ), 0 );
			add_action( 'divino_template_parts_content_top', array( $this, 'do_template_parts' ), 0 );

			add_action( 'divino_entry_content_404_page', array( $this, 'do_template_part_404' ), 0 );
			 ...
			 * @since  3.7.5
			 */
			add_filter( 'divino_theme_woocommerce_dynamic_css', array( $this, 'elementor_wc_widgets_compatibility_styles' ) );

			if ( $this->is_elementor_editor() && class_exists( 'WooCommerce' ) && divino_check_elementor_pro_3_5_version() ) {
				add_action( 'init', array( $this, 'update_woocommerce_checkout' ) );
			}

			add_filter( 'divino_shop_add_to_cart_js_localize', array( $this, 'divino_shop_add_to_cart_js_localize' ), 10, 1 );
		}
		 ...
		 * @return Array
		 */
		public function divino_shop_add_to_cart_js_localize( $localize_data ) {
		...
		public function elementor_wc_widgets_compatibility_styles( $css_output ) {
			...
			if ( ! divino_check_elementor_pro_3_5_version() ) {
				return $css_output;
			}
			...
			$css_output .= divino_parse_css( $woo_widgets_desktop_css );
		...
		public function do_template_parts() {
			...
			if ( $did_location ) {
				// Search and default.
				remove_action( 'divino_template_parts_content', array( \divino_Loop::get_instance(), 'template_parts_search' ) );// phpcs:ignore PHPCompatibility.LanguageConstructs.NewLanguageConstructs.t_ns_separatorFound
				remove_action( 'divino_template_parts_content', array( \divino_Loop::get_instance(), 'template_parts_default' ) );// phpcs:ignore PHPCompatibility.LanguageConstructs.NewLanguageConstructs.t_ns_separatorFound

				// Remove pagination.
				remove_action( 'divino_pagination', 'divino_number_pagination' );
				remove_action( 'divino_entry_after', 'divino_single_post_navigation_markup' );

				// Content.
				remove_action( 'divino_entry_content_single', 'divino_entry_content_single_template' );
			}
			...
			if ( $did_location ) {
				...
				remove_action( 'divino_page_template_parts_content', array( \divino_Loop::get_instance(), 'template_parts_page' ) );
				remove_action( 'divino_template_parts_content', array( \divino_Loop::get_instance(), 'template_parts_post' ) );
				remove_action( 'divino_template_parts_content', array( \divino_Loop::get_instance(), 'template_parts_comments' ), 15 );
				remove_action( 'divino_page_template_parts_content', array( \divino_Loop::get_instance(), 'template_parts_comments' ), 15 );
		...
		public function do_template_part_404() {
			if ( is_404() ) {
				...
				$did_location = Module::instance()->get_locations_manager()->do_location( 'single' );
				if ( $did_location ) {
					remove_action( 'divino_entry_content_404_page', 'divino_entry_content_404_page_template' );
				}
			}
		}
		...
		public function override_with_post_meta( $post_id = 0 ) {
			...
			if ( 'disabled' === $title ) {
				...
				// Archive page.
				add_filter( 'divino_the_title_enabled', '__return_false', 99 );
			...
			if ( 'disabled' === $footer_widgets ) {
				add_filter(
					'divino_advanced_footer_disable',
					static function() {
						return true;
					}
				);
		...
		public function do_header() {
			...
			if ( $did_location ) {
				...
				if ( true === \divino_Builder_Helper::$is_header_footer_builder_active ) { // phpcs:ignore PHPCompatibility.Keywords.NewKeywords.t_namespaceFound, PHPCompatibility.LanguageConstructs.NewLanguageConstructs.t_ns_separatorFound
					remove_action( 'divino_header', array( \divino_Builder_Header::get_instance(), 'header_builder_markup' ) ); // phpcs:ignore PHPCompatibility.Keywords.NewKeywords.t_namespaceFound, PHPCompatibility.LanguageConstructs.NewLanguageConstructs.t_ns_separatorFound
				}
			}
		}
		...
		public function do_footer() {
			...
			if ( $did_location ) {
				remove_action( 'divino_footer', 'divino_footer_markup' );
				if ( true === \divino_Builder_Helper::$is_header_footer_builder_active ) { // phpcs:ignore PHPCompatibility.Keywords.NewKeywords.t_namespaceFound, PHPCompatibility.LanguageConstructs.NewLanguageConstructs.t_ns_separatorFound
					remove_action( 'divino_footer', array( \divino_Builder_Footer::get_instance(), 'footer_markup' ) ); // phpcs:ignore PHPCompatibility.Keywords.NewKeywords.t_namespaceFound, PHPCompatibility.LanguageConstructs.NewLanguageConstructs.t_ns_separatorFound
				}
			}
		}
	 ...
	 */
	divino_Elementor_Pro::get_instance();
