...
if ( ! class_exists( 'divino_LifterLMS' ) ) {
	...
	class divino_LifterLMS {
		...
		public function __construct() {
			...
			// Sidebar Layout.
			add_filter( 'divino_page_layout', array( $this, 'sidebar_layout' ) );
			// Content Layout.
			add_filter( 'divino_get_content_layout', array( $this, 'content_layout' ) );
		...
		public function lifterlms_init() {
			...
			if ( is_lesson() ) {
				...
				remove_action( 'divino_entry_after', 'divino_single_post_navigation_markup' );
				if ( 'yes' !== apply_filters( 'llms_blocks_is_post_migrated', get_post_meta( get_the_ID(), '_llms_blocks_migrated', true ), get_the_ID() ) ) { // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
					add_action( 'divino_entry_after', 'lifterlms_template_lesson_navigation' );
				}
			}
			...
			if ( 'llms_certificate' === $lifter_certificate_post_type || 'llms_my_certificate' === $lifter_certificate_post_type ) {
				if ( ! is_admin() ) {
					add_filter( 'post_class', 'divino_certificate_class' );
					 ...
					 * @return array
					 */
					function divino_certificate_class( $array ) {
		...
		public function course_grid( $grid ) {
			...
			$course_grid = divino_get_option( 'llms-course-grid' );
		...
		public function course_responsive_grid( $classes ) {
			...
			$llms_grid = divino_get_option( 'llms-course-grid' );
			if ( in_array( 'llms-membership-list', $classes ) ) {
				$llms_grid = divino_get_option( 'llms-membership-grid' );
		...
		public function add_dynamic_styles() {
			...
			$theme_forground_color = divino_get_foreground_color( $link_color );
			...
			$btn_bg_color   = divino_get_option( 'button-bg-color', '', $link_color );
			...
			$btn_border_color   = divino_get_option( 'theme-button-border-group-border-color', $link_color );
			$btn_border_h_color = divino_get_option( 'theme-button-border-group-border-h-color', $link_h_color );
			...
			$theme_btn_font_extras     = divino_get_option( 'font-extras-button' );
			$theme_btn_text_transform  = divino_get_font_extras( $theme_btn_font_extras, 'text-transform' );
			$theme_btn_line_height     = divino_get_font_extras( $theme_btn_font_extras, 'line-height', 'line-height-unit' );
			$theme_btn_letter_spacing  = divino_get_font_extras( $theme_btn_font_extras, 'letter-spacing', 'letter-spacing-unit' );
			$theme_btn_text_decoration = divino_get_font_extras( $theme_btn_font_extras, 'text-decoration' );
			...
			$css_output = array(
				'.lifterlms a.llms-button-primary, .lifterlms a.llms-button-secondary, .lifterlms .llms-button-action, .lifterlms button.llms-field-button, .lifterlms a.llms-field-button' => array(
					...
					'border-top-width'           => isset( $btn_border_size['top'] ) && '' !== $btn_border_size['top'] ? divino_get_css_value( $btn_border_size['top'], 'px' ) : '0',
					'border-right-width'         => isset( $btn_border_size['right'] ) && '' !== $btn_border_size['right'] ? divino_get_css_value( $btn_border_size['right'], 'px' ) : '0',
					'border-left-width'          => isset( $btn_border_size['left'] ) && '' !== $btn_border_size['left'] ? divino_get_css_value( $btn_border_size['left'], 'px' ) : '0',
					'border-bottom-width'        => isset( $btn_border_size['bottom'] ) && '' !== $btn_border_size['bottom'] ? divino_get_css_value( $btn_border_size['bottom'], 'px' ) : '0',
			...
			wp_add_inline_style( 'lifterlms-styles', apply_filters( 'divino_theme_lifterlms_dynamic_css', $css_output ) );
		}
		...
		public function sidebar_layout( $layout ) {
			...
			if ( is_lifterlms() || is_llms_account_page() || is_llms_checkout() ) {
				...
				$llms_sidebar = divino_get_option( 'lifterlms-sidebar-layout' );
				if ( is_lesson() || is_course() ) {
					$llms_sidebar = divino_get_option( 'lifterlms-course-lesson-sidebar-layout' );
				}
				...
				if ( in_array( $post_type, $supported_post_types ) ) {
					...
					if ( is_singular() ) {
						$dynamic_sidebar_layout = divino_get_option( 'single-' . $post_type . '-sidebar-layout' );
					}

					if ( is_archive() ) {
						$dynamic_sidebar_layout = divino_get_option( 'archive-' . $post_type . '-sidebar-layout' );
					}
		...
		public function content_layout( $layout ) {
			...
			if ( is_lifterlms() || is_llms_account_page() || is_llms_checkout() ) {
				...
				$llms_layout = divino_toggle_layout( 'lifterlms-ast-content-layout', 'global', false );
				...
				if ( in_array( $post_type, $supported_post_types ) ) {
					...
					if ( is_singular() ) {
						$dynamic_sidebar_layout = divino_toggle_layout( 'single-' . $post_type . '-ast-content-layout', 'single', false );
					}
					if ( is_archive() ) {
						$dynamic_sidebar_layout = divino_toggle_layout( 'archive-' . $post_type . '-ast-content-layout', 'archive', false );
					}
		...
		public function register_builder_fields( $default_fields ) {
			...
			$show_meta_field = ! divino_Meta_Boxes::is_bb_themer_layout();
			/**
			 * Main Header.
			 */
			$disable_fields[] = array(
			...
			if ( $show_meta_field && 'disabled' !== divino_get_option( 'footer-adv' ) ) {
				$disable_fields[] = array(
			...
			if ( 'disabled' !== divino_get_option( 'footer-sml-layout' ) ) {
				$disable_fields[] = array(
		...
		public function llms_single_lesson_css() {
			...
			return divino_Enqueue_Scripts::trim_css( $single_lesson_static_css );
		}
	}
 ...
 */
if ( apply_filters( 'divino_enable_lifterlms_integration', true ) ) {
	divino_LifterLMS::get_instance();
}
