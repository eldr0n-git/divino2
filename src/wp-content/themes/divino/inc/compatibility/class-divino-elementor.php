...
use divino_Dynamic_CSS;
use divino_Global_Palette;
...
if ( ! class_exists( 'divino_Elementor' ) ) {
	...
	class divino_Elementor {
		...
		public function __construct() {
			 ...
			 * @since  2.4.5
			 */
			add_filter( 'divino_dynamic_theme_css', array( $this, 'enqueue_elementor_compatibility_styles' ) );
			...
			add_filter( 'divino_dynamic_theme_css', array( $this, 'generate_global_elementor_style' ), 11 );
			 ...
			 * @since  4.1.0
			 */
			add_filter( 'divino_entry_header_class', array( $this, 'divino_entry_header_class_custom' ), 1, 99 );
		}
		 ...
		 * @since 4.1.0
		 */
		public function divino_entry_header_class_custom( $classes ) {
			$edit_mode         = get_post_meta( divino_get_post_id(), '_elementor_edit_mode', true );
			$divino_layout_type = divino_get_option( 'ast-dynamic-single-' . get_post_type() . '-layout', 'layout-1' );

			if ( ( $edit_mode && $edit_mode === 'builder' ) || ( $edit_mode === 'builder' && $divino_layout_type === 'layout-2' ) ) {
				$classes[] = 'ast-header-without-markup';
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				if ( $divino_layout_type === 'layout-2' && in_array( 'ast-header-without-markup', $classes ) ) {
		...
		public function enqueue_elementor_compatibility_styles( $dynamic_css, $dynamic_css_filtered = '' ) {
			...
			if ( $this->is_elementor_activated( $id ) ) {
				...
				$parse_css = divino_parse_css( $elementor_heading_margin_comp );
				...
				if ( divino_can_remove_elementor_toc_margin_space() ) {
					$elementor_base_css['.elementor-toc__list-wrapper'] = array(
						'margin' => 0,
					);
				}

				if ( divino_can_add_styling_for_hr() ) {
				...
				$parse_css .= divino_parse_css( $elementor_rtl_support_css );
			...
			$dynamic_css .= divino_parse_css( $elementor_posts_container_css );
			...
			$dynamic_css               .= divino_parse_css( $elementor_archive_page_css );
		...
		public function elementor_default_setting() {
			...
			if ( false === divino_enable_page_builder_compatibility() || 'post' === get_post_type() ) {
				return;
			}
			...
			$page_builder_flag = get_post_meta( $id, '_divino_content_layout_flag', true );
			if ( isset( $post ) && empty( $page_builder_flag ) && ( is_admin() || is_singular() ) ) {

				if ( empty( $post->post_content ) && $this->is_elementor_activated( $id ) ) {

					update_post_meta( $id, '_divino_content_layout_flag', 'disabled' );
					...
					add_filter( 'divino_the_post_title_enabled', '__return_false' );
					add_filter( 'divino_featured_image_enabled', '__return_false' );
				}
			}
		}
		...
		public function elementor_add_theme_colors( $response, $handler, $request ) {
			...
			if ( divino_maybe_disable_global_color_in_elementor() ) {
				return $response;
			}
			...
			$slugs          = divino_Global_Palette::get_palette_slugs();
			$labels         = divino_Global_Palette::get_palette_labels();
		...
		public function display_global_colors_front_end( $response, $handler, $request ) {
			...
			$slug_map      = array();
			$palette_slugs = divino_Global_Palette::get_palette_slugs();
			...
			$colors = divino_get_option( 'global-color-palette' );
			return rest_ensure_response(
				array(
					'id'    => esc_attr( $rest_id ),
					'title' => divino_Global_Palette::get_css_variable_prefix() . esc_html( $slug_map[ $rest_id ] ),
		...
		public function generate_global_elementor_style( $dynamic_css ) {
			...
			if ( defined( 'ELEMENTOR_PRO_VERSION' ) && 'no' === get_option( 'elementor_use_mini_cart_template' ) ) {
				...
				$dynamic_css           .= divino_parse_css( $mini_cart_template_css );
			}
			return $dynamic_css;
		}
