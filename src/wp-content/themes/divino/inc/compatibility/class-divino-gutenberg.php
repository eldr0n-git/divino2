...
class divino_Gutenberg {
	...
	public function is_layout_with_blocks() {
		...
		if ( $post_id ) {
			...
			$enable_block_editor_attr = apply_filters( 'divino_disable_block_content_attr', true, $post_id );
			/** @psalm-suppress TooManyArguments */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort

			/** @psalm-suppress PossiblyInvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			if ( has_blocks( $current_post ) && $enable_block_editor_attr ) {
				/** @psalm-suppress PossiblyInvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				add_filter( 'divino_attr_article-entry-content-single-layout', array( $this, 'add_ast_block_container' ) );
				add_filter( 'divino_attr_article-entry-content', array( $this, 'add_ast_block_container' ) );
				add_filter( 'divino_attr_article-entry-content-page', array( $this, 'add_ast_block_container' ) );
			}
		}
	...
	public function add_iframe_wrapper( $block_content, $block ) {
		...
		return preg_replace_callback(
			...
			static function ( $matches ) use ( $video_url ) {
				return divino_After_Setup_Theme::get_instance()->responsive_oembed_wrapper( $matches[1], $video_url, array(), true );
			},
			$block_content
		);
	}
