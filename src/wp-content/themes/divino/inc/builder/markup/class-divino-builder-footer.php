...
if ( ! class_exists( 'divino_Builder_Footer' ) ) {

	/**
	 * Class divino_Builder_Footer.
	 */
	final class divino_Builder_Footer {
		/**
		 * Member Variable
		 *
		 * @var mixed instance
		...
		public function __construct() {
			...
			if ( true === divino_Builder_Helper::$is_header_footer_builder_active ) {
				...
				// Footer Builder.
				add_action( 'divino_footer', array( $this, 'footer_markup' ), 10 );

				add_action( 'divino_above_footer', array( $this, 'above_footer' ), 10 );
				add_action( 'divino_primary_footer', array( $this, 'primary_footer' ), 10 );
				add_action( 'divino_below_footer', array( $this, 'below_footer' ), 10 );

				add_action( 'divino_render_footer_column', array( $this, 'render_column' ), 10, 2 );

				// Core Components.
				add_action( 'divino_footer_copyright', array( $this, 'footer_copyright' ), 10 );

				for ( $index = 1; $index <= divino_Builder_Helper::$component_limit; $index++ ) {

					// Buttons.
					add_action( 'divino_footer_button_' . $index, array( $this, 'button_' . $index ) );
					self::$methods[] = 'button_' . $index;

					// Htmls.
					add_action( 'divino_footer_html_' . $index, array( $this, 'footer_html_' . $index ) );
					self::$methods[] = 'footer_html_' . $index;

					// Social Icons.
					add_action( 'divino_footer_social_' . $index, array( $this, 'footer_social_' . $index ) );
					self::$methods[] = 'footer_social_' . $index;

				}

				// Navigation menu.
				add_action( 'divino_footer_menu', array( $this, 'footer_menu' ) );
			}
		}
		...
		public function __call( $func, $params ) {
			...
			if ( in_array( $func, self::$methods, true ) ) {
				...
				if ( 0 === strpos( $func, 'footer_html_' ) ) {
					...
					if ( $index ) {
						divino_Builder_UI_Controller::render_button( $index, 'footer' );
					}
				} elseif ( 0 === strpos( $func, 'footer_social_' ) ) {
					$index = (int) substr( $func, strrpos( $func, '_' ) + 1 );

					if ( $index ) {
						divino_Builder_UI_Controller::render_social_icon( $index, 'footer' );
					}
				}
			}
		}
		...
		public function remove_existing_actions() {
			remove_action( 'divino_footer_content_top', 'divino_footer_content_top' );
			remove_action( 'divino_footer_content', 'divino_advanced_footer_markup', 1 );
			remove_action( 'divino_footer_content', 'divino_footer_small_footer_template', 5 );
			remove_action( 'divino_footer_content_bottom', 'divino_footer_content_bottom' );
		...
		public function footer_markup() {
			...
			$display_footer = get_post_meta( divino_get_post_id(), 'footer-sml-layout', true );

			$display_footer = apply_filters( 'divino_footer_bar_display', $display_footer );
		...
		public function render_column( $row, $column ) {
			...
			divino_Builder_Helper::render_builder_markup( $row, $column, 'desktop', 'footer' );
		}
		...
		public function footer_html_1() {
			divino_Builder_UI_Controller::render_html_markup( 'footer-html-1' );
		}
		...
		public function footer_html_2() {
			divino_Builder_UI_Controller::render_html_markup( 'footer-html-2' );
		}
		...
		public function footer_html_3() {
			divino_Builder_UI_Controller::render_html_markup( 'footer-html-3' );
		}
		...
		public function footer_html_4() {
			divino_Builder_UI_Controller::render_html_markup( 'footer-html-4' );
		}
		...
		public function footer_menu() {
			divino_Footer_Menu_Component::menu_markup();
		}
	}

	/**
	 *  Prepare if class 'divino_Builder_Footer' exist.
	 *  Kicking this off by calling 'get_instance()' method
	 */
	divino_Builder_Footer::get_instance();
}
