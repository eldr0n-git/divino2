...
if ( ! class_exists( 'divino_Builder_UI_Controller' ) ) {

	/**
	 * Class divino_Builder_UI_Controller.
	 */
	final class divino_Builder_UI_Controller {
		...
		public static function fetch_svg_icon( $icon = '', $base = true ) {
			...
			if ( ! self::$ast_svgs ) {
				ob_start();
				include_once divino_THEME_DIR . 'assets/svg/svgs.json'; // phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
				self::$ast_svgs = json_decode( ob_get_clean(), true );
				self::$ast_svgs = apply_filters( 'divino_svg_icons', self::$ast_svgs );
			}
		...
		public static function render_social_icon( $index, $builder_type = 'header' ) {
			$items        = divino_get_option( $builder_type . '-social-icons-' . $index );
			$items        = isset( $items['items'] ) ? $items['items'] : array();
			$show_label   = divino_get_option( $builder_type . '-social-' . $index . '-label-toggle' );
			$color_type   = divino_get_option( $builder_type . '-social-' . $index . '-color-type' );
			$social_stack = divino_get_option( $builder_type . '-social-' . $index . '-stack', 'none' );
			...
			if ( is_array( $items ) && ! empty( $items ) ) {
				...
				foreach ( $items as $item ) {
					if ( $item['enabled'] ) {
						 ...
						 */
						$rel = apply_filters( 'divino_social_icon_attribute_rel', 'noopener noreferrer', $item );
			...
			echo apply_filters( 'divino_social_icons_after', '' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			echo '</div>';
			echo '</div>';
		}
		...
		public static function render_html_markup( $index = 'header-html-1' ) {
			...
			$content = divino_get_i18n_option( $index, divino_Builder_Helper::get_translatable_string( $index ) );
		...
		public static function render_mobile_trigger() {
			...
			<div class="ast-button-wrap">
				<button type="button" class="menu-toggle main-header-menu-toggle ast-mobile-menu-trigger-<?php echo esc_attr( $toggle_btn_style ); ?>" <?php echo apply_filters( 'divino_nav_toggle_data_attrs', '' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?> <?php echo esc_attr( $aria_controls ); ?> aria-expanded="false">
		...
		public static function render_button( $index, $builder_type = 'header' ) {
			...
			$button_size = divino_get_option( $builder_type . '-button' . $index . '-size' );
			...
			echo divino_get_custom_button( $builder_type . '-button' . $index . '-text', $builder_type . '-button' . $index . '-link-option', 'header-button' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		...
		public static function render_account() {
			...
			$account_link     = '';
			$link_url         = '';
			$logout_preview   = divino_get_option( 'header-account-logout-preview' );
			$is_customizer    = is_customize_preview();
			$logged_out_style = divino_get_option( 'header-account-logout-style' );

			if ( ! $is_logged_in && 'none' === $logged_out_style ) {
				return;
			}

			$icon_skin = '' !== divino_get_option( 'header-account-icon-type' ) ? divino_get_option( 'header-account-icon-type' ) : 'account-1';
			...
			<div class="ast-header-account-wrap" tabindex="0">
				...
				if ( $is_logged_in && ( ( ! $logout_preview || ( 'none' === $logged_out_style && $logout_preview ) && $is_customizer ) || ( ! $is_customizer ) ) ) {
					...
					$login_profile_type = divino_get_option( 'header-account-login-style' );

					$extend_text_profile_type = divino_get_option( 'header-account-login-style-extend-text-profile-type' );

					$action_type = divino_get_option( 'header-account-action-type' );
					$link_type   = divino_get_option( 'header-account-link-type' );
					...
					<div class="ast-header-account-inner-wrap">
						...
						<?php
						if ( defined( 'divino_EXT_VER' ) && 'menu' === $action_type ) {
							divino_Header_Account_Component::account_menu_markup();
						}
						?>
					</div>
