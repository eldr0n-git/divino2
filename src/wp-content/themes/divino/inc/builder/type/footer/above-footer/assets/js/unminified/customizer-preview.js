...
( function( $ ) {
    ...
    divino_css(
        'divino-settings[hba-footer-vertical-alignment]',
        'align-items',
        selector + ' .ast-builder-grid-row, ' + selector + ' .site-footer-section'
    );

	// Border Bottom width.
	wp.customize( 'divino-settings[hba-footer-separator]', function( setting ) {
		setting.bind( function( separator ) {
			...
			divino_add_dynamic_css( 'hba-footer-separator', dynamicStyle );

		} );
	} );

	// Inner Space.
	wp.customize( 'divino-settings[hba-inner-spacing]', function( value ) {
		value.bind( function( spacing ) {
			...
			divino_add_dynamic_css( 'hba-inner-spacing-toggle-button', dynamicStyle );
		} );
	} );

	// Border Color.
	wp.customize( 'divino-settings[hba-footer-top-border-color]', function( setting ) {
		setting.bind( function( color ) {
			...
			divino_add_dynamic_css( 'hba-footer-top-border-color', dynamicStyle );

		} );
	} );

	// Primary Header - Layout.
	wp.customize( 'divino-settings[hba-footer-layout-width]', function( setting ) {
		setting.bind( function( layout ) {
			...
			divino_add_dynamic_css( 'hba-footer-layout-width', dynamicStyle );

		} );
	} );

	// Responsive BG styles > Above Footer Row.
	divino_apply_responsive_background_css( 'divino-settings[hba-footer-bg-obj-responsive]', selector, 'desktop' );
	divino_apply_responsive_background_css( 'divino-settings[hba-footer-bg-obj-responsive]', selector, 'tablet' );
	divino_apply_responsive_background_css( 'divino-settings[hba-footer-bg-obj-responsive]', selector, 'mobile' );

	// Advanced CSS Generation.
	divino_builder_advanced_css( section, selector );

	// Advanced Visibility CSS Generation.
	divino_builder_visibility_css( section, selector, 'grid' );
