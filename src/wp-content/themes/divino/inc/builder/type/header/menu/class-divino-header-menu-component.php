...
define( 'divino_HEADER_MENU_DIR', divino_THEME_DIR . 'inc/builder/type/header/menu' );
define( 'divino_HEADER_MENU_URI', divino_THEME_URI . 'inc/builder/type/header/menu' );
 ...
 * @since 3.0.0
 */
class divino_Header_Menu_Component {
	/**
	...
	public function __construct() {
		...
		if ( ! is_admin() || divino_Builder_Customizer::divino_collect_customizer_builder_data() ) {
			require_once divino_HEADER_MENU_DIR . '/dynamic-css/dynamic.css.php';
		}
	...
	public static function menu_markup( $index, $device = 'desktop' ) {
		...
		$submenu_class         = apply_filters( 'divino_secondary_submenu_border_class', ' submenu-with-border' );
		$stack_on_mobile_class = 'inline-on-mobile';

		if ( divino_get_option( 'header-' . $_prefix . '-menu-stack-on-mobile' ) ) {
		...
		$menu_hover_animation = divino_get_option( 'header-' . $_prefix . '-menu-hover-animation' );
		 ...
		 * @var Array
		 */
		$menu_classes = apply_filters( 'divino_' . $theme_location . '_menu_classes', array( 'main-header-menu', 'ast-menu-shadow', 'ast-nav-menu', 'ast-flex', $submenu_class, $stack_on_mobile_class ) );

		$items_wrap  = '<nav ';
		$items_wrap .= divino_attr(
			'site-navigation',
			array(
				'id'         => apply_filters( 'divino_header_site_navigation_id', esc_attr( $theme_location ) . '-site-navigation-' . esc_attr( $device ) ),
		...
		$fallback_menu_args = array(
			...
			'menu_id'        => apply_filters( 'divino_header_menu_ul_id', 'ast-hf-menu-' . $index ),
		...
		if ( has_nav_menu( $theme_location ) ) {
			...
			$nav_menu_markup = $divino_builder->nofollow_markup( $theme_location, $nav_menu_markup );
 ...
 */
new divino_Header_Menu_Component();
