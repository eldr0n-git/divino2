 ...
 * Account
 */
add_filter( 'divino_dynamic_theme_css', 'divino_hb_account_dynamic_css' );
 ...
 * @since 3.0.0
 */
function divino_hb_account_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {

	if ( ! divino_Builder_Helper::is_component_loaded( 'account', 'header' ) ) {
		return $dynamic_css;
	}
	...
	$icon_size = divino_get_option( 'header-account-icon-size' );
	...
	$image_width = divino_get_option( 'header-account-image-width' );
	...
	$margin          = divino_get_option( 'header-account-margin' );
	...
	$css_output_desktop = array(
		...
		$selector . ' .ast-header-account-type-icon .ahfb-svg-iconset svg path:not( .ast-hf-account-unfill ), ' . $selector . ' .ast-header-account-type-icon .ahfb-svg-iconset svg circle' => array(
			'fill' => esc_attr( divino_get_option( 'header-account-icon-color' ) ),
		),
		...
		$selector . ' .ast-header-account-type-avatar .avatar' => array(
			'width' => divino_get_css_value( $image_width_desktop, 'px' ),
		),
		$selector . ' .ast-header-account-text'   => array(
			'color' => esc_attr( divino_get_option( 'header-account-type-text-color' ) ),
		),
	...
	$css_output_tablet = array(
		...
		$selector . ' .ast-header-account-type-avatar .avatar' => array(
			'width' => divino_get_css_value( $image_width_tablet, 'px' ),
		),
		$margin_selector => array(
			// Margin CSS.
	...
	$css_output_mobile = array(
		...
		$selector . ' .ast-header-account-type-avatar .avatar' => array(
			'width' => divino_get_css_value( $image_width_mobile, 'px' ),
		),
		$margin_selector => array(
			// Margin CSS.
	...
	$dynamic_css .= divino_Builder_Base_Dynamic_CSS::prepare_advanced_typography_css( $_section, $selector . ' .ast-header-account-text' );
