 ...
 * Search
 */
add_filter( 'divino_dynamic_theme_css', 'divino_hb_woo_cart_dynamic_css' );

if ( ! function_exists( 'divino_cart_position' ) ) {
	/**
	 * Position markup
	 *
	 * @since 3.9.0
	 ...
	 * @return array
	 */
	function divino_cart_position( $postion, $device ) {
		switch ( $postion ) {
			case 'bottom':
				return array(
 ...
 * @since 3.0.0
 */
function divino_hb_woo_cart_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {

	if ( ! divino_Builder_Helper::is_component_loaded( 'woo-cart', 'header' ) ) {
		return $dynamic_css;
	}
	...
	$icon_color            = esc_attr( divino_get_option( 'header-woo-cart-icon-color', $theme_color ) );
	$icon_hover_color      = esc_attr( divino_get_option( 'header-woo-cart-icon-hover-color' ) ); // icon cart hover color.

	$header_cart_icon_radius_fields  = divino_get_option( 'woo-header-cart-icon-radius-fields' );
	$cart_h_color                    = divino_get_foreground_color( $icon_color );
	$header_cart_icon_style          = divino_get_option( 'woo-header-cart-icon-style' );
	$theme_h_color                   = divino_get_foreground_color( $theme_color );
	$cart_products_count_color       = divino_get_option( 'woo-header-cart-product-count-color', $theme_h_color );
	$cart_products_count_color_hover = divino_get_option( 'woo-header-cart-product-count-h-color' );

	$transparent_header_icon_color   = esc_attr( divino_get_option( 'transparent-header-woo-cart-icon-color', $icon_color ) );
	$transparent_header_cart_h_color = divino_get_foreground_color( $transparent_header_icon_color );
	$header_woo_cart_list            = divino_get_option( 'woo-header-cart-icon', 'default' );
	 ...
	 */
	$cart_text_color        = divino_get_option( 'header-woo-cart-text-color' );
	$cart_link_color        = divino_get_option( 'header-woo-cart-link-color' );
	$cart_bg_color          = divino_get_option( 'header-woo-cart-background-color' );
	$cart_h_bg_color        = divino_get_option( 'header-woo-cart-background-hover-color' );
	$cart_separator_color   = divino_get_option( 'header-woo-cart-separator-color' );
	$cart_h_link_color      = divino_get_option( 'header-woo-cart-link-hover-color' );
	$cart_button_text_color = divino_get_option( 'header-woo-cart-btn-text-color' );
	$cart_icon_size         = divino_get_option( 'header-woo-cart-icon-size' );

	$cart_button_bg_color     = divino_get_option( 'header-woo-cart-btn-background-color' );
	$cart_button_text_h_color = divino_get_option( 'header-woo-cart-btn-text-hover-color' );
	$cart_button_bg_h_color   = divino_get_option( 'header-woo-cart-btn-bg-hover-color' );

	$checkout_button_text_color   = divino_get_option( 'header-woo-checkout-btn-text-color' );
	$checkout_button_bg_color     = divino_get_option( 'header-woo-checkout-btn-background-color' );
	$checkout_button_text_h_color = divino_get_option( 'header-woo-checkout-btn-text-hover-color' );
	$checkout_button_bg_h_color   = divino_get_option( 'header-woo-checkout-btn-bg-hover-color' );
	$cart_total_label_position    = divino_get_option( 'woo-header-cart-icon-total-label-position' );
	$cart_badge_display           = divino_get_option( 'woo-header-cart-badge-display' );

	$check_transparent_is_enabled_on = divino_get_option( 'transparent-header-on-devices' );
	...
	$css_output_desktop = array(
		...
		$selector . ' .ast-addon-cart-wrap i.divino-icon:after' => array(
			'color'            => esc_attr( $theme_h_color ),
			'background-color' => esc_attr( $icon_color ),
		),
		'.ast-icon-shopping-bag .ast-icon svg, .ast-icon-shopping-cart .ast-icon svg, .ast-icon-shopping-basket .ast-icon svg' => array(
			'height' => divino_get_css_value( $cart_icon_size_desktop, 'px' ),
			'width'  => divino_get_css_value( $cart_icon_size_desktop, 'px' ),
		),
		'.ast-cart-menu-wrap, i.divino-icon.ast-icon-shopping-' . $header_woo_cart_list => array(
			'font-size' => divino_get_css_value( $cart_icon_size_desktop, 'px' ),
		),
		 ...
		 */
		'.ast-site-header-cart .ast-site-header-cart-data .widget_shopping_cart_content a:not(.button), .divino-cart-drawer .widget_shopping_cart_content a:not(.button)' => array(
		...
		'.ast-site-header-cart .ast-site-header-cart-data .widget_shopping_cart_content a:not(.button):hover, .divino-cart-drawer .widget_shopping_cart_content a:not(.button):hover' => array(
			'color' => esc_attr( $cart_h_link_color_desktop ),
		),
		'.ast-site-header-cart .woocommerce.widget_shopping_cart, div.divino-cart-drawer ' => array(
			'background-color' => esc_attr( $cart_bg_color_desktop ),
			'border-color'     => esc_attr( $cart_bg_color_desktop ),
		),
		'.ast-site-header-cart .widget_shopping_cart:hover, .divino-cart-drawer:hover' => array(
			'background-color' => esc_attr( $cart_h_bg_color_desktop ),
			'border-color'     => esc_attr( $cart_h_bg_color_desktop ),
		),
		'.ast-site-header-cart .widget_shopping_cart .woocommerce-mini-cart__total, .divino-cart-drawer .divino-cart-drawer-, .woocommerce-js .divino-cart-drawer .divino-cart-drawer-content .woocommerce-mini-cart__total, .divino-cart-drawer .divino-cart-drawer-header' => array(
			'border-top-color'    => esc_attr( $cart_separator_color_desktop ),
			'border-bottom-color' => esc_attr( $cart_separator_color_desktop ),
		),
		'.ast-site-header-cart .widget_shopping_cart .mini_cart_item,.divino-cart-drawer .divino-cart-drawer-content .widget_shopping_cart_content ul li' => array(
			'border-bottom-color' => $cart_separator_color_desktop ? divino_hex_to_rgba( $cart_separator_color_desktop ) : '',
		),
		 ...
		 */
		'.ast-site-header-cart .ast-site-header-cart-data .widget_shopping_cart_content a.button.wc-forward:not(.checkout), .woocommerce-js .divino-cart-drawer .widget_shopping_cart_content a.button.wc-forward:not(.checkout), .ast-site-header-cart .ast-site-header-cart-data .widget_shopping_cart_content a.button.wc-forward:not(.checkout),  .woocommerce-js .divino-cart-drawer .divino-cart-drawer-content .woocommerce-mini-cart__buttons .button:not(.checkout):not(.ast-continue-shopping)' => array(
			'color'            => esc_attr( $cart_button_text_color_desktop ),
			'background-color' => esc_attr( $cart_button_bg_color_desktop ),
		),
		'.ast-site-header-cart .ast-site-header-cart-data .widget_shopping_cart_content a.button.wc-forward:not(.checkout):hover, .woocommerce-js .divino-cart-drawer .widget_shopping_cart_content a.button.wc-forward:not(.checkout):hover, .woocommerce-js .divino-cart-drawer .divino-cart-drawer-content .woocommerce-mini-cart__buttons .button:not(.checkout):not(.ast-continue-shopping):hover' => array(
		 ...
		 */
		'.ast-site-header-cart .ast-site-header-cart-data .widget_shopping_cart_content a.button.checkout.wc-forward, .divino-cart-drawer .widget_shopping_cart_content a.button.checkout.wc-forward' => array(
		...
		'.ast-site-header-cart .ast-site-header-cart-data .widget_shopping_cart_content a.button.checkout.wc-forward:hover, .divino-cart-drawer .widget_shopping_cart_content a.button.checkout.wc-forward:hover' => array(
			'color'            => esc_attr( $checkout_button_text_h_color_desktop ),
			'background-color' => esc_attr( $checkout_button_bg_h_color_desktop ),
		),

		'.ast-site-header-cart .ast-addon-cart-wrap i.divino-icon:after, .ast-theme-transparent-header .ast-site-header-cart .ast-addon-cart-wrap i.divino-icon:after' => array(
			'color' => esc_attr( $cart_products_count_color ),
		),
	);

	// Offcanvas cart.
	$flyout_cart_width              = divino_get_option( 'woo-slide-in-cart-width' );
	...
	if ( 'flyout' === divino_get_option( 'woo-header-cart-click-action' ) || is_customize_preview() ) {
		$desktop_flyout_cart_direction = divino_get_option( 'woo-desktop-cart-flyout-direction' );
		...
		$css_output_desktop['.ast-desktop .divino-cart-drawer'] = array(
			'width' => divino_get_css_value( $flyout_cart_width_desktop, $flyout_cart_width_desktop_unit ),
		);
		if ( 'left' === $desktop_flyout_cart_direction ) {
			$css_output_desktop['.ast-desktop .divino-cart-drawer']        = array(
				'width' => divino_get_css_value( $flyout_cart_width_desktop, $flyout_cart_width_desktop_unit ),
				'left'  => '-' . divino_get_css_value( $flyout_cart_width_desktop, $flyout_cart_width_desktop_unit ),
			);
			$css_output_desktop['.ast-desktop .divino-cart-drawer.active'] = array(
				'left' => divino_get_css_value( $flyout_cart_width_desktop, $flyout_cart_width_desktop_unit ),
			);
		}
	}
	...
	$responsive_selector = '.divino-cart-drawer';

	$css_output_mobile = array(
		$responsive_selector . ' .divino-cart-drawer-title, ' . $responsive_selector . ' .widget_shopping_cart_content span, ' . $responsive_selector . ' .widget_shopping_cart_content strong,' . $responsive_selector . ' .woocommerce-mini-cart__empty-message, .divino-cart-drawer .woocommerce-mini-cart *' => array(
		...
		$responsive_selector . '#divino-mobile-cart-drawer' => array(
			'background-color' => esc_attr( $cart_bg_color_mobile ),
			'border-color'     => esc_attr( $cart_bg_color_mobile ),
		),
		$responsive_selector . '#divino-mobile-cart-drawer:hover' => array(
			'background-color' => esc_attr( $cart_h_bg_color_mobile ),
			'border-color'     => esc_attr( $cart_h_bg_color_mobile ),
		),
		'#divino-mobile-cart-drawer:hover .widget_shopping_cart:before, #divino-mobile-cart-drawer:hover .widget_shopping_cart:after, .open-preview-woocommerce-cart #divino-mobile-cart-drawer .widget_shopping_cart:before' => array(
		...
		'#divino-mobile-cart-drawer .widget_shopping_cart .woocommerce-mini-cart__total, .divino-cart-drawer .divino-cart-drawer-header' => array(
			'border-top-color'    => esc_attr( $cart_separator_color_mobile ),
			'border-bottom-color' => esc_attr( $cart_separator_color_mobile ),
		),
		'#divino-mobile-cart-drawer .widget_shopping_cart .mini_cart_item' => array(
			'border-bottom-color' => $cart_separator_color_mobile ? divino_hex_to_rgba( $cart_separator_color_mobile ) : '',
		),
		...
		$responsive_selector . '.ast-icon-shopping-bag .ast-icon svg, .ast-icon-shopping-cart .ast-icon svg, .ast-icon-shopping-basket .ast-icon svg' => array(
			'height' => divino_get_css_value( $cart_icon_size_mobile, 'px' ),
			'width'  => divino_get_css_value( $cart_icon_size_mobile, 'px' ),
		),
		'.ast-header-break-point.ast-hfb-header .ast-cart-menu-wrap, i.divino-icon.ast-icon-shopping-' . $header_woo_cart_list => array(
			'font-size' => divino_get_css_value( $cart_icon_size_mobile, 'px' ),
		),
		...
		$responsive_selector . '.active'                   => array(
			'width' => divino_get_css_value( $flyout_cart_width_mobile, $flyout_cart_width_mobile_unit ),
		),
		 ...
		 * Cart button color for widget
		 */
		'.woocommerce-js ' . $responsive_selector . ' .divino-cart-drawer-content .woocommerce-mini-cart__buttons .button:not(.checkout):not(.ast-continue-shopping)' => array(
			'color'            => esc_attr( $cart_button_text_color_mobile ),
			'background-color' => esc_attr( $cart_button_bg_color_mobile ),
		),
		'.woocommerce-js ' . $responsive_selector . ' .divino-cart-drawer-content .woocommerce-mini-cart__buttons .button:not(.checkout):not(.ast-continue-shopping):hover' => array(
	...
	$css_output_tablet = array(
		...
		'#divino-mobile-cart-drawer .widget_shopping_cart .mini_cart_item' => array(
			'border-bottom-color' => $cart_separator_color_tablet ? divino_hex_to_rgba( $cart_separator_color_tablet ) : '',
		),
		$responsive_selector . ' .widget_shopping_cart_content a:not(.button):hover' => array(
			'color' => esc_attr( $cart_h_link_color_tablet ),
		),
		$responsive_selector . '.ast-icon-shopping-bag .ast-icon svg, .ast-icon-shopping-cart .ast-icon svg, .ast-icon-shopping-basket .ast-icon svg' => array(
			'height' => divino_get_css_value( $cart_icon_size_tablet, 'px' ),
			'width'  => divino_get_css_value( $cart_icon_size_tablet, 'px' ),
		),
		'.ast-header-break-point.ast-hfb-header .ast-cart-menu-wrap, i.divino-icon.ast-icon-shopping-' . $header_woo_cart_list => array(
			'font-size' => divino_get_css_value( $cart_icon_size_tablet, 'px' ),
		),
		 ...
		 */
		$responsive_selector . '.active'                   => array(
			'width' => divino_get_css_value( $flyout_cart_width_tablet, $flyout_cart_width_tablet_unit ),
		),
	...
	if ( 'none' !== $header_cart_icon_style ) {
		...
		if ( function_exists( 'divino_has_pro_woocommerce_addon' ) && ! divino_has_pro_woocommerce_addon() && 'outline' === $header_cart_icon_style && 'default' !== $header_woo_cart_list ) {
			$border_width             = divino_get_option( 'woo-header-cart-border-width' );
			$header_cart_icon_outline = array(
				'.ast-menu-cart-outline .ast-cart-menu-wrap .count, .ast-menu-cart-outline .ast-addon-cart-wrap'  => array(
					...
					'border-width' => divino_get_css_value( $border_width, 'px' ),
				),
			);

			$css_output .= divino_parse_css( $header_cart_icon_outline );
		}
		...
		$header_cart_icon = array(
			...
			'.ast-site-header-cart.ast-menu-cart-outline .ast-cart-menu-wrap, .ast-site-header-cart.ast-menu-cart-fill .ast-cart-menu-wrap, .ast-site-header-cart.ast-menu-cart-outline .ast-cart-menu-wrap .count, .ast-site-header-cart.ast-menu-cart-fill .ast-cart-menu-wrap .count, .ast-site-header-cart.ast-menu-cart-outline .ast-addon-cart-wrap, .ast-site-header-cart.ast-menu-cart-fill .ast-addon-cart-wrap, .ast-site-header-cart.ast-menu-cart-outline .ast-woo-header-cart-info-wrap, .ast-site-header-cart.ast-menu-cart-fill .ast-woo-header-cart-info-wrap' => array(
				'border-top-left-radius'     => divino_responsive_spacing( $header_cart_icon_radius_fields, 'top', 'desktop' ),
				'border-top-right-radius'    => divino_responsive_spacing( $header_cart_icon_radius_fields, 'right', 'desktop' ),
				'border-bottom-right-radius' => divino_responsive_spacing( $header_cart_icon_radius_fields, 'bottom', 'desktop' ),
				'border-bottom-left-radius'  => divino_responsive_spacing( $header_cart_icon_radius_fields, 'left', 'desktop' ),
			),
		...
		$header_cart_icon_tablet = array(
			// Border radius.
			'.ast-site-header-cart.ast-menu-cart-outline .ast-cart-menu-wrap, .ast-site-header-cart.ast-menu-cart-fill .ast-cart-menu-wrap, .ast-site-header-cart.ast-menu-cart-outline .ast-cart-menu-wrap .count, .ast-site-header-cart.ast-menu-cart-fill .ast-cart-menu-wrap .count, .ast-site-header-cart.ast-menu-cart-outline .ast-addon-cart-wrap, .ast-site-header-cart.ast-menu-cart-fill .ast-addon-cart-wrap, .ast-site-header-cart.ast-menu-cart-outline .ast-woo-header-cart-info-wrap, .ast-site-header-cart.ast-menu-cart-fill .ast-woo-header-cart-info-wrap' => array(
				'border-top-left-radius'     => divino_responsive_spacing( $header_cart_icon_radius_fields, 'top', 'tablet' ),
				'border-top-right-radius'    => divino_responsive_spacing( $header_cart_icon_radius_fields, 'right', 'tablet' ),
				'border-bottom-right-radius' => divino_responsive_spacing( $header_cart_icon_radius_fields, 'bottom', 'tablet' ),
				'border-bottom-left-radius'  => divino_responsive_spacing( $header_cart_icon_radius_fields, 'left', 'tablet' ),
			),
		...
		$header_cart_icon_mobile = array(
			// Border radius.
			'.ast-site-header-cart.ast-menu-cart-outline .ast-cart-menu-wrap, .ast-site-header-cart.ast-menu-cart-fill .ast-cart-menu-wrap, .ast-site-header-cart.ast-menu-cart-outline .ast-cart-menu-wrap .count, .ast-site-header-cart.ast-menu-cart-fill .ast-cart-menu-wrap .count, .ast-site-header-cart.ast-menu-cart-outline .ast-addon-cart-wrap, .ast-site-header-cart.ast-menu-cart-fill .ast-addon-cart-wrap, .ast-site-header-cart.ast-menu-cart-outline .ast-woo-header-cart-info-wrap, .ast-site-header-cart.ast-menu-cart-fill .ast-woo-header-cart-info-wrap' => array(
				'border-top-left-radius'     => divino_responsive_spacing( $header_cart_icon_radius_fields, 'top', 'mobile' ),
				'border-top-right-radius'    => divino_responsive_spacing( $header_cart_icon_radius_fields, 'right', 'mobile' ),
				'border-bottom-right-radius' => divino_responsive_spacing( $header_cart_icon_radius_fields, 'bottom', 'mobile' ),
				'border-bottom-left-radius'  => divino_responsive_spacing( $header_cart_icon_radius_fields, 'left', 'mobile' ),
			),
		);

		// We adding this conditional CSS only to maintain backwards. Remove this condition after 2-3 updates of add-on.
		if ( defined( 'divino_EXT_VER' ) && version_compare( divino_EXT_VER, '3.4.2', '<' ) ) {
		...
		$css_output .= divino_parse_css( $header_cart_icon );
		$css_output .= divino_parse_css( $header_cart_icon_tablet, '', divino_get_tablet_breakpoint() );
		$css_output .= divino_parse_css( $header_cart_icon_mobile, '', divino_get_mobile_breakpoint() );
	}
	...
	$remove_when_transparent_header = array(
		...
		'.ast-menu-cart-fill .ast-site-header-cart-li:hover .ast-cart-menu-wrap .count, .ast-menu-cart-fill .ast-site-header-cart-li:hover .ast-cart-menu-wrap, .ast-menu-cart-fill .ast-site-header-cart-li:hover .ast-addon-cart-wrap, .ast-menu-cart-fill .ast-site-header-cart-li:hover .ast-addon-cart-wrap .ast-woo-header-cart-info-wrap, .ast-menu-cart-fill .ast-site-header-cart-li:hover .ast-addon-cart-wrap i.divino-icon:after' => array(
		...
		$selector . ' .ast-site-header-cart-li:hover .ast-addon-cart-wrap i.divino-icon:after' => array(
			'color'            => esc_attr( $theme_h_color ),
			'background-color' => $icon_hover_color,
		),

		// Label/Fill icon hover Color.
		'.ast-site-header-cart-li:hover .ast-woo-header-cart-info-wrap,' . $selector . ' .ast-site-header-cart-li:hover .ast-addon-cart-wrap, .ast-menu-cart-outline .ast-site-header-cart-li:hover .ast-addon-cart-wrap .divino-icon' => array(
			'color' => $icon_hover_color,
		),
		...
		'.ast-site-header-cart.ast-menu-cart-fill .ast-site-header-cart-li:hover .divino-icon' => array(
			'color' => $cart_h_color,
		),
	...
	if ( 'default' !== $header_woo_cart_list ) {
		...
		if ( ! divino_Ext_Transparent_Header_Markup::is_transparent_header() ) {
			$css_output .= divino_parse_css( $svg_cart_icon_not_transparent );
		}
	}

	$remove_when_tp_header = array(
		'.ast-site-header-cart .ast-site-header-cart-li:hover .ast-addon-cart-wrap i.divino-icon:after, .ast-theme-transparent-header .ast-site-header-cart .ast-site-header-cart-li:hover .ast-addon-cart-wrap i.divino-icon:after' => array(
			'color' => esc_attr( $cart_products_count_color_hover ),
		),
	);

	if ( $check_transparent_is_enabled_on && 'desktop' !== $check_transparent_is_enabled_on && 'both' !== $check_transparent_is_enabled_on ) {
		$css_output .= divino_parse_css( $remove_when_tp_header, divino_get_tablet_breakpoint( '', 1 ) );
	}

	if ( $check_transparent_is_enabled_on && 'mobile' !== $check_transparent_is_enabled_on && 'both' !== $check_transparent_is_enabled_on ) {
		$css_output .= divino_parse_css( $remove_when_tp_header, '', divino_get_tablet_breakpoint() );
	}
	...
	$add_when_transparent_header = array(
		...
		$trans_header_selector . ' .ast-addon-cart-wrap i.divino-icon:after' => array(
	...
	if ( ! divino_Ext_Transparent_Header_Markup::is_transparent_header() ) {
		$css_output .= divino_parse_css( $remove_when_transparent_header );
	}

	if ( divino_Ext_Transparent_Header_Markup::is_transparent_header() ) {

		/** @psalm-suppress InvalidScalarArgument  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$tablet_breakpoint = divino_get_tablet_breakpoint( '', 1 );

		if ( $check_transparent_is_enabled_on && 'desktop' !== $check_transparent_is_enabled_on && 'both' !== $check_transparent_is_enabled_on ) {
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$css_output .= divino_parse_css( $remove_when_transparent_header, $tablet_breakpoint, '' );
		}

		if ( $check_transparent_is_enabled_on && 'mobile' !== $check_transparent_is_enabled_on && 'both' !== $check_transparent_is_enabled_on ) {
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$css_output .= divino_parse_css( $remove_when_transparent_header, '', divino_get_tablet_breakpoint() );
		}

		if ( $check_transparent_is_enabled_on && 'desktop' === $check_transparent_is_enabled_on ) {
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$css_output .= divino_parse_css( $add_when_transparent_header, $tablet_breakpoint, '' );
		} elseif ( $check_transparent_is_enabled_on && 'mobile' === $check_transparent_is_enabled_on ) {
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$css_output .= divino_parse_css( $add_when_transparent_header, '', divino_get_tablet_breakpoint() );
		} else {
			$css_output .= divino_parse_css( $add_when_transparent_header );
		}
	}
	...
	if ( false === $cart_badge_display ) {
		$css_badge_output = array(
			'.divino-icon.divino-icon::after' => array(
				'display' => 'none',
			),
			...
			$css_output  .= divino_parse_css( $css_badge_output );
	}

	/**
	 * Added for the Cart total label badge position
	 */
	$cart_dynamic_label = divino_get_option( 'woo-header-cart-label-display' );
	if ( ! empty( $cart_dynamic_label ) ) {
		...
		$css_output .= divino_parse_css( $css_total_position_common_selector );
	}

	if ( 'default' !== $header_woo_cart_list ) {

		$svg_cart_icon_not_transparent = array(
			'.ast-site-header-cart .ast-site-header-cart-li:hover .ast-addon-cart-wrap i.divino-icon:after' => array(
				'color' => esc_attr( $cart_products_count_color_hover ),
			),
		);
	...
	if ( $cart_label_position_desktop ) {
		$cart_l_p_desktop = divino_cart_position( $cart_label_position_desktop, 'desktop' );
		/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$css_output .= divino_parse_css( $cart_l_p_desktop, divino_get_tablet_breakpoint( '', '1' ) );
	}
	if ( $cart_label_position_mobile ) {
		$cart_l_p_mobile = divino_cart_position( $cart_label_position_mobile, 'mobile' );
	}
	if ( $cart_label_position_tablet ) {
		$cart_l_p_tablet = divino_cart_position( $cart_label_position_tablet, 'tablet' );
	}
	/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	$css_output .= divino_parse_css( $cart_l_p_tablet, divino_get_mobile_breakpoint( '', '1' ), divino_get_tablet_breakpoint() );
	/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	$css_output .= divino_parse_css( $cart_l_p_mobile, '', divino_get_mobile_breakpoint( '', '1' ) );
	...
	$css_output      .= divino_parse_css( $angle_transition );

	$css_output .= divino_Extended_Base_Dynamic_CSS::prepare_advanced_margin_padding_css( 'section-header-woo-cart', '.woocommerce .ast-header-woo-cart .ast-site-header-cart .ast-addon-cart-wrap, .ast-header-woo-cart .ast-site-header-cart .ast-addon-cart-wrap' );
	$css_output .= divino_Builder_Base_Dynamic_CSS::prepare_visibility_css( 'section-header-woo-cart', '.ast-header-woo-cart' );
