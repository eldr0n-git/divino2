 ...
 * @since 3.0.0
 */
class divino_Header_Button_Component_Loader {
	/**
	 * Constructor
	 *
	 * @since 3.0.0
	...
	public function add_fonts() {
		 ...
		 * Header - Button
		 */
		$num_of_header_button = divino_Builder_Helper::$num_of_header_button;
		for ( $index = 1; $index <= $num_of_header_button; $index++ ) {

			if ( ! divino_Builder_Helper::is_component_loaded( 'button-' . $index, 'header' ) ) {
				continue;
			}

			$_prefix = 'button' . $index;

			$btn_font_family = divino_get_option( 'header-' . $_prefix . '-font-family' );
			$btn_font_weight = divino_get_option( 'header-' . $_prefix . '-font-weight' );
 ...
 */
new divino_Header_Button_Component_Loader();
