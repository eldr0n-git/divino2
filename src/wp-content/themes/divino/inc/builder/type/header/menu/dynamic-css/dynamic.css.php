 ...
 * Heading Colors
 */
add_filter( 'divino_dynamic_theme_css', 'divino_hb_menu_dynamic_css' );
 ...
 * @since 3.0.0
 */
function divino_hb_menu_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {

	for ( $index = 1; $index <= divino_Builder_Helper::$num_of_header_menu; $index++ ) {

		if ( ! divino_Builder_Helper::is_component_loaded( 'menu-' . $index, 'header' ) ) {
			continue;
		}
		...
		// Sub Menu.
		$sub_menu_border               = divino_get_option( 'header-' . $_prefix . '-submenu-border' );
		$sub_menu_divider_toggle       = divino_get_option( 'header-' . $_prefix . '-submenu-item-border' );
		$sub_menu_divider_size         = divino_get_option( 'header-' . $_prefix . '-submenu-item-b-size' );
		$sub_menu_divider_color        = divino_get_option( 'header-' . $_prefix . '-submenu-item-b-color' );
		$sub_menu_border_radius_fields = divino_get_option( 'header-' . $_prefix . '-submenu-border-radius-fields' );
		$sub_menu_top_offset           = divino_get_option( 'header-' . $_prefix . '-submenu-top-offset' );
		$sub_menu_width                = divino_get_option( 'header-' . $_prefix . '-submenu-width' );

		// Menu.
		$menu_resp_color           = divino_get_option( 'header-' . $_prefix . '-color-responsive' );
		$menu_resp_bg_color        = divino_get_option( 'header-' . $_prefix . '-bg-obj-responsive' );
		$menu_resp_color_hover     = divino_get_option( 'header-' . $_prefix . '-h-color-responsive' );
		$menu_resp_bg_color_hover  = divino_get_option( 'header-' . $_prefix . '-h-bg-color-responsive' );
		$menu_resp_color_active    = divino_get_option( 'header-' . $_prefix . '-a-color-responsive' );
		$menu_resp_bg_color_active = divino_get_option( 'header-' . $_prefix . '-a-bg-color-responsive' );
		...
		// Typography.
		$menu_font_family     = divino_get_option( 'header-' . $_prefix . '-font-family' );
		$menu_font_size       = divino_get_option( 'header-' . $_prefix . '-font-size' );
		$menu_font_weight     = divino_get_option( 'header-' . $_prefix . '-font-weight' );
		$menu_text_transform  = divino_get_font_extras( divino_get_option( 'header-' . $_prefix . '-font-extras' ), 'text-transform' );
		$menu_line_height     = divino_get_font_extras( divino_get_option( 'header-' . $_prefix . '-font-extras' ), 'line-height', 'line-height-unit' );
		$menu_letter_spacing  = divino_get_font_extras( divino_get_option( 'header-' . $_prefix . '-font-extras' ), 'letter-spacing', 'letter-spacing-unit' );
		$menu_text_decoration = divino_get_font_extras( divino_get_option( 'header-' . $_prefix . '-font-extras' ), 'text-decoration' );
		...
		// Spacing.
		$menu_spacing = divino_get_option( 'header-' . $_prefix . '-menu-spacing' );
		...
		$css_output_desktop = array(
			...
			// Menu.
			$selector                                    => array(
				'font-family'    => divino_get_font_family( $menu_font_family ),
			...
			$selector . ' .sub-menu, ' . $selector . ' .inline-on-mobile .sub-menu' => array(
				'border-top-width'           => divino_get_css_value( $sub_menu_border_top, 'px' ),
				'border-bottom-width'        => divino_get_css_value( $sub_menu_border_bottom, 'px' ),
				'border-right-width'         => divino_get_css_value( $sub_menu_border_right, 'px' ),
				'border-left-width'          => divino_get_css_value( $sub_menu_border_left, 'px' ),
				'border-color'               => esc_attr( divino_get_option( 'header-' . $_prefix . '-submenu-b-color', $theme_color ) ),
				'border-style'               => 'solid',
				'width'                      => divino_get_css_value( $sub_menu_width, 'px' ),
				'border-top-left-radius'     => divino_responsive_spacing( $sub_menu_border_radius_fields, 'top', 'desktop' ),
				'border-top-right-radius'    => divino_responsive_spacing( $sub_menu_border_radius_fields, 'right', 'desktop' ),
				'border-bottom-right-radius' => divino_responsive_spacing( $sub_menu_border_radius_fields, 'bottom', 'desktop' ),
				'border-bottom-left-radius'  => divino_responsive_spacing( $sub_menu_border_radius_fields, 'left', 'desktop' ),
			),
			...
			$selector . ' .menu-item.menu-item-has-children > .ast-menu-toggle' => array(
				'top'   => divino_responsive_spacing( $menu_spacing, 'top', 'desktop' ),
		...
		$css_output_desktop[ $selector . ' .main-header-menu, ' . $selector . ' .main-header-menu .sub-menu' ] = divino_get_responsive_background_obj( $menu_resp_bg_color, 'desktop' );
		...
		$css_output_tablet = array(
			...
			// Sub Menu.
			$selector . ' .sub-menu, ' . $selector . ' .inline-on-mobile .sub-menu' => array(
				'border-top-left-radius'     => divino_responsive_spacing( $sub_menu_border_radius_fields, 'top', 'tablet' ),
				'border-top-right-radius'    => divino_responsive_spacing( $sub_menu_border_radius_fields, 'right', 'tablet' ),
				'border-bottom-right-radius' => divino_responsive_spacing( $sub_menu_border_radius_fields, 'bottom', 'tablet' ),
				'border-bottom-left-radius'  => divino_responsive_spacing( $sub_menu_border_radius_fields, 'left', 'tablet' ),
			),
		...
		$css_output_tablet[ $selector . ' .main-header-menu, ' . $selector . ' .main-header-menu .sub-menu' ] = divino_get_responsive_background_obj( $menu_resp_bg_color, 'tablet' );
		...
		$css_output_mobile = array(
			...
			// Sub Menu.
			$selector . ' .sub-menu, ' . $selector . ' .inline-on-mobile .sub-menu' => array(
				'border-top-left-radius'     => divino_responsive_spacing( $sub_menu_border_radius_fields, 'top', 'mobile' ),
				'border-top-right-radius'    => divino_responsive_spacing( $sub_menu_border_radius_fields, 'right', 'mobile' ),
				'border-bottom-right-radius' => divino_responsive_spacing( $sub_menu_border_radius_fields, 'bottom', 'mobile' ),
				'border-bottom-left-radius'  => divino_responsive_spacing( $sub_menu_border_radius_fields, 'left', 'mobile' ),
			),
			$selector . ' .main-header-menu .menu-item > .menu-link' => array(
		...
		$css_output_mobile[ $selector . ' .main-header-menu, ' . $selector . ' .main-header-menu .sub-menu' ] = divino_get_responsive_background_obj( $menu_resp_bg_color, 'mobile' );

		// Submenu border radius inner items fix.
		$sub_menu_border = strval( $sub_menu_border_top );
		$devices         = array( 'desktop', 'tablet', 'mobile' );
		foreach ( $devices as $device ) {
			$border_top_left_radius     = divino_responsive_spacing( $sub_menu_border_radius_fields, 'top', $device );
			$border_top_right_radius    = divino_responsive_spacing( $sub_menu_border_radius_fields, 'right', $device );
			$border_bottom_right_radius = divino_responsive_spacing( $sub_menu_border_radius_fields, 'bottom', $device );
			$border_bottom_left_radius  = divino_responsive_spacing( $sub_menu_border_radius_fields, 'left', $device );
			...
			$css_output_var[ $selector . ' .sub-menu .menu-item:first-of-type > .menu-link, ' . $selector . ' .inline-on-mobile .sub-menu .menu-item:first-of-type > .menu-link' ] = array(
				'border-top-left-radius'  => $border_top_left_radius ? 'calc(' . $border_top_left_radius . ' - ' . divino_get_css_value( $sub_menu_border, 'px' ) . ')' : '',
				'border-top-right-radius' => $border_top_right_radius ? 'calc(' . $border_top_right_radius . ' - ' . divino_get_css_value( $sub_menu_border, 'px' ) . ')' : '',
			);
			...
			$css_output_var[ $selector . ' .sub-menu .menu-item:last-of-type > .menu-link, ' . $selector . ' .inline-on-mobile .sub-menu .menu-item:last-of-type > .menu-link' ] = array(
				'border-bottom-right-radius' => $border_bottom_right_radius ? 'calc(' . $border_bottom_right_radius . ' - ' . divino_get_css_value( $sub_menu_border, 'px' ) . ')' : '',
				'border-bottom-left-radius'  => $border_bottom_left_radius ? 'calc(' . $border_bottom_left_radius . ' - ' . divino_get_css_value( $sub_menu_border, 'px' ) . ')' : '',
			);
		}
	...
	$dynamic_css .= divino_menu_hover_style_css();
	return $dynamic_css;
}
 ...
 * @since 3.5.0
 */
function divino_menu_hover_style_css() {
	$hover_style_flg = false;
	$menu_hover_css  = '';
	for ( $index = 1; $index <= divino_Builder_Helper::$num_of_header_menu; $index++ ) {
		if ( '' !== divino_get_option( 'header-menu' . $index . '-menu-hover-animation' ) ) {
	...
	return divino_Enqueue_Scripts::trim_css( $menu_hover_css );
}
