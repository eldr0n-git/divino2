 ...
 * Off Canvas Row.
 */
add_filter( 'divino_dynamic_theme_css', 'divino_off_canvas_row_setting', 11 );
 ...
 * @since 3.0.0
 */
function divino_off_canvas_row_setting( $dynamic_css, $dynamic_css_filtered = '' ) {

	$selector = '.ast-mobile-popup-drawer.active';
	if ( ! divino_Builder_Helper::is_component_loaded( 'mobile-trigger', 'header' ) && ! is_customize_preview() ) {
		return $dynamic_css;
	}

	$off_canvas_background       = divino_get_option( 'off-canvas-background' );
	$off_canvas_close_color      = divino_get_option( 'off-canvas-close-color' );
	$offcanvas_content_alignment = divino_get_option( 'header-offcanvas-content-alignment', 'flex-start' );
	$padding                     = divino_get_option( 'off-canvas-padding' );
	$menu_content_alignment      = 'center';
	$inner_spacing               = divino_get_option( 'off-canvas-inner-spacing' );
	$mobile_header_type          = divino_get_option( 'mobile-header-type' );
	$move_body                   = divino_get_option( 'off-canvas-move-body' );
	$is_site_rtl                 = is_rtl();
	...
	if ( 'off-canvas' === $mobile_header_type || 'full-width' === $mobile_header_type || is_customize_preview() ) {
		$dynamic_css .= divino_off_canvas_static_css();
	}
	if ( 'dropdown' === $mobile_header_type || is_customize_preview() ) {
		$dynamic_css .= divino_dropdown_type_static_css();
	}
	...
	$css_output = array(
		...
		$selector . ' .ast-mobile-popup-inner' => divino_get_background_obj( $off_canvas_background ),

		'.ast-mobile-header-wrap .ast-mobile-header-content, .ast-desktop-header-content' => divino_get_background_obj( $off_canvas_background ),
		...
		'.ast-mobile-popup-content > *, .ast-mobile-header-content > *, .ast-desktop-popup-content > *, .ast-desktop-header-content > *' => array(
			'padding-top'    => divino_get_css_value( $inner_spacing, 'px' ),
			'padding-bottom' => divino_get_css_value( $inner_spacing, 'px' ),
		),
 ...
 * @return string.
 */
function divino_off_canvas_static_css() {
	$off_canvas_css = '
	...
	return divino_Enqueue_Scripts::trim_css( $off_canvas_css );
}
 ...
 * @return string.
 */
function divino_dropdown_type_static_css() {
	$dropdown_type_css = '
	...
	return divino_Enqueue_Scripts::trim_css( $dropdown_type_css );
}
