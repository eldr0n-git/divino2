 ...
 * Heading Colors
 */
add_filter( 'divino_dynamic_theme_css', 'divino_primary_header_breakpoint_style', 11 );
 ...
 * @since 3.0.0
 */
function divino_primary_header_breakpoint_style( $dynamic_css, $dynamic_css_filtered = '' ) {

	if ( ! is_customize_preview() && ( ! divino_Builder_Helper::is_row_empty( 'primary', 'header', 'desktop' ) && ! divino_Builder_Helper::is_row_empty( 'primary', 'header', 'mobile' ) ) ) {
		return $dynamic_css;
	}
	...
	$hb_header_height = divino_get_option( 'hb-header-height' );
	...
	$common_css_output = array(
		'.ast-mobile-header-wrap .ast-primary-header-bar, .ast-primary-header-bar .site-primary-header-wrap' => array(
			'min-height' => divino_get_css_value( $hb_header_height_desktop, 'px' ),
		),
		'.ast-desktop .ast-primary-header-bar .main-header-menu > .menu-item' => array(
			'line-height' => divino_get_css_value( $hb_header_height_desktop, 'px' ),
		),
	);

	$parse_css .= divino_parse_css( $common_css_output );
	...
	if ( divino_Builder_Helper::is_component_loaded( 'woo-cart', 'header' ) || divino_Builder_Helper::is_component_loaded( 'edd-cart', 'header' ) ) {
		...
		$parse_css .= divino_parse_css( $common_css_cart_output );
	}

	$divino_header_width         = divino_get_option( 'hb-header-main-layout-width' );
	$header_breadcrumb_position = divino_get_option( 'breadcrumb-position' );
	...
	if ( 'content' !== $divino_header_width ) {
		...
		$parse_css .= divino_parse_css( $general_global_responsive );

	} elseif ( 'divino_header_primary_container_after' === $header_breadcrumb_position ) {
	...
	$parse_css .= divino_parse_css( $padding_below_breakpoint );

	// Header Separator.
	$header_separator = divino_get_option( 'hb-header-main-sep' );

	// Apply border only when it has positive value.
	if ( '' !== $header_separator && 'inherit' !== $header_separator ) {
		$header_separator_color = divino_get_option( 'hb-header-main-sep-color' );
	...
	$parse_css .= divino_parse_css( $border_responsive_style );
	$parse_css .= divino_parse_css( $border_desktop_style, divino_get_tablet_breakpoint( '', 1 ) );

	// Header background colours.
	$header_bg_obj     = divino_get_option( 'hb-header-bg-obj-responsive' );
	$hba_header_bg_obj = divino_get_option( 'hba-header-bg-obj-responsive' );
	$hbb_header_bg_obj = divino_get_option( 'hbb-header-bg-obj-responsive' );
	...
	if ( is_customize_preview() ) {
		...
		$header_main_color = array(
			'.ast-sg-element-wrap.ast-sg-logo-section' => divino_get_responsive_background_obj( $header_bg_obj, 'desktop' ),
		);
		...
		foreach ( $sections as $section => $bg_obj ) {
			if ( isset( $header_items[ $section ] ) && divino_is_logo_in_section( $header_items[ $section ] ) ) {
				$header_main_color = array(
					'.ast-sg-element-wrap.ast-sg-logo-section' => divino_get_responsive_background_obj( $bg_obj, 'desktop' ),
				);
				break;
			}
		}
		...
		// Parse the CSS
		$parse_css .= divino_parse_css( $header_main_color );
	}
	...
	$desktop_colors = array(
		'.ast-primary-header-bar' => divino_get_responsive_background_obj( $header_bg_obj, 'desktop' ),
	);

	$tablet_colors = array(
		'.ast-primary-header-bar.ast-primary-header' => divino_get_responsive_background_obj( $header_bg_obj, 'tablet' ),
		'.ast-mobile-header-wrap .ast-primary-header-bar, .ast-primary-header-bar .site-primary-header-wrap' => array(
			'min-height' => divino_get_css_value( $hb_header_height_tablet, 'px' ),
		),
	);
	$mobile_colors = array(
		'.ast-primary-header-bar.ast-primary-header' => divino_get_responsive_background_obj( $header_bg_obj, 'mobile' ),
		'.ast-mobile-header-wrap .ast-primary-header-bar , .ast-primary-header-bar .site-primary-header-wrap' => array(
			'min-height' => divino_get_css_value( $hb_header_height_mobile, 'px' ),
		),
	);
	 ...
	 */
	$parse_css .= apply_filters( 'divino_addon_colors_dynamic_css_desktop', divino_parse_css( $desktop_colors ) );
	$parse_css .= apply_filters( 'divino_addon_colors_dynamic_css_tablet', divino_parse_css( $tablet_colors, '', divino_get_tablet_breakpoint() ) );
	...
	$dynamic_css .= divino_Enqueue_Scripts::trim_css( $parse_css );
	...
	$dynamic_css .= divino_Extended_Base_Dynamic_CSS::prepare_advanced_margin_padding_css( $_section, $parent_selector );

	$dynamic_css .= divino_Builder_Base_Dynamic_CSS::prepare_visibility_css( $_section, '.ast-primary-header-bar', 'block', 'grid' );

	// Advanced CSS for Header Builder.
	$margin = divino_get_option( 'section-header-builder-layout-margin' );
