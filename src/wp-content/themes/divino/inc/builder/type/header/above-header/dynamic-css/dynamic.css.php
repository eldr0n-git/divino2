 ...
 * Above Header Row.
 */
add_filter( 'divino_dynamic_theme_css', 'divino_above_header_row_setting', 11 );
 ...
 * @since 3.0.0
 */
function divino_above_header_row_setting( $dynamic_css, $dynamic_css_filtered = '' ) {

	if ( ! is_customize_preview() && ( ! divino_Builder_Helper::is_row_empty( 'above', 'header', 'desktop' ) && ! divino_Builder_Helper::is_row_empty( 'above', 'header', 'mobile' ) ) ) {
		return $dynamic_css;
	}
	...
	// Common CSS options.
	$hba_header_height  = divino_get_option( 'hba-header-height' );
	$hba_header_divider = divino_get_option( 'hba-header-separator' );
	$hba_border_color   = divino_get_option( 'hba-header-bottom-border-color' );
	...
	$common_css_output = array(
		...
		'.ast-mobile-header-wrap .ast-above-header-bar , .ast-above-header-bar .site-above-header-wrap' => array(
			'min-height' => divino_get_css_value( $hba_header_height_desktop, 'px' ),
		),
		'.ast-desktop .ast-above-header-bar .main-header-menu > .menu-item' => array(
			'line-height' => divino_get_css_value( $hba_header_height_desktop, 'px' ),
		),
	...
	if ( '' !== $hba_header_divider && 'inherit' !== $hba_header_divider ) {
		$common_css_output['.ast-above-header-bar'] = array(
			'border-bottom-width' => divino_get_css_value( $hba_header_divider, 'px' ),
	...
	$desktop_bg = array(
		'.ast-above-header.ast-above-header-bar'        => divino_get_responsive_background_obj( $hba_header_bg_obj, 'desktop' ),
		'.ast-header-break-point .ast-above-header-bar' => array(
			'background-color' => esc_attr( $desktop_background ),
		),
	);
	$parse_css .= divino_parse_css( $desktop_bg );

	// Above Header Background Responsive - Tablet.
	$tablet_bg  = array(
		'.ast-above-header.ast-above-header-bar'        => divino_get_responsive_background_obj( $hba_header_bg_obj, 'tablet' ),
		'.ast-header-break-point .ast-above-header-bar' => array(
			'background-color' => esc_attr( $tablet_background ),
		),
		'.ast-mobile-header-wrap .ast-above-header-bar , .ast-above-header-bar .site-above-header-wrap' => array(
			'min-height' => divino_get_css_value( $hba_header_height_tablet, 'px' ),
		),
	...
	$parse_css .= divino_parse_css( $tablet_bg, '', divino_get_tablet_breakpoint() );

	// Above Header Background Responsive - Mobile.
	$mobile_bg  = array(
		'.ast-above-header.ast-above-header-bar'        => divino_get_responsive_background_obj( $hba_header_bg_obj, 'mobile' ),
		'.ast-header-break-point .ast-above-header-bar' => array(
			'background-color' => esc_attr( $mobile_background ),
		),
		'.ast-mobile-header-wrap .ast-above-header-bar , .ast-above-header-bar .site-above-header-wrap' => array(
			'min-height' => divino_get_css_value( $hba_header_height_mobile, 'px' ),
		),
	);
	$parse_css .= divino_parse_css( $mobile_bg, '', divino_get_mobile_breakpoint() );
	...
	$dynamic_css .= divino_Builder_Base_Dynamic_CSS::prepare_visibility_css( $_section, '.ast-above-header-bar', 'block', $mobile_tablet_default_display = 'grid' );
