...
if ( ! class_exists( 'divino_Builder_Base_Dynamic_CSS' ) ) {

	/**
	 * Class divino_Builder_Base_Dynamic_CSS.
	 */
	final class divino_Builder_Base_Dynamic_CSS {
		/**
		 * Member Variable
		 *
		 * @var mixed instance
		...
		public function __construct() {
			...
			add_filter( 'divino_dynamic_theme_css', array( $this, 'footer_dynamic_css' ) );
			add_filter( 'divino_dynamic_theme_css', array( $this, 'mobile_header_logo_css' ) );
		}
		...
		public static function prepare_advanced_typography_css( $section_id, $selector ) {
			...
			$font_size = divino_get_option( 'font-size-' . $section_id );
		...
		public static function mobile_header_logo_css( $dynamic_css ) {
			...
			if ( '' !== $mobile_header_logo && '1' == $different_mobile_header_order ) {
				...
				$dynamic_css .= divino_Enqueue_Scripts::trim_css( $mobile_header_css );
			}
			return $dynamic_css;
		}
		...
		public static function prepare_visibility_css( $section_id, $selector, $default_property = 'flex', $mobile_tablet_default = '' ) {
			...
			$parent_visibility = divino_get_option(
				$section_id . '-visibility-responsive',
				array(
					'desktop' => ! isset( $divino_options[ $section_id . '-visibility-responsive' ] ) && isset( $divino_options[ $section_id . '-hide-desktop' ] ) ? ( $divino_options[ $section_id . '-hide-desktop' ] ? 0 : 1 ) : 1,
					'tablet'  => ! isset( $divino_options[ $section_id . '-visibility-responsive' ] ) && isset( $divino_options[ $section_id . '-hide-tablet' ] ) ? ( $divino_options[ $section_id . '-hide-tablet' ] ? 0 : 1 ) : 1,
					'mobile'  => ! isset( $divino_options[ $section_id . '-visibility-responsive' ] ) && isset( $divino_options[ $section_id . '-hide-mobile' ] ) ? ( $divino_options[ $section_id . '-hide-mobile' ] ? 0 : 1 ) : 1,
				)
			);
	...
	/**
	 *  Prepare if class 'divino_Builder_Base_Dynamic_CSS' exist.
	 *  Kicking this off by calling 'get_instance()' method
	 */
	divino_Builder_Base_Dynamic_CSS::get_instance();
}
