<?php
/**
 * divino Social Component Dynamic CSS.
 *
 ...
 * @since 3.0.0
 */
class divino_Social_Component_Dynamic_CSS {
	/**
	 * Dynamic CSS
	 *
	 ...
	 * @since 3.0.0
	 */
	public static function divino_social_dynamic_css( $builder_type = 'header' ) {

		$generated_css   = '';
		$social_css_flag = false;

		$number_of_social_icons = 'header' === $builder_type ? divino_Builder_Helper::$num_of_header_social_icons : divino_Builder_Helper::$num_of_footer_social_icons;

		for ( $index = 1; $index <= $number_of_social_icons; $index++ ) {

			if ( ! divino_Builder_Helper::is_component_loaded( 'social-icons-' . $index, $builder_type ) ) {
				continue;
			}
			...
			$icon_spacing       = divino_get_option( $builder_type . '-social-' . $index . '-space' );
			$icon_bg_spacing    = divino_get_option( $builder_type . '-social-' . $index . '-bg-space' );
			$icon_size          = divino_get_option( $builder_type . '-social-' . $index . '-size' );
			$icon_radius_fields = divino_get_option( $builder_type . '-social-' . $index . '-radius-fields' );
			...
			$color_type                       = divino_get_option( $builder_type . '-social-' . $index . '-color-type' );
			$toggle_brand_hover               = divino_get_option( $builder_type . '-social-' . $index . '-brand-hover-toggle' );
			$social_icons_brand_color_desktop = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-brand-color' ), 'desktop' );
			$social_icons_brand_color_tablet  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-brand-color' ), 'tablet' );
			$social_icons_brand_color_mobile  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-brand-color' ), 'mobile' );

			$social_label_brand_color_desktop      = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-brand-label-color' ), 'desktop' );
			$social_label_icons_brand_color_tablet = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-brand-label-color' ), 'tablet' );
			$social_label_icons_brand_color_mobile = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-brand-label-color' ), 'mobile' );

			$social_icons_color_desktop = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-color' ), 'desktop' );
			$social_icons_color_tablet  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-color' ), 'tablet' );
			$social_icons_color_mobile  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-color' ), 'mobile' );

			// Hover Responsive Colors.
			$social_icons_h_color_desktop = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-h-color' ), 'desktop' );
			$social_icons_h_color_tablet  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-h-color' ), 'tablet' );
			$social_icons_h_color_mobile  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-h-color' ), 'mobile' );

			// Normal Responsive Bg Colors.
			$social_icons_bg_color_desktop = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-bg-color' ), 'desktop' );
			$social_icons_bg_color_tablet  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-bg-color' ), 'tablet' );
			$social_icons_bg_color_mobile  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-bg-color' ), 'mobile' );

			// Hover Responsive Bg Colors.
			$social_icons_h_bg_color_desktop = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-bg-h-color' ), 'desktop' );
			$social_icons_h_bg_color_tablet  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-bg-h-color' ), 'tablet' );
			$social_icons_h_bg_color_mobile  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-bg-h-color' ), 'mobile' );

			// Normal Responsive Label Colors.
			$social_icons_label_color_desktop = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-label-color' ), 'desktop' );
			$social_icons_label_color_tablet  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-label-color' ), 'tablet' );
			$social_icons_label_color_mobile  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-label-color' ), 'mobile' );

			// Hover Responsive Label Colors.
			$social_icons_label_h_color_desktop = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-label-h-color' ), 'desktop' );
			$social_icons_label_h_color_tablet  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-label-h-color' ), 'tablet' );
			$social_icons_label_h_color_mobile  = divino_get_prop( divino_get_option( $builder_type . '-social-' . $index . '-label-h-color' ), 'mobile' );
			...
			$css_output_desktop = array(
				$selector . ' .ast-builder-social-element,' . $selector . ' .social-show-label-true .ast-builder-social-element' => array(
					// Icon Spacing.
					'margin-left'                => divino_get_css_value( $icon_spacing_desktop, 'px' ),
					'margin-right'               => divino_get_css_value( $icon_spacing_desktop, 'px' ),

					// Icon Background Space.
					'padding'                    => divino_get_css_value( $icon_bg_spacing, 'px' ),

					// Icon Radius.
					'border-top-left-radius'     => divino_responsive_spacing( $icon_radius_fields, 'top', 'desktop' ),
					'border-top-right-radius'    => divino_responsive_spacing( $icon_radius_fields, 'right', 'desktop' ),
					'border-bottom-right-radius' => divino_responsive_spacing( $icon_radius_fields, 'bottom', 'desktop' ),
					'border-bottom-left-radius'  => divino_responsive_spacing( $icon_radius_fields, 'left', 'desktop' ),
				),
				$selector . ' .ast-builder-social-element svg' => array(

					// Icon Size.
					'width'  => divino_get_css_value( $icon_size_desktop, 'px' ),
					'height' => divino_get_css_value( $icon_size_desktop, 'px' ),
				),
				$selector . ' .ast-social-icon-image-wrap' => array(

					// Icon Background Space.
					'margin' => divino_get_css_value( $icon_bg_spacing, 'px' ),
				),
				$selector                                  => array(
					// Margin CSS.
			...
			$css_output_tablet = array(
				$selector . ' .ast-builder-social-element svg' => array(
					...
					// Icon Size.
					'width'  => divino_get_css_value( $icon_size_tablet, 'px' ),
					'height' => divino_get_css_value( $icon_size_tablet, 'px' ),
				),
				$selector . ' .ast-builder-social-element' => array(
					// Icon Spacing.
					'margin-left'                => divino_get_css_value( $icon_spacing_tablet, 'px' ),
					'margin-right'               => divino_get_css_value( $icon_spacing_tablet, 'px' ),

					// Border Radius.
					'border-top-left-radius'     => divino_responsive_spacing( $icon_radius_fields, 'top', 'tablet' ),
					'border-top-right-radius'    => divino_responsive_spacing( $icon_radius_fields, 'right', 'tablet' ),
					'border-bottom-right-radius' => divino_responsive_spacing( $icon_radius_fields, 'bottom', 'tablet' ),
					'border-bottom-left-radius'  => divino_responsive_spacing( $icon_radius_fields, 'left', 'tablet' ),
				),
				$selector                                  => array(
					// Margin CSS.
			...
			$css_output_mobile = array(
				$selector . ' .ast-builder-social-element svg' => array(
					...
					// Icon Size.
					'width'  => divino_get_css_value( $icon_size_mobile, 'px' ),
					'height' => divino_get_css_value( $icon_size_mobile, 'px' ),
				),
				$selector . ' .ast-builder-social-element' => array(
					// Icon Spacing.
					'margin-left'                => divino_get_css_value( $icon_spacing_mobile, 'px' ),
					'margin-right'               => divino_get_css_value( $icon_spacing_mobile, 'px' ),

					// Border Radius.
					'border-top-left-radius'     => divino_responsive_spacing( $icon_radius_fields, 'top', 'mobile' ),
					'border-top-right-radius'    => divino_responsive_spacing( $icon_radius_fields, 'right', 'mobile' ),
					'border-bottom-right-radius' => divino_responsive_spacing( $icon_radius_fields, 'bottom', 'mobile' ),
					'border-bottom-left-radius'  => divino_responsive_spacing( $icon_radius_fields, 'left', 'mobile' ),
				),
				$selector                                  => array(
					// Margin CSS.
			...
			if ( 'footer' === $builder_type ) {
				...
				$alignment = divino_get_option( 'footer-social-' . $index . '-alignment' );
			...
			$css_output .= divino_Builder_Base_Dynamic_CSS::prepare_advanced_typography_css( $_section, $selector );

			$visibility_selector = '.ast-builder-layout-element[data-section="' . $_section . '"]';
			$css_output         .= divino_Builder_Base_Dynamic_CSS::prepare_visibility_css( $_section, $visibility_selector );
	...
	public static function get_social_static_css() {
		...
		return divino_Enqueue_Scripts::trim_css( $social_static_css );
	}
}
...
new divino_Social_Component_Dynamic_CSS();
