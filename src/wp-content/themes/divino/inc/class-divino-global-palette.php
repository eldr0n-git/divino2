...
class divino_Global_Palette {
	...
	public function __construct() {
		...
		if ( ( get_template_directory() !== $get_stylesheet && false === file_exists( $is_theme_json ) ) || divino_wp_version_compare( '5.8', '<' ) ) {
			add_action( 'after_setup_theme', array( $this, 'support_editor_color_palette' ) );
		}
		add_filter( 'divino_theme_customizer_js_localize', array( $this, 'localize_variables' ) );
		add_filter( 'divino_before_foreground_color_generation', array( $this, 'get_color_by_palette_variable' ) );
		$this->includes();
		add_filter( 'divino_dynamic_theme_css', array( $this, 'global_border_compatibility' ) );
	}
	...
	public function localize_variables( $object ) {
		...
		if ( isset( $object['customizer'] ) ) {
			...
			$object['customizer']['isElementorActive']              = divino_is_elemetor_active();
			$object['customizer']['isGlobalColorElementorDisabled'] = divino_maybe_disable_global_color_in_elementor();
	...
	public static function get_default_color_palette() {
		...
		$update_color_styles_with_presets  = divino_Dynamic_CSS::divino_4_5_0_compatibility();
	 ...
	 */
	public static function divino_get_active_global_palette() {
		$active_global_palette_key = '';
		// Get the current palette option from global color palette.
		$divino_options = divino_get_options();
		if ( isset( $divino_options['global-color-palette']['palette'] ) ) {
			$current_palette = $divino_options['global-color-palette']['palette'];
		 ...
		 * @since 4.10.0
		 */
		return apply_filters( 'divino_get_active_global_palette_key', $active_global_palette_key );
	}
	...
	public static function is_dark_palette( $palette_key = 'current' ) {
		if ( $palette_key === 'current' ) {
			if ( ! self::$is_dark_palette ) {
				...
				self::$is_dark_palette = self::divino_get_active_global_palette() === 'palette_4';
			}
	...
	public static function get_palette_labels() {
		$divino_settings           = divino_Theme_Options::get_options();
		$color_palette_reorganize = divino_Dynamic_CSS::divino_4_8_9_compatibility();
	...
	public function includes() {
		require_once divino_THEME_DIR . 'inc/dynamic-css/global-color-palette.php';// PHPCS:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
	}
	...
	public static function generate_global_palette_style() {
		$palette_data = divino_get_option( 'global-color-palette' );
	...
	public function global_border_compatibility( $dynamic_css ) {
		$global_border_color = divino_get_option( 'border-color', '#dddddd' );
		...
		return $dynamic_css . divino_Enqueue_Scripts::trim_css( $global_border );
	}
}

new divino_Global_Palette();
