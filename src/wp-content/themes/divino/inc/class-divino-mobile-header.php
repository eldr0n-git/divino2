...
if ( ! class_exists( 'divino_Mobile_Header' ) ) {

	/**
	 * divino_Mobile_Header
	 *
	 * @since 1.4.0
	 */
	class divino_Mobile_Header {
		/**
		 * Instance
		 *
		 * @since 1.4.0
		...
		public function __construct() {
			add_action( 'divino_header', array( $this, 'mobile_header_markup' ), 5 );
			add_action( 'body_class', array( $this, 'add_body_class' ) );
			add_filter( 'divino_main_menu_toggle_classes', array( $this, 'menu_toggle_classes' ) );
			add_filter( 'walker_nav_menu_start_el', array( $this, 'toggle_button' ), 20, 4 );
			add_filter( 'divino_walker_nav_menu_start_el', array( $this, 'toggle_button' ), 10, 4 );
		}
		...
		public function toggle_button( $item_output, $item, $depth, $args ) {
			...
			for ( $index = 3; $index <= divino_Builder_Helper::$component_limit; $index++ ) {
		...
		public function menu_arrow_button_markup( $item_output, $item ) {
			$item_output  = apply_filters( 'divino_toggle_button_markup', $item_output, $item );
			$item_output .= '<button ' . divino_attr(
				'ast-menu-toggle',
				array(
					'aria-expanded' => 'false',
				),
		...
		public function menu_toggle_classes( $classes ) {
			return ' ast-mobile-menu-buttons-' . divino_get_option( 'mobile-header-toggle-btn-style' ) . ' ';
		}
		...
		public function mobile_header_markup() {
			...
			$different_logo     = divino_get_option( 'different-mobile-logo' );

			if ( '' !== $mobile_header_logo && '1' == $different_logo ) {
				add_filter( 'divino_has_custom_logo', '__return_true' );
				add_filter( 'get_custom_logo', array( $this, 'divino_mobile_header_custom_logo' ), 10, 2 );
				add_filter( 'divino_is_logo_attachment', array( $this, 'add_mobile_logo_svg_class' ), 10, 2 );
			}
		}
		 ...
		 */
		public function divino_mobile_header_custom_logo( $html, $blog_id ) {
		...
		public function add_body_class( $classes ) {
			 ...
			 */
			$header_content_layout = divino_get_option( 'different-mobile-logo' );
	 ...
	 */
	divino_Mobile_Header::get_instance();
