 ...
 * @since 4.0.0
 */
class divino_Posts_Structure_Loader {
	/**
	 * Instance
	 *
	...
	public function add_fonts() {
		...
		foreach ( $post_types as $post_type ) {
			...
			$single_title_font_family = divino_get_option( $title_section . '-title-font-family' );
			$single_title_font_weight = divino_get_option( $title_section . '-title-font-weight' );
			divino_Fonts::add_font( $single_title_font_family, $single_title_font_weight );

			$single_text_font_family = divino_get_option( $title_section . '-text-font-family' );
			$single_text_font_weight = divino_get_option( $title_section . '-text-font-weight' );
			divino_Fonts::add_font( $single_text_font_family, $single_text_font_weight );

			$single_meta_font_family = divino_get_option( $title_section . '-meta-font-family' );
			$single_meta_font_weight = divino_get_option( $title_section . '-meta-font-weight' );
			divino_Fonts::add_font( $single_meta_font_family, $single_meta_font_weight );

			// Archive Banner - Font Support.
			$title_section = 'ast-dynamic-archive-' . $post_type;

			$archive_text_font_family = divino_get_option( $title_section . '-text-font-family' );
			$archive_text_font_weight = divino_get_option( $title_section . '-text-font-weight' );
			divino_Fonts::add_font( $archive_text_font_family, $archive_text_font_weight );

			$archive_title_font_family = divino_get_option( $title_section . '-title-font-family' );
			$archive_title_font_weight = divino_get_option( $title_section . '-title-font-weight' );
			divino_Fonts::add_font( $archive_title_font_family, $archive_title_font_weight );
		}

		foreach ( self::get_special_page_types() as $special_type ) {
			$title_section = 'section-' . $special_type . '-page-title';

			$instance_text_font_family = divino_get_option( $title_section . '-text-font-family' );
			$instance_text_font_weight = divino_get_option( $title_section . '-text-font-weight' );
			divino_Fonts::add_font( $instance_text_font_family, $instance_text_font_weight );

			$instance_title_font_family = divino_get_option( $title_section . '-title-font-family' );
			$instance_title_font_weight = divino_get_option( $title_section . '-title-font-weight' );
			divino_Fonts::add_font( $instance_title_font_family, $instance_title_font_weight );
		}
	}
	...
	public static function get_supported_post_types() {
		if ( empty( self::$supported_post_types ) || is_customize_preview() ) {
			self::$supported_post_types = divino_get_queried_post_types();
		...
		return apply_filters( 'divino_dynamic_post_structure_posttypes', self::$supported_post_types );
	}
	...
	public static function get_special_page_types() {
		...
		return apply_filters( 'divino_dynamic_special_pages', $special_pages );
	}
	...
	public function preview_scripts() {
		...
		wp_localize_script(
			...
			array(
				...
				'enabled_related_post' => divino_get_option( 'enable-related-posts', false ),
			)
		);
	}
 ...
 */
new divino_Posts_Structure_Loader();
