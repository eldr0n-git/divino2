 ...
 * Post Structures
 */
add_filter( 'divino_dynamic_theme_css', 'divino_special_archive_dynamic_css' );
 ...
 * @since 4.6.0
 */
function divino_special_archive_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {
	...
	foreach ( divino_Posts_Structure_Loader::get_special_page_types() as $special_type ) {
		...
		$layout_type     = divino_get_option( $title_section . '-layout', 'layout-1' );
		...
		$horizontal_alignment = divino_get_option( $title_section . '-horizontal-alignment' );
		...
		$elements_gap   = divino_get_option( $title_section . '-elements-gap', 10 );
		$banner_padding = divino_get_option( $title_section . '-banner-padding', divino_Posts_Structure_Loader::get_customizer_default( 'responsive-padding' ) );
		$banner_margin  = divino_get_option( $title_section . '-banner-margin' );

		$banner_height      = divino_get_option( $title_section . '-banner-height' );
		...
		$text_color       = divino_get_option( $title_section . '-banner-text-color' );
		$title_color      = divino_get_option( $title_section . '-banner-title-color' );
		$link_color       = divino_get_option( $title_section . '-banner-link-color' );
		$link_hover_color = divino_get_option( $title_section . '-banner-link-hover-color' );

		$vert_alignment  = $layout_2_active ? divino_get_option( $title_section . '-vertical-alignment', 'center' ) : 'center';
		$width_type      = divino_get_option( $title_section . '-banner-width-type', 'fullwidth' );
		$custom_width    = divino_get_option( $title_section . '-banner-custom-width', 1200 );
		$background_type = divino_get_option( $title_section . '-banner-image-type', 'none' );

		// Banner Text typography dynamic stylings.
		$banner_text_font_size = divino_get_option( $title_section . '-text-font-size' );

		// Banner Title typography dynamic stylings.
		$banner_title_font_size = divino_get_option( $title_section . '-title-font-size', divino_Posts_Structure_Loader::get_customizer_default( 'title-font-size' ) );
		...
		if ( 'layout-1' === $layout_type ) {
			...
			$css_output_desktop = array(
				...
				$selector . ' *'                          => divino_get_font_array_css( divino_get_option( $title_section . '-text-font-family' ), divino_get_option( $title_section . '-text-font-weight' ), $banner_text_font_size, $title_section . '-text-font-extras', $text_color ),
				$selector . ' h1, ' . $selector . ' h1 *' => divino_get_font_array_css( divino_get_option( $title_section . '-title-font-family' ), divino_get_option( $title_section . '-title-font-weight', divino_Posts_Structure_Loader::get_customizer_default( 'title-font-weight' ) ), $banner_title_font_size, $title_section . '-title-font-extras', $title_color ),
			...
			if ( 'none' !== $background_type ) {
				$custom_background = divino_get_option( $title_section . '-banner-custom-bg' );
				$css_output_desktop['.search section.ast-archive-description'] = divino_get_responsive_background_obj( $custom_background, 'desktop' );
				$css_output_tablet['.search section.ast-archive-description']  = divino_get_responsive_background_obj( $custom_background, 'tablet' );
				$css_output_mobile['.search section.ast-archive-description']  = divino_get_responsive_background_obj( $custom_background, 'mobile' );
			}
		} else {
			/**
			 * Desktop CSS.
			...
			$css_output_desktop = array(
				...
				$selector . ' .ast-container *'  => divino_get_font_array_css( divino_get_option( $title_section . '-text-font-family' ), divino_get_option( $title_section . '-text-font-weight' ), $banner_text_font_size, $title_section . '-text-font-extras', $text_color ),
				$selector . ' .ast-container h1, ' . $selector . ' .ast-container h1 *' => divino_get_font_array_css( divino_get_option( $title_section . '-title-font-family' ), divino_get_option( $title_section . '-title-font-weight', divino_Posts_Structure_Loader::get_customizer_default( 'title-font-weight' ) ), $banner_title_font_size, $title_section . '-title-font-extras', $title_color ),
