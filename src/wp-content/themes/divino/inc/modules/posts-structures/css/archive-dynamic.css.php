 ...
 * Post Structures
 */
add_filter( 'divino_dynamic_theme_css', 'divino_post_archive_structure_dynamic_css' );
 ...
 * @since 4.0.0
 */
function divino_post_archive_structure_dynamic_css( $dynamic_css, $dynamic_css_filtered = '' ) {

	$current_post_type    = divino_get_post_type();
	...
	if ( false === divino_get_option( 'ast-archive-' . $current_post_type . '-title', class_exists( 'WooCommerce' ) && 'product' === $current_post_type ? false : true ) ) {
		return $dynamic_css;
	}
	...
	$layout_type     = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-layout', 'layout-1' );
	...
	$horz_alignment   = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-horizontal-alignment' );
	...
	$elements_gap   = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-elements-gap', 10 );
	$banner_padding = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-padding', class_exists( 'WooCommerce' ) && 'product' === $current_post_type ? divino_Posts_Structure_Loader::get_customizer_default( 'responsive-spacing' ) : divino_Posts_Structure_Loader::get_customizer_default( 'responsive-padding' ) );
	$banner_margin  = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-margin' );

	$banner_height      = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-height' );
	...
	$text_color       = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-text-color' );
	$title_color      = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-title-color' );
	$link_color       = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-link-color' );
	$link_hover_color = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-link-hover-color' );

	$vert_alignment  = $layout_2_active ? divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-vertical-alignment', 'center' ) : 'center';
	$width_type      = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-width-type', 'fullwidth' );
	$custom_width    = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-custom-width', 1200 );
	$background_type = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-image-type', 'none' );

	// Banner Text typography dynamic stylings.
	$banner_text_font_size = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-text-font-size' );

	// Banner Title typography dynamic stylings.
	$banner_title_font_size = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-title-font-size', divino_Posts_Structure_Loader::get_customizer_default( 'title-font-size' ) );
	...
	if ( 'layout-1' === $layout_type ) {
		...
		$css_output_desktop = array(
			...
			$selector . ' *'                        => divino_get_font_array_css( divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-text-font-family' ), divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-text-font-weight' ), $banner_text_font_size, 'ast-dynamic-archive-' . $current_post_type . '-text-font-extras', $text_color ),
			$selector . ' .ast-archive-title, ' . $selector . ' .ast-archive-title *' => divino_get_font_array_css( divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-title-font-family' ), divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-title-font-weight', divino_Posts_Structure_Loader::get_customizer_default( 'title-font-weight' ) ), $banner_title_font_size, 'ast-dynamic-archive-' . $current_post_type . '-title-font-extras', $title_color ),
		...
		if ( 'none' !== $background_type ) {
			if ( class_exists( 'WooCommerce' ) && 'product' === $current_post_type ) {
				if ( 'custom' === $background_type ) {
					$custom_background = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-custom-bg' );
					$css_output_desktop['.archive section.ast-archive-description'] = divino_get_responsive_background_obj( $custom_background, 'desktop' );
					$css_output_tablet['.archive section.ast-archive-description']  = divino_get_responsive_background_obj( $custom_background, 'tablet' );
					$css_output_mobile['.archive section.ast-archive-description']  = divino_get_responsive_background_obj( $custom_background, 'mobile' );
				} else {
					...
					$overlay_color = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-featured-overlay', '' );
		...
		$css_output_desktop = array(
			...
			$selector . ' .ast-container *'              => divino_get_font_array_css( divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-text-font-family' ), divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-text-font-weight' ), $banner_text_font_size, 'ast-dynamic-archive-' . $current_post_type . '-text-font-extras', $text_color ),
			$selector . ' .ast-container h1'             => divino_get_font_array_css( divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-title-font-family' ), divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-title-font-weight', divino_Posts_Structure_Loader::get_customizer_default( 'title-font-weight' ) ), $banner_title_font_size, 'ast-dynamic-archive-' . $current_post_type . '-title-font-extras', $title_color ),
		...
		if ( 'none' !== $background_type ) {
			if ( 'product' !== $current_post_type ) {
				$custom_background = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-custom-bg' );
				$css_output_desktop[ $selector . '[data-banner-background-type="custom"]' ] = divino_get_responsive_background_obj( $custom_background, 'desktop' );
				$css_output_tablet[ $selector . '[data-banner-background-type="custom"]' ]  = divino_get_responsive_background_obj( $custom_background, 'tablet' );
				$css_output_mobile[ $selector . '[data-banner-background-type="custom"]' ]  = divino_get_responsive_background_obj( $custom_background, 'mobile' );
			} else {
				if ( 'custom' === $background_type ) {
					$custom_background = divino_get_option( 'ast-dynamic-archive-' . $current_post_type . '-banner-custom-bg' );
					$css_output_desktop[ $selector . '[data-banner-background-type="custom"]' ] = divino_get_responsive_background_obj( $custom_background, 'desktop' );
