 ...
 * @since 3.5.0
 */
class divino_Related_Posts_Markup {
	/**
	 *  Constructor
	 */
	public function __construct() {
		add_action( 'divino_content_before', array( $this, 'initialize_related_posts' ) );
	}
	...
	public function initialize_related_posts() {
		$priority         = 10;
		$location         = divino_get_option( 'related-posts-outside-location' );
		$module_placement = divino_get_option( 'related-posts-box-placement' );
		if ( 'outside' === $module_placement ) {
			$action = 'divino_content_after';
			if ( divino_get_option( 'enable-comments-area', true ) && 'outside' === divino_get_option( 'comments-box-placement' ) ) {
				$priority = 'below' === $location ? 20 : 9;
			}
		} elseif ( 'inside' === $module_placement ) {
			$action   = 'divino_entry_bottom';
			$priority = 'below' === $location ? 20 : 10;
		} else {
			$action = 'divino_entry_after';
		}
		add_action( $action, array( $this, 'divino_related_posts_markup' ), $priority );
	}
	 ...
	 * @return void
	 */
	public function divino_related_posts_markup() {
		if ( divino_target_rules_for_related_posts() ) {
			$this->divino_get_related_posts();
		}
	}
	...
	public function divino_get_related_posts() {
		...
		$related_posts_total_count = absint( divino_get_option( 'related-posts-total-count', 2 ) );
		$module_container_width    = divino_get_option( 'related-posts-container-width' );
		$module_container_width    = 'inside' === divino_get_option( 'related-posts-box-placement' ) ? '' : 'ast-container--' . $module_container_width;
		$related_category_style    = divino_get_option( 'related-posts-category-style' );
		$related_tag_style         = divino_get_option( 'related-posts-tag-style' );

		// Get related posts by WP_Query.
		$query_posts = $this->divino_get_related_posts_by_query( $post_id );

		if ( $query_posts ) {

			if ( ! $query_posts->have_posts() ) {
				return apply_filters( 'divino_related_posts_no_posts_avilable_message', '', $post_id );
			}
			...
			do_action( 'divino_related_posts_loop_before' );
			...
			while ( $query_posts->have_posts() && $post_counter < $total_posts_count ) {
				...
				$post_id    = get_the_ID();
				$separator  = divino_get_option( 'related-metadata-separator', '/' );
				$output_str = divino_get_post_meta( $related_post_meta, $separator, 'related-posts' );
				...
				if ( is_array( $exclude_ids ) && ! in_array( $post_id, $exclude_ids ) ) {
					...
					if ( false === $related_posts_section_loaded ) {
						...
						do_action( 'divino_related_posts_title_before' );

						if ( '' !== $related_posts_title ) {
							echo apply_filters( // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
								'divino_related_posts_title',
								sprintf(
									'<div class="ast-related-posts-title-section"> <%1$s class="ast-related-posts-title"> %2$s </%1$s> </div>',
									apply_filters( 'divino_related_posts_box_heading_tag', 'h2' ),
									$related_posts_title
								)
							);
						}

						do_action( 'divino_related_posts_title_after' );
							...
							<div class="ast-related-posts-inner-section">
								<div class="ast-related-post-content">
									...
									if ( is_array( $related_post_structure ) ) {
										foreach ( $related_post_structure as $post_thumb_title_order ) {
											if ( 'featured-image' === $post_thumb_title_order ) {
												do_action( 'divino_related_post_before_featured_image', $post_id );
												$this->divino_get_related_post_featured_image( $post_id );
												do_action( 'divino_related_post_after_featured_image', $post_id );
											} else {
												?>
														<header class="entry-header related-entry-header">
														<?php
															$this->divino_get_related_post_title( $post_id );
															echo apply_filters( 'divino_related_posts_meta_html', '<div class="entry-meta ast-related-cat-style--' . $related_category_style . ' ast-related-tag-style--' . $related_tag_style . '">' . $output_str . '</div>' ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
														?>
														</header>
													<?php
											}
									...
									<div class="entry-content clear">
										<?php
											$this->divino_get_related_post_excerpt( $post_id );
											$this->divino_get_related_post_read_more( $post_id );
										?>
									</div>
								</div>
							</div>
			...
			do_action( 'divino_related_posts_loop_after' );
		}
	}
	 ...
	 * @since 3.5.0
	 */
	public function divino_get_related_post_read_more( $current_post_id ) {
		if ( ! divino_get_option( 'enable-related-posts-excerpt' ) ) {
			return;
		}

		$related_posts_content_type = apply_filters( 'divino_related_posts_content_type', 'excerpt' );
		...
		$custom_class = apply_filters( 'divino_related_post_cta_custom_classes', $class );

		do_action( 'divino_related_post_before_cta', $current_post_id );
		...
		do_action( 'divino_related_post_after_cta', $current_post_id );
	}
	 ...
	 * @since 3.5.0
	 */
	public function divino_get_related_post_excerpt( $current_post_id ) {
		if ( ! divino_get_option( 'enable-related-posts-excerpt' ) ) {
			return;
		}
		...
		$excerpt_length = absint( divino_get_option( 'related-posts-excerpt-count' ) );
		...
		$excerpt = apply_filters( 'divino_related_post_excerpt', $excerpt, $current_post_id );

		do_action( 'divino_related_post_before_excerpt', $current_post_id );
		...
		do_action( 'divino_related_post_after_excerpt', $current_post_id );
	}
	 ...
	 * @since 3.5.0
	 */
	public function divino_get_related_post_title( $current_post_id ) {
		$related_post_structure = divino_get_option_meta( 'related-posts-structure' );
		...
		$target    = apply_filters( 'divino_related_post_title_opening_target', '_self' );
		$title_tag = apply_filters( 'divino_related_post_title_tag', 'h3' );

		do_action( 'divino_related_post_before_title', $current_post_id );
		?>
			<<?php echo esc_html( $title_tag ); ?> class="ast-related-post-title entry-title">
				<a href="<?php echo esc_url( apply_filters( 'divino_related_post_link', get_the_permalink(), $current_post_id ) ); ?>" target="<?php echo esc_attr( $target ); ?>" rel="bookmark noopener noreferrer"><?php the_title(); ?></a>
			</<?php echo esc_html( $title_tag ); ?>>
		<?php
		do_action( 'divino_related_post_after_title', $current_post_id );
	}
	 ...
	 * @since 3.5.0
	 */
	public function divino_get_related_post_featured_image( $current_post_id, $before = '', $after = '', $echo = true ) {
		$related_post_structure = divino_get_option_meta( 'related-posts-structure' );

		if ( ! in_array( 'featured-image', $related_post_structure ) ) {
			return;
		}
		$featured_image_size = divino_get_option( 'related-posts-image-size', 'large' );
		...
		$post_thumb = apply_filters(
			'divino_related_post_featured_image_markup',
			get_the_post_thumbnail(
				$current_post_id,
				apply_filters( 'divino_related_posts_thumbnail_default_size', $featured_image_size ),
				array(
					'alt'      => $alt_text ? $alt_text : get_the_title( $current_post_id ),
					'itemprop' => apply_filters( 'divino_related_posts_thumbnail_itemprop', '' ),
				)
			)
		);
		...
		if ( '' !== $post_thumb ) {
			...
			$featured_img_markup .= divino_markup_open(
		...
		$featured_img_markup  = apply_filters( 'divino_related_post_featured_image_after', $featured_img_markup );
		$featured_img_markup .= '</div>';

		$featured_img_markup = apply_filters( 'divino_related_post_thumbnail', $featured_img_markup, $before, $after );
	 ...
	 */
	public function divino_get_related_posts_by_query( $post_id ) {
		$term_ids                  = array();
		...
		$related_posts_order_by    = divino_get_option( 'related-posts-order-by', 'date' );
		$related_posts_order       = divino_get_option( 'related-posts-order', 'desc' );
		$related_posts_based_on    = divino_get_option( 'related-posts-based-on', 'categories' );
		...
		$query_args = apply_filters( 'divino_related_posts_query_args', $query_args );
 ...
 */
new divino_Related_Posts_Markup();
