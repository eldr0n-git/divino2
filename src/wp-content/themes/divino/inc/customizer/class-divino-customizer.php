 ...
 * Customizer Loader
 */
if ( ! class_exists( 'divino_Customizer' ) ) {
	...
	class divino_Customizer {
		 ...
		 * @since 4.8.3
		 */
		public static function is_divino_customizer() {
		...
		public function __construct() {
			...
			add_action( 'wp_ajax_divino_regenerate_fonts_folder', array( $this, 'regenerate_divino_fonts_folder' ) );
		 ...
		 * @since 3.6.0
		 */
		public function regenerate_divino_fonts_folder() {
			check_ajax_referer( 'divino_update_admin_setting', 'security' );

			if ( ! current_user_can( 'edit_theme_options' ) ) {
				wp_send_json_error( 'invalid_permissions' );
			}

			if ( divino_API_Init::get_admin_settings_option( 'self_hosted_gfonts', false ) ) {
				$local_font_loader = divino_webfont_loader_instance( '' );
		...
		public function delete_cached_partials() {
			delete_option( 'divino_partials_config_cache' );

			// Delete previously stored local fonts data, if exists.
			if ( divino_API_Init::get_admin_settings_option( 'self_hosted_gfonts', false ) ) {
				$local_webfont_loader = divino_webfont_loader_instance( '' );
				$local_webfont_loader->divino_delete_fonts_folder();
			}
		}
		...
		public function prepare_customizer_javascript_configs() {
			...
			$cached_data = get_option( 'divino_partials_config_cache', false );
			...
			$defaults = $this->get_divino_customizer_configuration_defaults();
				...
				if ( isset( $configuration['context'] ) ) {
					...
					if ( isset( $configuration['type'] ) && ( ( 'control' === $configuration['type'] ) || ( 'sub-control' === $configuration['type'] ) ) ) {
						if ( ( isset( $configuration['control'] ) && 'ast-builder-header-control' !== $configuration['control'] ) && ( isset( $configuration['name'] ) && strpos( $configuration['name'], 'ast-callback-notice' ) === false ) ) {
							self::$contexts[ $configuration['name'] ] = divino_Builder_Helper::$general_tab;
						}
					}
				}
			...
			update_option( 'divino_partials_config_cache', self::$dynamic_options, false );
		}
		...
		public function logo_svg_icons() {
			...
			wp_send_json_success(
				array(
					'icons' => function_exists( 'divino_get_logo_svg_icons_array' ) ? divino_get_logo_svg_icons_array() : array(),
				)
			);
		}
		...
		public function prepare_javascript_panel_configs( $config ) {
			...
			$panel_name = divino_get_prop( $config, 'name' );
		...
		public function prepare_javascript_section_configs( $config ) {
			...
			$section_name = divino_get_prop( $config, 'name' );
			...
			if ( isset( $config['clone_type'] ) && isset( $config['clone_index'] ) ) {
				...
				if ( isset( divino_Builder_Helper::$component_count_array[ $config['clone_type'] ] ) ) {
					/** @psalm-suppress PossiblyUndefinedStringArrayOffset */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
					if ( in_array( $section_name, divino_Builder_Helper::$component_count_array['removed-items'], true ) || divino_Builder_Helper::$component_count_array[ $config['clone_type'] ] < $config['clone_index'] ) {
						/** @psalm-suppress PossiblyUndefinedStringArrayOffset */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		...
		public function prepare_javascript_sub_control_configs( $config ) {
			...
			$name             = divino_get_prop( $config, 'name' );
			$parent           = divino_get_prop( $config, 'parent' );
			$sub_control_name = divino_THEME_SETTINGS . '[' . $name . ']';
			...
			$sanitize_callback = in_array( $config['control'], $ignore_controls, true ) ? false : divino_get_prop( $config, 'sanitize_callback', divino_Customizer_Control_Base::get_sanitize_call( divino_get_prop( $config, 'control' ) ) );
			...
			$new_config = array(
				...
				'control'           => 'ast-hidden',
				'section'           => divino_get_prop( $config, 'section', 'title_tagline' ),
				'title'             => divino_get_prop( $config, 'title' ),
				'priority'          => divino_get_prop( $config, 'priority', '10' ),
				'default'           => divino_get_prop( $config, 'default' ),
				'sanitize_callback' => $sanitize_callback,
				'suffix'            => divino_get_prop( $config, 'suffix' ),
				'control_type'      => divino_get_prop( $config, 'control' ),
				'linked'            => divino_get_prop( $config, 'linked' ),
				'variant'           => divino_get_prop( $config, 'variant' ),
				'help'              => divino_get_prop( $config, 'help' ),
				'description'       => divino_get_prop( $config, 'description' ),
				'input_attrs'       => divino_get_prop( $config, 'input_attrs' ),
				'disable'           => divino_get_prop( $config, 'disable' ),
			);

			self::$dynamic_options['settings'][ divino_get_prop( $new_config, 'name' ) ] = array(
				'default'           => divino_get_prop( $new_config, 'default' ),
				'type'              => divino_get_prop( $new_config, 'datastore_type' ),
				'transport'         => divino_get_prop( $new_config, 'transport', 'refresh' ),
				'sanitize_callback' => divino_get_prop( $new_config, 'sanitize_callback', divino_Customizer_Control_Base::get_sanitize_call( divino_get_prop( $new_config, 'control' ) ) ),
			);

			$new_config['type']                               = divino_get_prop( $new_config, 'control' );
			$new_config['id']                                 = divino_get_prop( $new_config, 'name' );
			$new_config['settings']                           = array( 'default' => divino_get_prop( $new_config, 'name' ) );
		...
		public function prepare_javascript_control_configs( $config ) {
			...
			$config['label'] = divino_get_prop( $config, 'title' );
			$config['type']  = divino_get_prop( $config, 'control' );
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			if ( false !== divino_get_prop( $config, 'font-type', false ) ) {
				/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$config['type'] = divino_get_prop( $config, 'font-type', false );
			}
			...
			self::$dynamic_options['settings'][ $name ] = array(
				'default'           => divino_get_prop( $config, 'default' ),
				'type'              => divino_get_prop( $config, 'datastore_type' ),
				'transport'         => divino_get_prop( $config, 'transport', 'refresh' ),
				'sanitize_callback' => $sanitize_callback,
			);
			/** @psalm-suppress InvalidArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			if ( divino_get_prop( $config, 'partial', false ) ) {
				self::$dynamic_options['partials'][ $name ] = array(
					'selector'           => divino_get_prop( $config['partial'], 'selector' ),
					'render_callback'    => divino_get_prop( $config['partial'], 'render_callback' ),
					'containerInclusive' => divino_get_prop( $config['partial'], 'container_inclusive' ),
					'fallbackRefresh'    => divino_get_prop( $config['partial'], 'fallback_refresh', true ),
				);
			}
		...
		public function sanitize_control( $config ) {
			...
			switch ( $control_type ) {
				case 'color':
					$config['sanitize_callback'] = array( 'divino_Customizer_Sanitizes', 'sanitize_hex_color' );
					break;
				case 'ast-border':
					$config['sanitize_callback'] = array( 'divino_Customizer_Sanitizes', 'sanitize_border' );
					break;
				case 'ast-html-editor':
					$config['sanitize_callback'] = array( 'divino_Customizer_Sanitizes', 'sanitize_html' );
					break;
				case 'ast-color':
					$config['sanitize_callback'] = array( 'divino_Customizer_Sanitizes', 'sanitize_alpha_color' );
					break;
				case 'ast-sortable':
					$config ['sanitize_callback'] = array( 'divino_Customizer_Sanitizes', 'sanitize_multi_choices' );
					break;
				case 'ast-radio-image':
					$config ['sanitize_callback'] = array( 'divino_Customizer_Sanitizes', 'sanitize_choices' );
					break;
				case 'ast-link':
					$config ['sanitize_callback'] = array( 'divino_Customizer_Sanitizes', 'sanitize_link' );
					break;
				case 'ast-customizer-link':
					$config ['sanitize_callback'] = array( 'divino_Customizer_Sanitizes', 'sanitize_customizer_links' );
					break;
				case 'ast-responsive-slider':
					$config ['sanitize_callback'] = array( 'divino_Customizer_Sanitizes', 'sanitize_responsive_slider' );
					break;
				case 'ast-logo-svg-icon':
					$config ['sanitize_callback'] = array( 'divino_Customizer_Sanitizes', 'sanitize_logo_svg_icon' );
					break;
				case 'ast-toggle-control':
				case 'ast-section-toggle':
					$config ['sanitize_callback'] = array( 'divino_Customizer_Sanitizes', 'sanitize_toggle_control' );
					break;
				default:
					break;
			}
		...
		public function prepare_preload_controls( $config ) {
			...
			$instance = divino_Customizer_Control_Base::get_control_instance( divino_get_prop( $config, 'control' ) );

			// Forwarding to the DOM as default control.
			if ( 'title_tagline' !== $config['section'] ) {
				self::$js_configs ['wp_defaults'][ divino_get_prop( $config, 'name' ) ] = $config['section'];
				$config['section'] = 'title_tagline';
			}

			$wp_customize->add_setting(
				divino_get_prop( $config, 'name' ),
				array(
					'default'           => divino_get_prop( $config, 'default' ),
					'type'              => divino_get_prop( $config, 'datastore_type' ),
					'transport'         => divino_get_prop( $config, 'transport', 'refresh' ),
					'sanitize_callback' => divino_get_prop( $config, 'sanitize_callback', divino_Customizer_Control_Base::get_sanitize_call( divino_get_prop( $config, 'control' ) ) ),
				)
			);

			if ( false !== $instance ) {
				$wp_customize->add_control(
					new $instance( $wp_customize, divino_get_prop( $config, 'name' ), $config )
				);
			} else {
				$wp_customize->add_control( divino_get_prop( $config, 'name' ), $config );
			}
		}
		...
		public function prepare_group_configs( $wp_customize ) {
			...
			foreach ( $configurations as $configuration ) {
				...
				if ( 'sub-control' === $config['type'] ) {
					unset( $config['type'] );
					$parent = divino_get_prop( $config, 'parent' );
					$tab    = divino_get_prop( $config, 'tab' );
		...
		public static function get_contexts() {
			...
			// Return contexts.
			return apply_filters( 'divino_customizer_context', self::$contexts );
		}
		...
		public static function get_choices() {
			// Return contexts.
			return apply_filters( 'divino_customizer_choices', self::$choices );
		}
		...
		public static function get_js_configs() {
			...
			// Return contexts.
			return apply_filters( 'divino_javascript_configurations', self::$js_configs );
		}
		...
		public static function get_tabbed_sections() {
			...
			// Return contexts.
			return apply_filters( 'divino_customizer_tabbed_sections', self::$tabbed_sections );
		}
		...
		private function get_control_defaults() {
			...
			$defaults         = array();
			$default_values   = divino_Theme_Options::defaults();
			...
			foreach ( $default_controls as $section_controls ) {
				foreach ( $section_controls as $control ) {
					$control_id = divino_get_prop( $control, 'name' );
		...
		public function enqueue_customizer_scripts() {
			...
			wp_localize_script(
				...
				array(
					...
					'component_limit'         => divino_Builder_Helper::$component_limit,
					'is_site_rtl'             => is_rtl(),
					'defaults'                => $this->get_control_defaults(),
					'isWP_5_9'                => divino_wp_version_compare( '5.8.99', '>=' ),
					'googleFonts'             => divino_Font_Families::get_google_fonts(),
					'variantLabels'           => divino_Font_Families::font_variant_labels(),
					'upgradeUrl'              => array(
						'default'        => divino_get_upgrade_url( 'customizer' ),
						'global'         => divino_get_pro_url( '/pricing/', 'free-theme', 'customizer', 'global' ),
						'header-builder' => divino_get_pro_url( '/pricing/', 'free-theme', 'customizer', 'header-builder' ),
						'footer-builder' => divino_get_pro_url( '/pricing/', 'free-theme', 'customizer', 'footer-builder' ),
						'sidebar'        => divino_get_pro_url( '/pricing/', 'free-theme', 'customizer', 'sidebar' ),
			...
			wp_enqueue_style(
				...
				divino_THEME_URI . 'inc/assets/css/' . $builder_customizer_css_file . '.css',
				array( 'wp-components' ),
			...
			wp_enqueue_style(
				...
				divino_THEME_URI . 'inc/assets/css/' . $font_icon_picker_css_file . '.css',
				array( 'wp-components' ),
				divino_THEME_VERSION
			);
		}
		...
		private function get_customizer_configurations() {
			...
			self::$configuration = apply_filters( 'divino_customizer_configurations', array(), $wp_customize );
			return self::$configuration;
		}
		 ...
		 */
		private function get_divino_customizer_configuration_defaults() {
			return apply_filters(
				'divino_customizer_configuration_defaults',
				array(
					'priority'             => null,
					'title'                => null,
					'label'                => null,
		...
		public function customize_register_panel( $wp_customize ) {
			 ...
			 */
			$wp_customize->register_panel_type( 'divino_WP_Customize_Panel' );
			$wp_customize->register_section_type( 'divino_WP_Customize_Section' );
			$wp_customize->register_section_type( 'divino_WP_Customize_Separator' );
			...
			if ( ! defined( 'divino_EXT_VER' ) ) {
				$wp_customize->register_section_type( 'divino_Pro_Customizer' );
			}
			...
			divino_Customizer_Control_Base::add_control(
				'image',
				array(
			...
			divino_Customizer_Control_Base::add_control(
				'ast-font',
				array(
					'callback'          => 'divino_Control_Typography',
			...
			divino_Customizer_Control_Base::add_control(
				'ast-logo-svg-icon',
				array(
					'callback'          => 'divino_Control_Logo_SVG_Icon',
			...
			divino_Customizer_Control_Base::add_control(
				'ast-description',
				array(
					'callback'          => 'divino_Control_Description',
					'sanitize_callback' => '',
				)
			);
			...
			divino_Customizer_Control_Base::add_control(
				'ast-customizer-link',
				array(
					'callback'         => 'divino_Control_Customizer_Link',
					'santize_callback' => array( 'divino_Customizer_Sanitizes', 'sanitize_customizer_links' ),
				)
			);
		...
		public function customize_register( $wp_customize ) {
			 ...
			 * Override Defaults
			 */
			require divino_THEME_DIR . 'inc/customizer/override-defaults.php';// phpcs:ignore WPThemeReview.CoreFunctionality.FileInclude.FileIncludeFound
		}
				...
				apply_filters(
					...
					array(
						'customizer' => array(
							...
							'is_pro'              => defined( 'divino_EXT_VER' ),
							'show_upgrade_notice' => divino_showcase_upgrade_notices() ? true : false,
							'upgrade_link'        => esc_url( divino_get_upgrade_url( 'pricing' ) ),
							'is_block_widget'     => divino_has_widgets_block_editor(),
						),
						'theme'      => array(
							'option' => divino_THEME_SETTINGS,
						),
					)
				)
			);
		...
		public function get_formatted_font_style( $tag ) {
			$dataset   = array();
			$dataset[] = '<span class="ast-sg-font-family">' . divino_get_option( 'font-family-' . $tag ) . '</span>';

			$font_size = divino_get_option( 'font-size-' . $tag );
			$desktop   = divino_get_css_value( $font_size['desktop'], $font_size['desktop-unit'] );
			$tablet    = divino_get_css_value( $font_size['tablet'], $font_size['tablet-unit'] );
			$mobile    = divino_get_css_value( $font_size['mobile'], $font_size['mobile-unit'] );
			...
			$dataset[] = '<span class="ast-sg-line-height">' . divino_get_font_extras( divino_get_option( 'font-extras-' . $tag ), 'line-height', 'line-height-unit' ) . '</span>';
		...
		public function render_style_guide_markup() {
			$settings = apply_filters(
				'divino_quick_customizer_navigation_setup',
				array(
					'colors' => array(
						'color-0' => array(
		...
		public function generate_font_dropdown() {
			 ...
			 */
			do_action( 'divino_customizer_font_list', '' );
			?>

			<optgroup label="Google">
				<?php
				foreach ( $google_fonts as $name => $single_font ) {
					$category = divino_get_prop( $single_font, '1' );
					?>
		...
		public function preview_init() {
			...
			$localize_array = array(
				'headerBreakpoint'                     => divino_header_break_point(),
				'includeAnchorsInHeadindsCss'          => divino_Dynamic_CSS::anchors_in_css_selectors_heading(),
				'googleFonts'                          => divino_Font_Families::get_google_fonts(),
				'page_builder_button_style_css'        => divino_Dynamic_CSS::page_builder_button_style_css(),
				'elementor_default_color_font_setting' => divino_Dynamic_CSS::elementor_default_color_font_setting(),
				'dynamic_partial_options'              => self::$dynamic_options['partials'],
				'gb_outline_buttons_patterns_support'  => divino_Dynamic_CSS::gutenberg_core_patterns_compat(),
				'font_weights_widget_title_support'    => divino_Dynamic_CSS::support_font_css_to_widget_and_in_editor(),
				'is_content_bg_option_to_load'         => divino_has_gcp_typo_preset_compatibility(),
				'content_layout'                       => $content_layout,
				'site_layout'                          => divino_get_option( 'site-layout' ),
				'has_block_editor_support'             => divino_Dynamic_CSS::is_block_editor_support_enabled(),
				'updated_gb_outline_button_patterns'   => divino_button_default_padding_updated(),
				'apply_content_bg_fullwidth_layouts'   => divino_get_option( 'apply-content-background-fullwidth-layouts', true ),
				'divino_woo_btn_global_compatibility'   => is_callable( 'divino_Dynamic_CSS::divino_woo_support_global_settings' ) ? divino_Dynamic_CSS::divino_woo_support_global_settings() : false,
				'v4_2_2_core_form_btns_styling'        => true === divino_Dynamic_CSS::divino_core_form_btns_styling() ? ', #comments .submit, .search .search-submit' : '',
				'isLifterLMS'                          => class_exists( 'LifterLMS' ),
				'improved_button_selector'             => divino_Dynamic_CSS::divino_4_6_4_compatibility() ? ', .ast-single-post .entry-content .wp-block-button .wp-block-button__link, .ast-single-post .entry-content .wp-block-search .wp-block-search__button, body .entry-content .wp-block-file .wp-block-file__button' : '',
		...
		public function customize_save() {
			...
			if ( apply_filters( 'divino_resize_logo', true ) ) {

				/* Generate Header Logo */
				$custom_logo_id = get_theme_mod( 'custom_logo' );

				add_filter( 'intermediate_image_sizes_advanced', 'divino_Customizer::logo_image_sizes', 10, 2 );
				self::generate_logo_by_width( $custom_logo_id );
				remove_filter( 'intermediate_image_sizes_advanced', 'divino_Customizer::logo_image_sizes', 10 );
			...
			do_action( 'divino_customizer_save' );
		}
		...
		public static function logo_image_sizes( $sizes, $metadata ) {
			...
			$logo_width = divino_get_option( 'ast-header-responsive-logo-width' );
 ...
 */
divino_Customizer::get_instance();
 ...
 * @return void
 */
function divino_save_header_customizer_configs( $configs ) {
	if ( ! empty( $configs['name'] ) ) {
		$key = str_replace( divino_THEME_SETTINGS . '[', '', $configs['name'] );
		$key = str_replace( ']', '', $key );
		divino_Customizer::$customizer_header_configs[] = $key;
	}
}
 ...
 * @return void
 */
function divino_save_footer_customizer_configs( $configs ) {
	if ( ! empty( $configs['name'] ) ) {
		$key = str_replace( divino_THEME_SETTINGS . '[', '', $configs['name'] );
		$key = str_replace( ']', '', $key );
		divino_Customizer::$customizer_footer_configs[] = $key;
	}
}
