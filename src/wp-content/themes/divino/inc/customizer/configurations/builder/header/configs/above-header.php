 ...
 */
function divino_above_header_configuration() {
	...
	$_configs = array(
		...
		array(
			'name'              => divino_THEME_SETTINGS . '[hba-header-height]',
			'section'           => $_section,
			'transport'         => 'postMessage',
			'default'           => divino_get_option( 'hba-header-height' ),
			'priority'          => 30,
		...
		array(
			'name'        => divino_THEME_SETTINGS . '[hba-header-bg-obj-responsive]',
			'type'        => 'control',
			'section'     => $_section,
			'control'     => 'ast-responsive-background',
			'transport'   => 'postMessage',
			'default'     => divino_get_option( 'hba-header-bg-obj-responsive' ),
		...
		array(
			'name'              => divino_THEME_SETTINGS . '[hba-header-bottom-border-color]',
			'transport'         => 'postMessage',
			'default'           => divino_get_option( 'hba-header-bottom-border-color' ),
			'type'              => 'control',
			'control'           => 'ast-color',
			...
			'context'           => array(
				...
				array(
					'setting'  => divino_THEME_SETTINGS . '[hba-header-separator]',
					'operator' => '>=',
					'value'    => 1,
				),
			),
		...
		array(
			'name'        => divino_THEME_SETTINGS . '[hba-header-separator]',
			'section'     => $_section,
			'priority'    => 40,
			'transport'   => 'postMessage',
			'default'     => divino_get_option( 'hba-header-separator' ),
...
if ( divino_Builder_Customizer::divino_collect_customizer_builder_data() ) {
	add_action( 'init', 'divino_above_header_configuration' );
}
