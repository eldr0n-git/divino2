...
final class divino_Fonts {
	...
	public static function get_fonts() {
		...
		do_action( 'divino_get_fonts' );
		return apply_filters( 'divino_add_fonts', self::$fonts );
	}

	/**
	 * Renders the <link> tag for all fonts in the $fonts array.
	 *
	 * @since 1.0.16 Added the filter 'divino_render_fonts' to support custom fonts.
	 * @since 1.0.0
	 * @return void
	 */
	public static function render_fonts() {

		$font_list = apply_filters( 'divino_render_fonts', self::get_fonts() );
		...
		foreach ( $font_list as $name => $font ) {
			if ( ! empty( $name ) && ! isset( $system_fonts[ $name ] ) ) {
				...
				// Add Subset.
				$subset = apply_filters( 'divino_font_subset', '', $name );
				if ( ! empty( $subset ) ) {
					$font_subset[] = $subset;
				}
			}
		 ...
		 * @since 3.6.0
		 */
		if ( divino_API_Init::get_admin_settings_option( 'self_hosted_gfonts', false ) && ! is_customize_preview() && ! is_admin() ) {
	...
	public static function google_fonts_url( $fonts, $subsets = array() ) {
		...
		$fonts = apply_filters( 'divino_google_fonts', $fonts );

		$fonts = apply_filters( 'divino_google_fonts_selected', $fonts );
		...
		foreach ( $fonts as $font_name => $font_weight ) {
			...
			if ( ! empty( $font_weight ) ) {
				...
				$font_family = str_replace( "'", '', divino_get_prop( $font_family, 0 ) );
		...
		if ( ! empty( $family ) ) {
			...
			$font_args['display'] = divino_get_fonts_display_property();
