...
/**
 * Class divino_Builder_Customizer.
 *
 * Customizer Configuration for Header Footer Builder.
 *
 * @since 3.0.0
 */
final class divino_Builder_Customizer {
	/**
	 * Constructor
	 *
	 * @since 3.0.0
	...
	public function __construct() {
		...
		foreach ( scandir( divino_HEADER_BUILDER_CONFIGS_DIR ) as $config_file ) {
			$path = divino_HEADER_BUILDER_CONFIGS_DIR . $config_file;
			if ( is_file( $path ) ) {
				require_once $path;
			}
		}

		define( 'divino_FOOTER_BUILDER_CONFIGS_DIR', divino_THEME_DIR . 'inc/customizer/configurations/builder/footer/configs/' );
		foreach ( scandir( divino_FOOTER_BUILDER_CONFIGS_DIR ) as $config_file ) {
			$path = divino_FOOTER_BUILDER_CONFIGS_DIR . $config_file;
			if ( is_file( $path ) ) {
				require_once $path;
			}
		}
	...
	public function enqueue_customizer_preview_scripts() {
		...
		wp_enqueue_style(
			...
			divino_THEME_URI . 'inc/assets/css/customizer-preview.css',
			null,
		...
		wp_enqueue_script(
			...
			divino_THEME_URI . 'inc/assets/js/customizer-preview.js',
		...
		wp_enqueue_script(
			...
			divino_THEME_URI . 'inc/builder/type/base/assets/js/customizer-preview.js',
			array( 'customize-preview' ),
			divino_THEME_VERSION,
			true
		);
	 ...
	 * @return bool
	 */
	public static function divino_collect_customizer_builder_data() {
		return ! is_customize_preview() && apply_filters( 'divino_collect_customizer_builder_data', false ) ? true : false;
	}
}

/**
 *  Prepare if class 'divino_Builder_Customizer' exist.
 *  Kicking this off by creating new object of the class.
 */
new divino_Builder_Customizer();
