 ...
 * Theme Options
 */
if ( ! class_exists( 'divino_Theme_Options' ) ) {
	/**
	 * Theme Options
	 */
	class divino_Theme_Options {
		/**
		 * Class instance.
		 *
		 * @var divino_Theme_Options|null $instance Class instance.
		 */
		 ...
		 */
		public static $divino_options = null;
		...
		public static function defaults() {
			 ...
			 * @since 4.5.2
			 */
			$apply_scndry_default_padding_values = divino_scndry_btn_default_padding();
			$update_secondary_paddings           = divino_Dynamic_CSS::divino_4_6_4_compatibility();
			$update_secondary_border             = divino_Dynamic_CSS::divino_4_8_0_compatibility();
			...
			$divino_options        = self::get_divino_options();
			...
			self::$defaults = apply_filters(
				...
				array(
					...
					'button-radius-fields'                 => array(
						'desktop'      => array(
							'top'    => ! isset( $divino_options['button-radius'] ) ? '' : $divino_options['button-radius'],
							'right'  => ! isset( $divino_options['button-radius'] ) ? '' : $divino_options['button-radius'],
							'bottom' => ! isset( $divino_options['button-radius'] ) ? '' : $divino_options['button-radius'],
							'left'   => ! isset( $divino_options['button-radius'] ) ? '' : $divino_options['button-radius'],
						),
						'tablet'       => array(
							'top'    => '',
							'right'  => '',
					...
					// Entry Content.
					'wp-blocks-ui'                         => false === divino_check_is_structural_setup() ? 'custom' : 'comfort',
					...
					// Container.
					'single-page-ast-content-layout'       => false === divino_check_is_structural_setup() ? 'default' : 'normal-width-container',
					'single-page-content-style'            => false === divino_check_is_structural_setup() ? 'default' : 'unboxed',
					...
					'body-font-extras'                     => array(
						'line-height'         => ! isset( $divino_options['body-font-extras'] ) && isset( $divino_options['body-line-height'] ) ? $divino_options['body-line-height'] : '1.65',
						'line-height-unit'    => divino_Dynamic_CSS::divino_4_6_14_compatibility() ? '' : 'em',
						'letter-spacing'      => '',
						'letter-spacing-unit' => 'px',
						'text-transform'      => ! isset( $divino_options['body-font-extras'] ) && isset( $divino_options['body-text-transform'] ) ? $divino_options['body-text-transform'] : '',
						'text-decoration'     => '',
					),
					'headings-font-extras'                 => array(
						'line-height'         => ! isset( $divino_options['headings-font-extras'] ) && isset( $divino_options['headings-line-height'] ) ? $divino_options['headings-line-height'] : '',
						'line-height-unit'    => 'em',
						'letter-spacing'      => '',
						'letter-spacing-unit' => 'px',
						'text-transform'      => ! isset( $divino_options['headings-font-extras'] ) && isset( $divino_options['headings-text-transform'] ) ? $divino_options['headings-text-transform'] : '',
						'text-decoration'     => '',
					),
					'para-margin-bottom'                   => '',
					...
					// Sidebar.
					'site-sidebar-layout'                  => false === divino_check_old_sidebar_user() ? 'right-sidebar' : 'no-sidebar',
					'site-sidebar-width'                   => 30,
					'single-page-sidebar-layout'           => false === divino_check_is_structural_setup() ? 'default' : 'no-sidebar',
					...
					'ast-dynamic-single-download-structure' => true === divino_enable_edd_featured_image_defaults() ? array(
		 ...
		 * @since 4.0.0
		 */
		public static function get_divino_options() {
			if ( is_null( self::$divino_options ) || is_customize_preview() ) {
				self::$divino_options = get_option( divino_THEME_SETTINGS );
			}
			return self::$divino_options;
		}
		...
		public static function get_db_options() {
			self::$db_options_no_defaults = self::get_divino_options();
 ...
 */
divino_Theme_Options::get_instance();
