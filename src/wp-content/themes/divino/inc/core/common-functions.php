 ...
 * Foreground Color
 */
if ( ! function_exists( 'divino_get_foreground_color' ) ) {
	 ...
	 */
	function divino_get_foreground_color( $hex ) {

		$hex = apply_filters( 'divino_before_foreground_color_generation', $hex );
...
if ( ! function_exists( 'divino_css' ) ) {
	 ...
	 */
	function divino_css( $value = '', $css_property = '', $selector = '', $unit = '' ) {
 ...
 * Get Font Size value
 */
if ( ! function_exists( 'divino_responsive_font' ) ) {
	 ...
	 * @return mixed
	 */
	function divino_responsive_font( $font, $device = 'desktop', $default = '' ) {
		...
		if ( isset( $font[ $device ] ) ) {
			...
			if ( '' != $default ) {
				$font_size = divino_get_css_value( $font[ $device ], $font[ $device . '-unit' ], $default );
			} else {
				$font_size = divino_get_font_css_value( $font[ $device ], $font[ $device . '-unit' ] );
			}
		} elseif ( is_numeric( $font ) ) {
			$font_size = divino_get_css_value( $font );
		} else {
 ...
 * Get Font Size value
 */
if ( ! function_exists( 'divino_get_font_css_value' ) ) {
	 ...
	 * Syntax:
	 *
	 *  divino_get_font_css_value( VALUE, DEVICE, UNIT );
	 *
	 * E.g.
	 *
	 *  divino_get_css_value( VALUE, 'desktop', '%' );
	 *  divino_get_css_value( VALUE, 'tablet' );
	 *  divino_get_css_value( VALUE, 'mobile' );
	 *
	 * @param  mixed  $value        CSS value.
	 ...
	 */
	function divino_get_font_css_value( $value, $unit = 'px', $device = 'desktop' ) {

		// If value is empty then return blank.
		if ( '' == $value || ( 0 == $value && ! divino_zero_font_size_case() ) ) {
			return '';
		}
		...
		switch ( $unit ) {
			...
			case 'px':
				if ( is_numeric( $value ) || strpos( $value, 'px' ) ) {
					 ...
					 * @since 4.8.10
					 */
					$disable_px_to_rem = apply_filters( 'divino_disable_px_to_rem_conversion', false );
 ...
 * Get Font family
 */
if ( ! function_exists( 'divino_get_font_family' ) ) {
	 ...
	 * Syntax:
	 *
	 *  divino_get_font_family( VALUE, DEFAULT );
	 *
	 * E.g.
	 *  divino_get_font_family( VALUE, '' );
	 *
	 * @since  1.0.19
	 *
	 ...
	 */
	function divino_get_font_family( $value = '' ) {
 ...
 * Get CSS value
 */
if ( ! function_exists( 'divino_get_css_value' ) ) {
	 ...
	 * Syntax:
	 *
	 *  divino_get_css_value( VALUE, UNIT );
	 *
	 * E.g.
	 *
	 *  divino_get_css_value( VALUE, 'url' );
	 *  divino_get_css_value( VALUE, 'px' );
	 *  divino_get_css_value( VALUE, 'em' );
	 *
	 * @param  string $value        CSS value.
	 * @param  string $unit         CSS unit.
	 ...
	 * @return mixed               CSS value depends on $unit
	 */
	function divino_get_css_value( $value = '', $unit = 'px', $default = '' ) {
		...
		switch ( $unit ) {
			...
			case 'font':
				if ( 'inherit' !== $value ) {
					$value   = divino_get_font_family( $value );
					$css_val = $value;
				} elseif ( '' != $default ) {
					$css_val = $default;
				} else {
 ...
 */
if ( ! function_exists( 'divino_get_background_obj' ) ) {
	 ...
	 */
	function divino_get_background_obj( $bg_obj ) {
							...
							if ( 'classic' === $overlay_type && '' !== $overlay_color ) {
								...
								if ( '' !== $overlay_opacity ) {
									...
									if ( $is_linked_with_gcp ) {
										$divino_gcp_instance    = new divino_Global_Palette();
										$updated_overlay_color = $divino_gcp_instance->get_color_by_palette_variable( $overlay_color );
									}

									if ( '#' === $updated_overlay_color[0] ) {
										$updated_overlay_color = divino_hex_to_rgba( $updated_overlay_color, $overlay_opacity );
									}
								}
 ...
 * Parse CSS
 */
if ( ! function_exists( 'divino_parse_css' ) ) {
	 ...
	 */
	function divino_parse_css( $css_output = array(), $min_media = '', $max_media = '' ) {
...
if ( ! function_exists( 'divino_get_options' ) ) {
	...
	function divino_get_options() {
		 ...
		 */
		if ( apply_filters( 'divino_get_options_nocache', false ) ) {
			$divino_options = get_option( divino_THEME_SETTINGS, array() );
		} else {
			// Use a static variable to cache the options for this request.
			static $cached_divino_options = null;

			// Fetch the options once and cache them in the static variable.
			if ( is_null( $cached_divino_options ) || is_customize_preview() ) {
				$cached_divino_options = divino_Theme_Options::get_divino_options();
			}

			$divino_options = $cached_divino_options;
		}
		 ...
		 * @return array The theme options array.
		 */
		return apply_filters( 'divino_get_options', $divino_options );
	}
}
 ...
 * Return Theme option.
 */
if ( ! function_exists( 'divino_get_option' ) ) {
	 ...
	 */
	function divino_get_option( $option, $default = '', $deprecated = '' ) {
		 ...
		 * @var Array
		 */
		$theme_options = apply_filters( 'divino_get_option_array', $theme_options, $option, $default );
		 ...
		 * @var Mixed.
		 */
		return apply_filters( "divino_get_option_{$option}", $value, $option, $default );
	}
}
 ...
 */
if ( ! function_exists( 'divino_get_i18n_string' ) ) {
...
if ( ! function_exists( 'divino_update_option' ) ) {
	 ...
	 * @return void
	 */
	function divino_update_option( $option, $value ) {

		do_action( "divino_before_update_option_{$option}", $value, $option );

		// Get all customizer options.
		$theme_options = get_option( divino_THEME_SETTINGS );
		...
		update_option( divino_THEME_SETTINGS, $theme_options );

		do_action( "divino_after_update_option_{$option}", $value, $option );
	}
}

if ( ! function_exists( 'divino_delete_option' ) ) {
	 ...
	 * @return void
	 */
	function divino_delete_option( $option ) {

		do_action( "divino_before_delete_option_{$option}", $option );
		...
		do_action( "divino_after_delete_option_{$option}", $option );
	}
}
 ...
 */
if ( ! function_exists( 'divino_get_option_meta' ) ) {
	 ...
	 * @return Mixed             Return option value.
	 */
	function divino_get_option_meta( $option_id, $default = '', $only_meta = false, $extension = '', $post_id = '' ) {

		$post_id = '' != $post_id ? $post_id : divino_get_post_id();

		$value = divino_get_option( $option_id, $default );
		 ...
		 * @var Mixed.
		 */
		return apply_filters( "divino_get_option_meta_{$option_id}", $value, $default, $default );
	}
}
 ...
 */
if ( ! function_exists( 'divino_get_post_id' ) ) {
	 ...
	 */
	function divino_get_post_id( $post_id_override = '' ) {

		if ( null == divino_Theme_Options::$post_id ) {
			global $post;
			...
			divino_Theme_Options::$post_id = $post_id;
		}

		return apply_filters( 'divino_get_post_id', divino_Theme_Options::$post_id, $post_id_override );
	}
}
 ...
 */
if ( ! function_exists( 'divino_primary_class' ) ) {
	 ...
	 */
	function divino_primary_class( $class = '' ) {

		// Separates classes with a single space, collates classes for body element.
		echo 'class="' . esc_attr( join( ' ', divino_get_primary_class( $class ) ) ) . '"';
	}
}
 ...
 */
if ( ! function_exists( 'divino_get_primary_class' ) ) {
	 ...
	 */
	function divino_get_primary_class( $class = '' ) {
		...
		$classes = apply_filters( 'divino_primary_class', $classes, $class );
 ...
 */
if ( ! function_exists( 'divino_secondary_class' ) ) {
	 ...
	 * @return void        echo classes.
	 */
	function divino_secondary_class( $class = '' ) {

		// Separates classes with a single space, collates classes for body element.
		echo 'class="' . esc_attr( join( ' ', divino_get_secondary_class( $class ) ) ) . '"';
	}
}
 ...
 * Retrieve the classes for the secondary element as an array.
 */
if ( ! function_exists( 'divino_get_secondary_class' ) ) {
	 ...
	 * @return array        Return array of classes.
	 */
	function divino_get_secondary_class( $class = '' ) {
		...
		$classes = apply_filters( 'divino_secondary_class', $classes, $class );
 ...
 * Get post format
 */
if ( ! function_exists( 'divino_get_post_format' ) ) {
	 ...
	 */
	function divino_get_post_format( $post_format_override = '' ) {
		...
		return apply_filters( 'divino_get_post_format', $post_format, $post_format_override );
	}
}
 ...
 */
if ( ! function_exists( 'divino_the_post_title' ) ) {
	 ...
	 */
	function divino_the_post_title( $before = '', $after = '', $post_id = 0, $echo = true ) {

		$enabled = apply_filters( 'divino_the_post_title_enabled', true );
		if ( $enabled ) {

			$title  = divino_get_the_title( $post_id );
			$before = apply_filters( 'divino_the_post_title_before', $before );
			$after  = apply_filters( 'divino_the_post_title_after', $after );
...
if ( ! function_exists( 'divino_the_search_page_title' ) ) {
	/**
	 ...
	 * @since 4.8.4
	 */
	function divino_the_search_page_title( $before = '', $after = '', $echo = false ) {
		$title = apply_filters(
			'divino_the_search_page_title',
			sprintf(
 ...
 */
if ( ! function_exists( 'divino_the_title' ) ) {
	 ...
	 * @return string|void String if $echo parameter is false.
	 */
	function divino_the_title( $before = '', $after = '', $post_id = 0, $echo = true ) {

		$title             = '';
		$post_type         = divino_get_post_type();
		$blog_post_title   = divino_get_option( 'ast-dynamic-archive-' . $post_type . '-structure', array( 'ast-dynamic-archive-' . $post_type . '-title', 'ast-dynamic-archive-' . $post_type . '-description' ) );
		$single_post_title = divino_get_option( 'ast-dynamic-single-' . $post_type . '-structure', 'page' === $post_type ? array( 'ast-dynamic-single-' . $post_type . '-image', 'ast-dynamic-single-' . $post_type . '-title' ) : array( 'ast-dynamic-single-' . $post_type . '-title', 'ast-dynamic-single-' . $post_type . '-meta' ) );
		...
		if ( ( ! is_singular() && ( in_array( 'ast-dynamic-archive-' . $post_type . '-title', $blog_post_title ) || in_array( 'ast-dynamic-archive-' . $post_type . '-meta', $blog_post_title ) ) )
			...
			if ( apply_filters( 'divino_the_title_enabled', true ) ) {

				$title  = divino_get_the_title( $post_id );
				$before = apply_filters( 'divino_the_title_before', $before );
				$after  = apply_filters( 'divino_the_title_after', $after );
 ...
 */
if ( ! function_exists( 'divino_get_the_title' ) ) {
	 ...
	 * @return string|void String if $echo parameter is false.
	 */
	function divino_get_the_title( $post_id = 0, $echo = false ) {
		...
		if ( $post_id || is_singular() ) {
			...
			if ( is_front_page() && is_home() ) {
				...
				$title = divino_the_search_page_title();
		...
		$title = apply_filters( 'divino_the_title', $title, $post_id );
 ...
 */
function divino_use_dynamic_blog_layouts() {
	$divino_settings = divino_get_options();
	return apply_filters( 'divino_get_option_dynamic_blog_layouts', isset( $divino_settings['dynamic-blog-layouts'] ) ? $divino_settings['dynamic-blog-layouts'] : true );
}
 ...
 * @since 4.0.0
 */
function divino_get_taxonomy_banner_legacy_layout() {
	$post_type        = divino_get_post_type();
	$banner_structure = is_search() ? divino_get_option( 'section-search-page-title-structure' ) : divino_get_option( 'ast-dynamic-archive-' . $post_type . '-structure', array( 'ast-dynamic-archive-' . $post_type . '-title', 'ast-dynamic-archive-' . $post_type . '-description' ) );
	...
	?>
		<section class="ast-archive-description">
			...
			foreach ( $banner_structure as $metaval ) {
				$meta_key = 'archive-' . divino_get_last_meta_word( $metaval );
				switch ( $meta_key ) {
					case 'archive-title':
						do_action( 'divino_before_archive_title' );
						if ( is_search() ) {
							divino_the_search_page_title( '<h1 class="page-title ast-archive-title">', '</h1>', true );
						} else {
					...
					case 'archive-breadcrumb':
						if ( ! is_author() ) {
							do_action( 'divino_before_archive_breadcrumb' );
							echo divino_get_breadcrumb(); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
							do_action( 'divino_after_archive_breadcrumb' );
						}
						break;
					case 'archive-description':
						...
						do_action( 'divino_after_archive_description' );
						break;
				}
			}
			?>
 ...
 * Archive Page Title
 */
if ( ! function_exists( 'divino_archive_page_info' ) ) {
	 ...
	 * Displays title only if the page title bar is disabled.
	 */
	function divino_archive_page_info() {
			...
			if ( is_author() ) {
				...
				<section class="ast-author-box ast-archive-description">
					<div class="ast-author-bio">
						<?php do_action( 'divino_before_archive_title' ); ?>
						<h1 class='page-title ast-archive-title'><?php echo esc_html( apply_filters( 'divino_author_page_title', $author_name_html ) ); ?></h1>
						<?php do_action( 'divino_after_archive_title' ); ?>
						<?php
						/** @psalm-suppress RedundantConditionGivenDocblockType */
						...
						<?php } ?>
						<?php do_action( 'divino_after_archive_description' ); ?>
					</div>
				...
				$taxonomy_banner_content = divino_get_taxonomy_banner_legacy_layout();
	...
	add_action( 'divino_archive_header', 'divino_archive_page_info' );
}

/**
 * Adjust the HEX color brightness
 */
if ( ! function_exists( 'divino_adjust_brightness' ) ) {
	 ...
	 */
	function divino_adjust_brightness( $hex, $steps, $type ) {
 ...
 */
if ( ! function_exists( 'divino_hex_to_rgba' ) ) {
	 ...
	 */
	function divino_hex_to_rgba( $color, $opacity = false ) {
...
if ( ! function_exists( 'divino_enable_page_builder_compatibility' ) ) {
	 ...
	 */
	function divino_enable_page_builder_compatibility() {
		return apply_filters( 'divino_enable_page_builder_compatibility', true );
	}
}

if ( ! function_exists( 'divino_get_pro_url' ) ) {
	/**
	 ...
	 * @return mixed
	 */
	function divino_get_pro_url( $path, $source = '', $medium = '', $campaign = '' ) {
		$url           = esc_url( divino_WEBSITE_BASE_URL . $path );
		$divino_pro_url = trailingslashit( $url );

		// Set up our URL if we have a source.
		if ( ! empty( $source ) ) {
			$divino_pro_url = add_query_arg( 'utm_source', sanitize_text_field( $source ), $divino_pro_url );
		}
		...
		if ( ! empty( $medium ) ) {
			$divino_pro_url = add_query_arg( 'utm_medium', sanitize_text_field( $medium ), $divino_pro_url );
		}

		// Set up our URL if we have a campaign.
		if ( ! empty( $campaign ) ) {
			$divino_pro_url = add_query_arg( 'utm_campaign', sanitize_text_field( $campaign ), $divino_pro_url );
		}

		$divino_pro_url = apply_filters( 'divino_get_pro_url', $divino_pro_url, $url );
		$divino_pro_url = remove_query_arg( 'bsf', $divino_pro_url );

		$ref = get_option( 'divino_partner_url_param', '' );
		if ( ! empty( $ref ) ) {
			$divino_pro_url = add_query_arg( 'bsf', sanitize_text_field( $ref ), $divino_pro_url );
		}

		return $divino_pro_url;
	}
}
 ...
 * Search Form
 */
if ( ! function_exists( 'divino_get_search_form' ) ) {
	/**
	 * Display search form.
	 *
	 ...
	 */
	function divino_get_search_form( $echo = true ) {
		 ...
		 */
		$result = apply_filters( 'divino_get_search_form', $form );
 ...
 * @return bool
 */
function divino_is_amp_endpoint() {
 ...
 */
if ( ! function_exists( 'divino_responsive_spacing' ) ) {
	 ...
	 * @return mixed
	 */
	function divino_responsive_spacing( $option, $side = '', $device = 'desktop', $default = '', $prefix = '' ) {

		if ( isset( $option[ $device ][ $side ] ) && isset( $option[ $device . '-unit' ] ) ) {
			$spacing = divino_get_css_value( $option[ $device ][ $side ], $option[ $device . '-unit' ], $default );
		} elseif ( is_numeric( $option ) ) {
			$spacing = divino_get_css_value( $option );
		} else {
 ...
 */
function divino_get_tablet_breakpoint( $min = '', $max = '' ) {

	$update_breakpoint = divino_get_option( 'can-update-theme-tablet-breakpoint', true );

	// Change default for new users.
	$default = true === $update_breakpoint ? 921 : 768;

	$header_breakpoint = apply_filters( 'divino_tablet_breakpoint', $default );
 ...
 */
function divino_get_mobile_breakpoint( $min = '', $max = '' ) {

	$header_breakpoint = apply_filters( 'divino_mobile_breakpoint', 544 );
...
if ( ! function_exists( 'divino_color_responsive_css' ) ) {
	 ...
	 */
	function divino_color_responsive_css( $setting, $css_property, $selector ) {
		$css = '';
		...
		if ( isset( $setting['tablet'] ) && ! empty( $setting['tablet'] ) ) {
			$css .= '@media (max-width:' . divino_get_tablet_breakpoint() . 'px) {' . $selector . '{' . $css_property . ':' . esc_attr( $setting['tablet'] ) . ';} }';
		}
		if ( isset( $setting['mobile'] ) && ! empty( $setting['mobile'] ) ) {
			$css .= '@media (max-width:' . divino_get_mobile_breakpoint() . 'px) {' . $selector . '{' . $css_property . ':' . esc_attr( $setting['mobile'] ) . ';} }';
		}
		return $css;
	}
}

if ( ! function_exists( 'divino_check_is_bb_themer_layout' ) ) {

	/**
	 * Check if layout is bb themer's layout
	 */
	function divino_check_is_bb_themer_layout() {
...
if ( ! function_exists( 'divino_is_white_labelled' ) ) {
	...
	function divino_is_white_labelled() {
		...
		if ( is_callable( 'divino_Ext_White_Label_Markup::show_branding' ) && ! divino_Ext_White_Label_Markup::show_branding() ) {
			return apply_filters( 'divino_is_white_labelled', true );
		}

		return apply_filters( 'divino_is_white_labelled', false );
	}
}
 ...
 * @return string
 */
function divino_get_fonts_display_property() {
	return apply_filters( 'divino_fonts_display_property', 'fallback' );
}
 ...
 * @return array
 */
function divino_get_responsive_background_obj( $bg_obj_res, $device ) {
 ...
 * @return bool
 */
function divino_check_pagination_enabled() {
	global  $wp_query;

	return $wp_query->max_num_pages > 1 && apply_filters( 'divino_pagination_enabled', true );
}
 ...
 * @return bool
 */
function divino_check_current_post_comment_enabled() {
 ...
 */
function divino_wp_version_compare( $version, $compare ) {
 ...
 */
function divino_block_based_legacy_setup() {
	$divino_settings = divino_get_options();
	return apply_filters( 'divino_block_based_legacy_setup', isset( $divino_settings['blocks-legacy-setup'] ) && isset( $divino_settings['wp-blocks-ui'] ) && 'legacy' === $divino_settings['wp-blocks-ui'] ? true : false );
}
 ...
 * @return bool true|false.
 */
function divino_check_is_structural_setup() {
	$divino_settings = divino_get_options();
	return apply_filters( 'divino_get_option_customizer-default-layout-update', isset( $divino_settings['customizer-default-layout-update'] ) ? false : true ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 * @return bool true|false.
 */
function divino_load_woocommerce_login_form_password_icon() {
	$divino_settings = divino_get_options();
	return apply_filters( 'divino_get_option_woo-show-password-icon', isset( $divino_settings['woo-show-password-icon'] ) ? false : true ); // phpcs:ignore WordPress.NamingConventions.ValidHookName.UseUnderscores
}
 ...
 */
function divino_narrow_container_width( $location, $narrow_container_max_width ) {

	if ( 'narrow-container' === $location ) {

		$narrow_container_css = array(
			'.ast-narrow-container .site-content > .ast-container' => array(
				'max-width' => divino_get_css_value( $narrow_container_max_width, 'px' ),
			),
		);

		// Remove Sidebar for Narrow Width Container Layout.
		if ( 'narrow-container' === divino_get_content_layout() ) {
			add_filter(
				'divino_page_layout',
		...
		return divino_parse_css( $narrow_container_css, divino_get_tablet_breakpoint( '', 1 ) );
	}
 ...
 */
function divino_get_sidebar_layout_for_editor( $post_type ) {

	$sidebar_layout = divino_get_option( 'single-' . $post_type . '-sidebar-layout' );
 ...
 */
function divino_get_filter_svg( $filter_id, $color ) {
 ...
 */
function divino_hex_to_rgb( $hex ) {
 ...
 */
function divino_split_rgba( $rgba ) {
 ...
 */
function divino_render_svg_mask( $id, $filter_name, $color ) {

	if ( 0 === strpos( $color, 'var(--' ) ) {
		$agp = new divino_Global_Palette();
		/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$svg_color = divino_hex_to_rgb( $agp->get_color_by_palette_variable( $color ) );
	} elseif ( preg_match( '/^#[a-f0-9]{6}$/i', $color ) ) {
		/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$svg_color = divino_hex_to_rgb( $color );
	} else {
		/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		$svg_color = divino_split_rgba( $color );
	}

	/** @psalm-suppress UndefinedFunction  */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
	echo divino_get_filter_svg( $id, apply_filters( 'divino_' . $filter_name, $svg_color ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
}
 ...
 * @since 4.7.0
 */
function divino_get_logo_svg_icons_array() {
	...
	$icons_dir = divino_THEME_DIR . 'assets/svg/logo-svg-icons';
...
function divino_get_post_type() {
	 ...
	 */
	return apply_filters( 'divino_get_post_type', strval( $post_type ) );
}
 ...
 */
function divino_get_upgrade_url( $type = 'pro' ) {
	...
	switch ( $type ) {
		case 'pricing':
			$upgrade_url = divino_get_pro_url( '/pricing/', 'free-theme', 'customizer', 'main-cta' );
			break;
		case 'dashboard':
			$upgrade_url = divino_get_pro_url( '/pricing/', 'free-theme', 'dashboard', 'upgrade' );
			break;
		case 'customizer':
			$upgrade_url = divino_get_pro_url( '/pricing/', 'free-theme', 'dashboard', 'upgrade' );
			break;
		default:
			$upgrade_url = divino_get_pro_url( '/pro/', 'free-theme', 'dashboard', 'upgrade-now' );
			break;
	}
 ...
 * @since 4.10.0
 */
function divino_icon_selector_svg( $icon, $echo = false, $default = '' ) {
	// Bail early if SVG icons are not enabled.
	if ( ! divino_Icons::is_svg_icons() || ! $icon ) {
		return $echo ? null : '';
	}
	...
	if ( isset( $icon['type'], $icon['value'] ) ) {
		...
		if ( $type === 'custom' ) {
			...
			$svg       = divino_Builder_UI_Controller::fetch_svg_icon( $value );
			$classes[] = 'icon-' . $value;
		}
	}

	if ( ! $svg && $default ) {
		$svg = divino_Builder_UI_Controller::fetch_svg_icon( $default );
	}
	...
	if ( $echo !== true ) {
		return wp_kses( $svg, divino_Icons::allowed_svg_args() );
	}

	echo wp_kses( $svg, divino_Icons::allowed_svg_args() );
}
