...
/**
 * Class divino_Builder_Helper.
 */
final class divino_Builder_Helper {
	/**
	...
	public function __construct() {
		...
		self::$num_of_header_button = defined( 'divino_EXT_VER' ) ? self::$component_count_array['header-button'] : 1;
		self::$num_of_footer_button = defined( 'divino_EXT_VER' ) ? self::$component_count_array['footer-button'] : 0;

		self::$num_of_header_html = defined( 'divino_EXT_VER' ) ? self::$component_count_array['header-html'] : 2;
		self::$num_of_footer_html = defined( 'divino_EXT_VER' ) ? self::$component_count_array['footer-html'] : 2;

		self::$num_of_header_menu = defined( 'divino_EXT_VER' ) ? self::$component_count_array['header-menu'] : 2;

		self::$num_of_header_widgets = defined( 'divino_EXT_VER' ) ? self::$component_count_array['header-widget'] : 2;
		self::$num_of_footer_widgets = defined( 'divino_EXT_VER' ) ? self::$component_count_array['footer-widget'] : 4;

		self::$num_of_header_social_icons = defined( 'divino_EXT_VER' ) ? self::$component_count_array['header-social-icons'] : 1;
		self::$num_of_footer_social_icons = defined( 'divino_EXT_VER' ) ? self::$component_count_array['footer-social-icons'] : 1;

		// Divider.
		self::$num_of_header_divider = defined( 'divino_EXT_VER' ) ? self::$component_count_array['header-divider'] : 0;
		self::$num_of_footer_divider = defined( 'divino_EXT_VER' ) ? self::$component_count_array['footer-divider'] : 0;

		self::$num_of_footer_columns = defined( 'divino_EXT_VER' ) ? apply_filters( 'divino_footer_column_count', 6 ) : 6;
	...
	public function initialize() {
		...
		add_filter( 'divino_addon_list', array( $this, 'deprecate_old_header_and_footer' ) );
	}
	...
	public static function is_header_footer_builder_active() {
		...
		return apply_filters( 'divino_is_header_footer_builder_active', isset( $divino_settings['is-header-footer-builder'] ) ? (bool) $divino_settings['is-header-footer-builder'] : true );
	}
	...
	public static function has_mobile_side_columns( $row = 'primary' ) {
		...
		$mobile_sides = false;
		$elements     = divino_get_option( 'header-mobile-items' );
	...
	public static function render_builder_markup( $row = 'primary', $column = 'left', $header = 'desktop', $builder = 'header' ) {
		$elements = divino_get_option( $builder . '-' . $header . '-items' );
	...
	public static function render_mobile_popup_markup() {
		...
		$off_canvas_slide   = divino_get_option( 'off-canvas-slide' );
		$mobile_header_type = divino_get_option( 'mobile-header-type' );
		$content_alignment  = divino_get_option( 'header-offcanvas-content-alignment' );
					...
					<div class="ast-mobile-popup-header">
						<button type="button" id="menu-toggle-close" class="menu-toggle-close" aria-label="Close menu" tabindex="0">
							<span class="ast-svg-iconset">
								<?php echo do_shortcode( divino_Builder_UI_Controller::fetch_svg_icon( 'close' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
							</span>
						</button>
					</div>
	...
	public static function has_center_column( $row = 'main', $builder_type = 'header', $device = 'desktop' ) {
		...
		$center   = false;
		$elements = divino_get_option( $builder_type . '-' . $device . '-items' );
		if ( isset( $elements ) && isset( $elements[ $row ] ) &&
	...
	public static function is_footer_row_empty( $row = 'primary' ) {
		$sides    = false;
		$elements = divino_get_option( 'footer-desktop-items' );
	...
	public static function is_component_loaded( $component_id, $builder_type = 'header', $device = 'both' ) {
		...
		if ( is_null( self::$loaded_grid ) ) {
			...
			$grids                   = array();
			$grids['header_desktop'] = divino_get_option( 'header-desktop-items', array() );
			$grids['header_mobile']  = divino_get_option( 'header-mobile-items', array() );
			$grids['footer_both']    = divino_get_option( 'footer-desktop-items', array() );
	...
	public static function apply_flex_based_css() {
		$divino_settings = divino_get_options();
		return apply_filters( 'divino_apply_flex_based_css', isset( $divino_settings['is-flex-based-css'] ) ? $divino_settings['is-flex-based-css'] : true );
	}
...
/**
 *  Prepare if class 'divino_Builder_Helper' exist.
 *  Kicking this off by calling 'get_instance()' method
 */
divino_Builder_Helper::get_instance();
