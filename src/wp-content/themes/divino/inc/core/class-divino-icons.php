...
class divino_Icons {
	...
	public static function get_icons( $icon, $is_echo = false, $replace = false, $menu_location = 'main' ) {
		...
		if ( true === self::is_svg_icons() ) {
			...
			if ( 'menu-bars' === $icon ) {
				$menu_icon = apply_filters( 'divino_' . $menu_location . '_menu_toggle_icon', 'menu-toggle-icon' );
				$output    = '<span class="' . esc_attr( $menu_icon ) . '"></span>';
			}
		}

		$output = apply_filters( 'divino_svg_icon_element', $output, $icon );
		...
		$output = apply_filters(
			'divino_svg_icon', // phpcs:ignore WordPress.NamingConventions.PrefixAllGlobals.NonPrefixedHooknameFound
			sprintf(
