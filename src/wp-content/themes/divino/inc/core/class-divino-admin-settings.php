...
if ( ! class_exists( 'divino_Admin_Settings' ) ) {
	...
	class divino_Admin_Settings {
		 ...
		 * @since 4.3.0
		 */
		private static $divino_addon_supported_version_map = array(
			'4.1.6' => '4.1.0',
			'4.0.2' => '4.0.0',
			'3.9.4' => '3.9.2',
			'3.9.1' => '3.9.0',
		 ...
		 * @since 3.6.6
		 */
		public static function load_divino_admin_script() {
		...
		public static function register_notices() {
			...
			if ( divino_is_white_labelled() || ! divino_THEME_ORG_VERSION || false === apply_filters( 'divino_showcase_starter_templates_notice', true ) || ! divino_Menu::is_promoting_starter_templates() ) {
				/** @psalm-suppress RedundantCondition */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				return;
			}
			...
			if ( current_user_can( 'install_plugins' ) && ! defined( 'divino_SITES_NAME' ) && '1' == get_option( 'fresh_site' ) && ! in_array( $current_slug, array( 'astra-advanced-hook', 'divino_adv_header' ), true ) ) {
				...
				$divino_sites_notice_args = array(
					...
					'message'                    => sprintf(
						...
						'data-init="' . divino_get_prop( $ast_sites_notice_btn, 'data_init', '' ) . '"',
						'data-settings-link-text="' . divino_get_prop( $ast_sites_notice_btn, 'data_settings_link_text', '' ) . '"',
						'data-settings-link="' . divino_get_prop( $ast_sites_notice_btn, 'data_settings_link', '' ) . '"',
					...
					'show_if'                    => class_exists( 'divino_Ext_White_Label_Markup' ) ? divino_Ext_White_Label_Markup::show_branding() : true,
				);

				divino_Notices::add_notice(
					$divino_sites_notice_args
				);
			}
		}
		...
		public static function minimum_addon_version_notice() {
			...
			if ( version_compare( divino_EXT_VER, divino_EXT_MIN_VER ) < 0 ) {
				...
				if ( ! $min_version ) {
					update_user_meta( get_current_user_id(), 'ast-minimum-addon-version-notice-min-ver', divino_EXT_MIN_VER );
				}

				if ( version_compare( $min_version, divino_EXT_MIN_VER, '!=' ) ) {
				...
				divino_Notices::add_notice( $notice_args );
			}
		}
		 ...
		 * @since 4.3.0
		 */
		public static function get_divino_addon_min_supported_version( $input_version ) {
			if ( defined( 'divino_EXT_VER' ) && version_compare( divino_EXT_VER, divino_EXT_MIN_VER ) < 0 ) {
				return divino_EXT_MIN_VER;
			}

			$supported_version = '';

			// First, check if the exact version is supported
			if ( isset( self::$divino_addon_supported_version_map[ $input_version ] ) ) {
				$supported_version = self::$divino_addon_supported_version_map[ $input_version ];
			} else {
				foreach ( self::$divino_addon_supported_version_map as $index => $supported ) {
		...
		public static function minimum_addon_supported_version_notice() {
			...
			// divino_EXT_MIN_VER < divino_EXT_VER && divino_EXT_VER < 4.0.0.
			if ( version_compare( divino_EXT_VER, divino_EXT_MIN_VER ) >= 0 || version_compare( '4.0.0', divino_EXT_VER ) < 0 ) {
				return;
			}

			$divino_addon_supported_version = self::get_divino_addon_min_supported_version( divino_EXT_VER );
			$message                       = sprintf(
				...
				$divino_addon_supported_version
			);
			...
			if ( ! $ext_min_supported_version ) {
				update_user_meta( get_current_user_id(), 'ast-addon-supported-version-notice', $divino_addon_supported_version );
			}

			if ( version_compare( $ext_min_supported_version, $divino_addon_supported_version, '!=' ) ) {
		 ...
		 */
		public static function divino_sites_notice_button() {
		...
		public static function get_starter_templates_slug() {
			...
			if ( defined( 'divino_PRO_SITES_VER' ) && version_compare( divino_PRO_SITES_VER, '2.0.0', '>=' ) ) {
				self::$starter_templates_slug = 'starter-templates';
			}

			if ( defined( 'divino_SITES_VER' ) && version_compare( divino_SITES_VER, '2.0.0', '>=' ) ) {
				self::$starter_templates_slug = 'starter-templates';
			}
		}
		...
		public static function customizer_scripts() {
			$color_palettes = wp_json_encode( divino_color_palette() );
			...
			if ( in_array( $hook, array( 'post.php', 'post-new.php' ) ) ) {
				...
				if ( null !== $post_type && is_callable( 'use_block_editor_for_post_type' ) && use_block_editor_for_post_type( $post_type ) && in_array( $post_type, (array) $post_types ) ) {
					...
					echo '<style class="astra-meta-box-style">
						...
						.block-editor-page #divino_settings_meta_box .components-base-control__field, .block-editor-page #divino_settings_meta_box .block-editor-page .transparent-header-wrapper, .block-editor-page #divino_settings_meta_box .adv-header-wrapper, .block-editor-page #divino_settings_meta_box .stick-header-wrapper, .block-editor-page #divino_settings_meta_box .disable-section-meta div { margin-bottom: 8px; }
						.block-editor-page #divino_settings_meta_box .disable-section-meta div label { vertical-align: inherit; }
						.block-editor-page #divino_settings_meta_box .post-attributes-label-wrapper { margin-bottom: 4px; }
						#side-sortables #divino_settings_meta_box select { min-width: 100%; }
						#normal-sortables #divino_settings_meta_box select { min-width: 200px; }
					</style>';
		...
		public static function get_page_url( $menu_slug ) {
			...
			if ( strpos( $parent_page, '?' ) !== false ) {
				/** @psalm-suppress InvalidLiteralArgument */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$query_var = '&page=' . divino_Menu::get_theme_page_slug();
			} else {
				$query_var = '?page=' . divino_Menu::get_theme_page_slug();
			}
