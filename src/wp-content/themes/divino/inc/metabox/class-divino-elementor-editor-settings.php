 ...
 * @since 4.11.3
 */
if ( ! class_exists( 'divino_Elementor_Editor_Settings' ) ) {
	/**
	 * Elementor Editor Settings integration class.
	 *
	 * @since 4.11.3
	 */
	class divino_Elementor_Editor_Settings {
		/**
		 * Class instance.
		 *
		 * @var self|null
		 ...
		 */
		public static function get_divino_elementor_setting_keys() {
			$settings = array(
		...
		public function enqueue_editor_assets() {
			...
			$dependencies = array();
			if ( file_exists( divino_THEME_DIR . 'inc/metabox/extend-metabox/build/elementor.asset.php' ) ) {
				$assets = require divino_THEME_DIR . 'inc/metabox/extend-metabox/build/elementor.asset.php';
		...
		public static function get_formatted_container_layout_options() {
			$options = divino_Meta_Boxes::get_instance()->get_content_layout_options();
		...
		public static function get_formatted_container_style_options() {
			$options = divino_Meta_Boxes::get_instance()->get_content_style_options();
		...
		public static function get_formatted_sidebar_layout_options() {
			$options = divino_Meta_Boxes::get_instance()->get_sidebar_options();
		...
		public static function get_formatted_sidebar_style_options() {
			$options = divino_Meta_Boxes::get_instance()->get_sidebar_style_options();
		...
		public static function get_formatted_header_enabled_options() {
			$options = divino_Meta_Boxes::get_instance()->get_header_enabled_options();
		...
		public function register_document_controls( $document ) {
			...
			$controls = divino_Meta_Boxes::get_instance()->get_disable_section_fields();
			...
			$controls = divino_Meta_Boxes::get_instance()->get_header_disable_meta_fields();
			foreach ( $controls as $details ) {
				$meta_key = $details['key'];
				$document->add_control(
					$meta_key,
			...
			if ( defined( 'divino_EXT_VER' ) && class_exists( 'divino_Ext_Extension' ) && divino_Ext_Extension::is_active( 'sticky-header' ) ) {
				$document->add_control(
					'stick-header-meta',
					array(
						'classes' => 'ast-selector-control',
				...
				$controls = divino_Meta_Boxes::get_instance()->get_sticky_header_options();
				foreach ( $controls as $details ) {
					$meta_key = $details['key'];
					$document->add_control(
						$meta_key,
		...
		public function add_custom_control_style() {
			...
			echo class_exists( 'divino_Minify' ) ? divino_Minify::trim_css( $css_rules ) : $css_rules;
		}
		...
		public function handle_preview() {
			...
			$keys     = self::get_divino_elementor_setting_keys();
		...
		public static function get_svg( $icon = '' ) {
			...
			$svg_icons = divino_Builder_UI_Controller::$ast_svgs;
			if ( ! divino_Builder_UI_Controller::$ast_svgs ) {
				divino_Builder_UI_Controller::fetch_svg_icon();
				$svg_icons = divino_Builder_UI_Controller::$ast_svgs;
			}
 ...
 */
divino_Elementor_Editor_Settings::get_instance();
