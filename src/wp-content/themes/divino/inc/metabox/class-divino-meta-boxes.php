 ...
 * Meta Boxes setup
 */
if ( ! class_exists( 'divino_Meta_Boxes' ) ) {

	/**
	 * Meta Boxes setup
	 */
	class divino_Meta_Boxes {
		/**
		 * Instance
		 *
		 * @var $instance
		...
		public function remove_metabox() {
			...
			if ( 'fl-theme-layout' === $post_type && $post_id ) {
					remove_meta_box( 'divino_settings_meta_box', 'fl-theme-layout', 'side' );
			}
		}
		...
		public function markup_meta_box( $post ) {
			...
			wp_nonce_field( basename( __FILE__ ), 'divino_settings_meta_box' );
			...
			<div class="disable-section-meta-wrap components-base-control__field">
				...
				<div class="disable-section-meta">
					...
					if ( $show_meta_field && true === divino_Builder_Helper::$is_header_footer_builder_active && divino_Builder_Helper::is_row_empty( 'above', 'header', 'desktop' ) ) {
						/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
						?>
					...
					<?php
					/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
					if ( $show_meta_field && divino_Builder_Helper::is_row_empty( 'primary', 'header', 'desktop' ) ) {
						/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
						?>
					...
					<?php
					/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
					if ( $show_meta_field && true === divino_Builder_Helper::$is_header_footer_builder_active && divino_Builder_Helper::is_row_empty( 'below', 'header', 'desktop' ) ) {
						/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
						?>
					...
					<?php
					/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
					if ( $show_meta_field && true === divino_Builder_Helper::$is_header_footer_builder_active && ( divino_Builder_Helper::is_row_empty( 'primary', 'header', 'mobile' ) || divino_Builder_Helper::is_row_empty( 'above', 'header', 'mobile' ) || divino_Builder_Helper::is_row_empty( 'below', 'header', 'mobile' ) ) ) {
						/** @psalm-suppress DocblockTypeContradiction */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
						?>
					...
					<?php do_action( 'divino_meta_box_markup_disable_sections_after_primary_header', $meta ); ?>
					<?php if ( $show_meta_field ) { ?>
						...
						<?php
						$ast_breadcrumbs_content = divino_get_option( 'ast-breadcrumbs-content' );
						if ( 'disabled' !== $ast_breadcrumbs_content && 'none' !== divino_get_option( 'breadcrumb-position' ) ) {
							?>
					<div class="ast-breadcrumbs-content-option-wrap">
						...
						if ( ( in_array( $post_type, $supported_post_types ) && true === divino_get_option( 'ast-single-' . $post_type . '-title', class_exists( 'WooCommerce' ) && 'product' === $post_type ? false : true ) ) ) {
					...
					<?php
					$footer_adv_layout = divino_get_option( 'footer-adv' );

					if ( $show_meta_field && ( 'disabled' !== $footer_adv_layout && ! divino_Builder_Helper::$is_header_footer_builder_active ) ) {
						?>
					...
					$footer_sml_layout = divino_get_option( 'footer-sml-layout' );
					if ( 'disabled' !== $footer_sml_layout || divino_Builder_Helper::$is_header_footer_builder_active ) {
						?>
					...
					?>
					<?php do_action( 'divino_meta_box_markup_disable_sections_after', $meta ); ?>
				</div>
			</div>
			<?php

			do_action( 'divino_meta_box_markup_after', $meta );
		}
		...
		public function save_meta_box( $post_id ) {
			...
			$is_valid_nonce = isset( $_POST['divino_settings_meta_box'] ) && wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['divino_settings_meta_box'] ) ), basename( __FILE__ ) ) ? true : false;
			...
			foreach ( $post_meta as $key => $data ) {
				...
				switch ( $sanitize_filter ) {
					...
					case 'FILTER_DEFAULT':
						 ...
						 */
						$meta_value = apply_filters( 'divino_php_default_filter_input', ! empty( $_POST[ $key ] ) ? sanitize_text_field( wp_unslash( $_POST[ $key ] ) ) : '', $key );
						/** @psalm-suppress PossiblyInvalidArgument */
						break;
		...
		public function load_scripts() {
			...
			if ( defined( 'divino_ADVANCED_HOOKS_POST_TYPE' ) && divino_ADVANCED_HOOKS_POST_TYPE === $post_type ) {
				return;
			}
			...
			wp_localize_script(
				...
				array(
					...
					'is_bb_themer_layout'            => ! divino_check_is_bb_themer_layout(), // Show page header option only when bb is not activated.
					'is_addon_activated'             => defined( 'divino_EXT_VER' ) ? true : false,
					'sticky_addon_enabled'           => $divino_ext_extension_class_exists && divino_Ext_Extension::is_active( 'sticky-header' ) ? true : false,
					'register_divino_metabox'         => apply_filters( 'divino_settings_metabox_register', $register_divino_metabox ),
					'is_hide_contnet_layout_sidebar' => $ast_content_layout_sidebar,
					'upgrade_pro_link'               => divino_get_pro_url( '/pricing/', 'free-theme', 'customizer', 'upgrade' ),
					'show_upgrade_notice'            => divino_showcase_upgrade_notices(),
					// Flag needed to check whether user is old or new, true for old user, false for new.
					'v4_1_6_migration'               => ! divino_Dynamic_CSS::divino_fullwidth_sidebar_support(),
					'color_addon_enabled'            => $divino_ext_extension_class_exists && divino_Ext_Extension::is_active( 'colors-and-background' ) ? true : false,
		 ...
		 *
		 * @return array $divino_header_options All header dependent toggle based page elements.
		 */
		public function get_header_disable_meta_fields() {
			...
			if (
				divino_Builder_Helper::is_row_empty( 'above', 'header', 'mobile' ) ||
				divino_Builder_Helper::is_row_empty( 'primary', 'header', 'mobile' ) ||
				divino_Builder_Helper::is_row_empty( 'below', 'header', 'mobile' )
			) {
				$divino_header_options[] = array(
			...
			return $divino_header_options;
		}

		/**
		 * Get disable section fields.
		 *
		 * @return array $divino_page_meta_elements All toggle based page elements.
		 */
		public function get_disable_section_fields() {
			...
			return $divino_page_meta_elements;
		}
		...
		public function get_sticky_header_options() {
			...
			if ( defined( 'divino_EXT_VER' ) && divino_Ext_Extension::is_active( 'sticky-header' ) ) {
				/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
				$sticky_on_devices = divino_get_option( 'sticky-header-on-devices' );
				switch ( $sticky_on_devices ) {
					case 'desktop':
						$sticky_above_header_condition   = divino_Builder_Helper::is_row_empty( 'above', 'header', 'desktop' );
						$sticky_primary_header_condition = divino_Builder_Helper::is_row_empty( 'primary', 'header', 'desktop' );
						$sticky_below_header_condition   = divino_Builder_Helper::is_row_empty( 'below', 'header', 'desktop' );
						break;
					case 'mobile':
						$sticky_above_header_condition   = divino_Builder_Helper::is_row_empty( 'above', 'header', 'mobile' );
						$sticky_primary_header_condition = divino_Builder_Helper::is_row_empty( 'primary', 'header', 'mobile' );
						$sticky_below_header_condition   = divino_Builder_Helper::is_row_empty( 'below', 'header', 'mobile' );
						break;
					default:
						$sticky_above_header_condition   = divino_Builder_Helper::is_row_empty( 'above', 'header', 'desktop' ) || divino_Builder_Helper::is_row_empty( 'above', 'header', 'mobile' ) ? true : false;
						$sticky_primary_header_condition = divino_Builder_Helper::is_row_empty( 'primary', 'header', 'desktop' ) || divino_Builder_Helper::is_row_empty( 'primary', 'header', 'mobile' ) ? true : false;
						$sticky_below_header_condition   = divino_Builder_Helper::is_row_empty( 'below', 'header', 'desktop' ) || divino_Builder_Helper::is_row_empty( 'below', 'header', 'mobile' ) ? true : false;
						break;
				}
			}
			...
			return $divino_sticky_header_options;
		}
		...
		public function check_page_header_availability() {
			...
			if ( class_exists( 'divino_Ext_Extension' ) && ! divino_Ext_Extension::is_active( 'advanced-headers' ) ) {
				return false;
			}

			/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$page_headers = divino_Target_Rules_Fields::get_post_selection( 'divino_adv_header' );
			/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
		...
		public function get_page_header_options() {
			...
			/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
			$header_options = divino_Target_Rules_Fields::get_post_selection( 'divino_adv_header' );
			/** @psalm-suppress UndefinedClass */ // phpcs:ignore Generic.Commenting.DocComment.MissingShort
 ...
 * @since 3.9.4
 */
function divino_footer_bar_display_cb( $display_footer ) {
...
add_filter( 'divino_footer_bar_display', 'divino_footer_bar_display_cb', 99, 1 );

/**
 * Kicking this off by calling 'get_instance()' method
 */
divino_Meta_Boxes::get_instance();
