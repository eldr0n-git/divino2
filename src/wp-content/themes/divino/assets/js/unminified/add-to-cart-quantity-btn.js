...
function astrawpWooQuantityButtons( $quantitySelector ) {
    ...
    for ( var i = 0; i < $quantityBoxesWrap.length; i++ ) {
        ...
        if ( $quantityBoxes && 'date' !== $quantityBoxes.getAttribute( 'type' ) && 'hidden' !== $quantityBoxes.getAttribute( 'type' ) ) {
            ...
            const minusBtn = `<span class="screen-reader-text">${ divino_qty_btn.minus_qty }</span><a href="javascript:void(0)" id="minus_qty-${ i }" class="minus %s">-</a>`;
            const plusBtn = `<span class="screen-reader-text">${ divino_qty_btn.plus_qty }</span><a href="javascript:void(0)" id="plus_qty-${ i }" class="plus %s">+</a>`;

            if ( 'vertical-icon' === divino_qty_btn.style_type ) {
                ...
                let styleTypeClass = '';
                if ( 'no-internal-border' === divino_qty_btn.style_type ) {
...
function sendAjaxQuantityRequest(currentTarget, quantity, itemHash ) {
    ...
    if ( miniCart && astra && astra.single_product_qty_ajax_nonce && astra.ajax_url ) {
        ...
        xhrRequest.send( 'action=divino_add_cart_single_product_quantity&hash=' + itemHash + '&quantity=' + quantity + '&qtyNonce=' + qtyNonce );
