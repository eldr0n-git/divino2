...
(function () {
	...
	function getSearchResultPostMarkup(resultsData) {
		...
		Object.entries(resultsData).forEach(([postType, postsData]) => {
			let postTypeLabel = divino_search.search_post_types_labels[postType]
				? divino_search.search_post_types_labels[postType]
				: postType + "s";
	...
	window.addEventListener("load", function (e) {
		...
		searchInputs.forEach((searchInput) => {
			searchInput.addEventListener("input", function (event) {
				...
				const postTypes = divino_search.search_page_condition ? divino_search.search_page_post_types : divino_search.search_post_types;
				...
				try {
					const restRequest = `${
						divino_search.rest_api_url
					}wp/v2/posts${
						divino_search.rest_api_url.indexOf("?") > -1 ? "&" : "?"
					}_embed=1&post_type=ast_queried:${postTypes.join(
						":"
					)}&per_page=${
						divino_search.search_posts_per_page
					}&search=${searchTerm}${
						divino_search.search_language
							? `&lang=${divino_search.search_language}`
							: ""
					}`;
								...
								resultsContainer = `
									...
									>
										<label class="ast-search--no-results-heading"> ${
											divino_search.no_live_results_found
										} </label>
									</div>
								`;
							}
